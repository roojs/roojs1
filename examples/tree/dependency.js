...
Roo.onReady(function(){
    ...
    var cview = Roo.DomHelper.append('main-ct',
        {cn:[{id:'main-tb'},{id:'cbody'}]}
    );


    // create the primary toolbar
    var tb = new Roo.Toolbar('main-tb');
    tb.add({
        id:'save',
        text:'Save',
        disabled:true,
    ...
    ctree.el.addKeyListener(Roo.EventObject.DELETE, removeNode);
    ...
    ctree.on('beforenodedrop', function(e){
        ...
        if(isSourceCopy(e, n)){
            var copy = new xt.TreeNode(
                Roo.apply({allowDelete:true,expanded:true}, n.attributes)
            );
    ...
    function save(){
        ...
        croot.eachChild(function(c){
            ...
            if(!onode.childrenRendered){
                ...
                onode.eachChild(function(o){
                    var opt = Roo.apply({}, o.attributes);
        ...
        Roo.lib.Ajax.request(
            'POST',
            'save-dep.php',
            {success:hide,failure:hide},
            'data='+encodeURIComponent(Roo.encode(ch))
        );
    }
