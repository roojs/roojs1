...
Roo.ComponentMgr=function(){var A=new Roo.util.MixedCollection();return {register:function(c){A.add(c);},unregister:function(c){A.remove(c);},get:function(id){return A.get(id);},onAvailable:function(id,fn,C){A.on("add",function(D,o){if(o.id==id){fn.call(C||o,o);A.un("add",fn,C);}});}};}();
...
Roo.ComponentMgr.register(this);Roo.Component.superclass.constructor.call(this);this.initComponent();if(this.renderTo){this.render(this.renderTo);delete this.renderTo;}};Roo.Component.AUTO_ID=1000;Roo.extend(Roo.Component,Roo.util.Observable,{hidden:false,disabled:false,rendered:false,disabledClass:"x-item-disabled",allowDomMove:true,hideMode:'display',ctype:"Roo.Component",actionMode:"el",getActionEl:function(){return this[this.actionMode];},initComponent:Roo.emptyFn,render:function(B,C){if(!this.rendered&&this.fireEvent("beforerender",this)!==false){if(!B&&this.el){this.el=Roo.get(this.el);B=this.el.dom.parentNode;this.allowDomMove=false;}
this.container=Roo.get(B);this.rendered=true;if(C!==undefined){if(typeof C=='number'){C=this.container.dom.childNodes[C];}else {C=Roo.getDom(C);}}
this.onRender(this.container,C||null);if(this.cls){this.el.addClass(this.cls);delete this.cls;}if(this.style){this.el.applyStyles(this.style);delete this.style;}
this.fireEvent("render",this);this.afterRender(this.container);if(this.hidden){this.hide();}if(this.disabled){this.disable();}}return this;},onRender:function(ct,D){if(this.el){this.el=Roo.get(this.el);if(this.allowDomMove!==false){ct.dom.insertBefore(this.el.dom,D);}}},getAutoCreate:function(){var E=typeof this.autoCreate=="object"?this.autoCreate:Roo.apply({},this.defaultAutoCreate);if(this.id&&!E.id){E.id=this.id;}return E;},afterRender:Roo.emptyFn,destroy:function(){if(this.fireEvent("beforedestroy",this)!==false){this.purgeListeners();this.beforeDestroy();if(this.rendered){this.el.removeAllListeners();this.el.remove();if(this.actionMode=="container"){this.container.remove();}}
this.onDestroy();Roo.ComponentMgr.unregister(this);this.fireEvent("destroy",this);}},beforeDestroy:function(){},onDestroy:function(){},getEl:function(){return this.el;},getId:function(){return this.id;},focus:function(F){if(this.rendered){this.el.focus();if(F===true){this.el.dom.select();}}return this;},blur:function(){if(this.rendered){this.el.blur();}return this;},disable:function(){if(this.rendered){this.onDisable();}
...
this.disabled=false;this.fireEvent("enable",this);return this;},onEnable:function(){this.getActionEl().removeClass(this.disabledClass);this.el.dom.disabled=false;},setDisabled:function(G){this[G?"disable":"enable"]();},show:function(){if(this.fireEvent("beforeshow",this)!==false){this.hidden=false;if(this.rendered){this.onShow();}
...
this.fireEvent("hide",this);}return this;},onHide:function(){var ae=this.getActionEl();if(this.hideMode=='visibility'){ae.dom.style.visibility="hidden";}else if(this.hideMode=='offsets'){ae.addClass('x-hidden');}else {ae.dom.style.display="none";}},setVisible:function(H){if(H){this.show();}else {this.hide();}return this;},isVisible:function(){return this.getActionEl().isVisible();},cloneConfig:function(I){I=I||{};var id=I.id||Roo.id();var J=Roo.applyIf(I,this.initialConfig);J.id=id;return new this.constructor(J);}});
...
C.setStyle('z-index',this.getZIndex()-2);this.shim=C;return C;},hideShim:function(){if(this.shim){this.shim.setDisplayed(false);B.push(this.shim);delete this.shim;}},disableShadow:function(){if(this.shadow){this.shadowDisabled=true;this.shadow.hide();this.lastShadowOffset=this.shadowOffset;this.shadowOffset=0;}},enableShadow:function(D){if(this.shadow){this.shadowDisabled=false;this.shadowOffset=this.lastShadowOffset;delete this.lastShadowOffset;if(D){this.sync(true);}}},sync:function(E){var sw=this.shadow;if(!this.updating&&this.isVisible()&&(sw||this.useShim)){var sh=this.getShim();var w=this.getWidth(),h=this.getHeight();var l=this.getLeft(true),t=this.getTop(true);if(sw&&!this.shadowDisabled){if(E&&!sw.isVisible()){sw.show(this);}else {sw.realign(l,t,w,h);}if(sh){if(E){sh.show();}var a=sw.adjusts,s=sh.dom.style;s.left=(Math.min(l,l+a.l))+"px";s.top=(Math.min(t,t+a.t))+"px";s.width=(w+a.w)+"px";s.height=(h+a.h)+"px";}}else if(sh){if(E){sh.show();}
...
Roo.Element.uncache(this.id);},remove:function(){this.destroy();},beginUpdate:function(){this.updating=true;},endUpdate:function(){this.updating=false;this.sync(true);},hideUnders:function(F){if(this.shadow){this.shadow.hide();}
this.hideShim();},constrainXY:function(){if(this.constrain){var vw=Roo.lib.Dom.getViewWidth(),vh=Roo.lib.Dom.getViewHeight();var s=Roo.get(document).getScroll();var xy=this.getXY();var x=xy[0],y=xy[1];var w=this.dom.offsetWidth+this.shadowOffset,h=this.dom.offsetHeight+this.shadowOffset;var moved=false;if((x+w)>vw+s.left){x=vw-w-this.shadowOffset;moved=true;}if((y+h)>vh+s.top){y=vh-h-this.shadowOffset;moved=true;}if(x<s.left){x=s.left;moved=true;}if(y<s.top){y=s.top;moved=true;}if(moved){if(this.avoidY){var ay=this.avoidY;if(y<=ay&&(y+h)>=ay){y=ay-h-5;}}
...
this.lastSize={width:w,height:h};var B=this.adjustSize(w,h);var aw=B.width,ah=B.height;if(aw!==undefined||ah!==undefined){var rz=this.getResizeEl();if(!this.deferHeight&&aw!==undefined&&ah!==undefined){rz.setSize(aw,ah);}else if(!this.deferHeight&&ah!==undefined){rz.setHeight(ah);}else if(aw!==undefined){rz.setWidth(aw);}
this.onResize(aw,ah,w,h);this.fireEvent('resize',this,aw,ah,w,h);}return this;},getSize:function(){return this.el.getSize();},getPosition:function(C){if(C===true){return [this.el.getLeft(true),this.el.getTop(true)];}return this.xy||this.el.getXY();},getBox:function(D){var s=this.el.getSize();if(D){s.x=this.el.getLeft(true);s.y=this.el.getTop(true);}else {var xy=this.xy||this.el.getXY();s.x=xy[0];s.y=xy[1];}return s;},updateBox:function(E){this.setSize(E.width,E.height);this.setPagePosition(E.x,E.y);return this;},getResizeEl:function(){return this.resizeEl||this.el;},getPositionEl:function(){return this.positionEl||this.el;},setPosition:function(x,y){this.x=x;this.y=y;if(!this.boxReady){return this;}var F=this.adjustPosition(x,y);var ax=F.x,ay=F.y;var el=this.getPositionEl();if(ax!==undefined||ay!==undefined){if(ax!==undefined&&ay!==undefined){el.setLeftTop(ax,ay);}else if(ax!==undefined){el.setLeft(ax);}else if(ay!==undefined){el.setTop(ay);}
...
this.overlay.setSize(Roo.lib.Dom.getViewWidth(true),Roo.lib.Dom.getViewHeight(true));this.overlay.show();Roo.get(this.proxy).setDisplayed("block");var F=this.adapter.getElementSize(this);this.activeMinSize=this.getMinimumSize();;this.activeMaxSize=this.getMaximumSize();;var c1=F-this.activeMinSize;var c2=Math.max(this.activeMaxSize-F,0);if(this.orientation==Roo.SplitBar.HORIZONTAL){this.dd.resetConstraints();this.dd.setXConstraint(this.placement==Roo.SplitBar.LEFT?c1:c2,this.placement==Roo.SplitBar.LEFT?c2:c1);this.dd.setYConstraint(0,0);}else {this.dd.resetConstraints();this.dd.setXConstraint(0,0);this.dd.setYConstraint(this.placement==Roo.SplitBar.TOP?c1:c2,this.placement==Roo.SplitBar.TOP?c2:c1);}
...
e.preventDefault();}return true;},getSelectionCount:function(){return this.selections.length;},getSelectedNodes:function(){return this.selections;},getSelectedIndexes:function(){var X=[],s=this.selections;for(var i=0,len=s.length;i<len;i++){X.push(s[i].nodeIndex);}return X;},clearSelections:function(Y){if(this.nodes&&(this.multiSelect||this.singleSelect)&&this.selections.length>0){this.cmp.elements=this.selections;this.cmp.removeClass(this.selectedClass);this.selections=[];if(!Y){this.fireEvent("selectionchange",this,this.selections);}}},isSelected:function(Z){var s=this.selections;if(s.length<1){return false;}
Z=this.getNode(Z);return s.indexOf(Z)!==-1;},select:function(a,b,c){if(a instanceof Array){if(!b){this.clearSelections(true);}for(var i=0,len=a.length;i<len;i++){this.select(a[i],true,true);}}else {var Z=this.getNode(a);if(Z&&!this.isSelected(Z)){if(!b){this.clearSelections(true);}if(this.fireEvent("beforeselect",this,Z,this.selections)!==false){Roo.fly(Z).addClass(this.selectedClass);this.selections.push(Z);if(!c){this.fireEvent("selectionchange",this,this.selections);}}}}},getNode:function(f){if(typeof f=="string"){return document.getElementById(f);}else if(typeof f=="number"){return this.nodes[f];}return f;},getNodes:function(g,h){var ns=this.nodes;g=g||0;h=typeof h=="undefined"?ns.length-1:h;var j=[];if(g<=h){for(var i=g;i<=h;i++){j.push(ns[i]);}}else {for(var i=g;i>=h;i--){j.push(ns[i]);}}return j;},indexOf:function(k){k=this.getNode(k);if(typeof k.nodeIndex=="number"){return k.nodeIndex;}var ns=this.nodes;for(var i=0,len=ns.length;i<len;i++){if(ns[i]==k){return i;}}return -1;}});
Roo.JsonView=function(A,B,C){Roo.JsonView.superclass.constructor.call(this,A,B,C);var um=this.el.getUpdateManager();um.setRenderer(this);um.on("update",this.onLoad,this);um.on("failure",this.onLoadException,this);this.addEvents({'beforerender':true,'load':true,'loadexception':true});};Roo.extend(Roo.JsonView,Roo.View,{jsonRoot:"",refresh:function(){this.clearSelections();this.el.update("");var D=[];var o=this.jsonData;if(o&&o.length>0){for(var i=0,len=o.length;i<len;i++){var data=this.prepareData(o[i],i,o);D[D.length]=this.tpl.apply(data);}}else {D.push(this.emptyText);}
this.el.update(D.join(""));this.nodes=this.el.dom.childNodes;this.updateIndexes(0);},load:function(){var um=this.el.getUpdateManager();um.update.apply(um,arguments);},render:function(el,E){this.clearSelections();this.el.update("");var o;try{o=Roo.util.JSON.decode(E.responseText);if(this.jsonRoot){o=eval("o."+this.jsonRoot);}}catch(e){}
this.jsonData=o;this.beforeRender();this.refresh();},getCount:function(){return this.jsonData?this.jsonData.length:0;},getNodeData:function(F){if(F instanceof Array){var data=[];for(var i=0,len=F.length;i<len;i++){data.push(this.getNodeData(F[i]));}return data;}return this.jsonData[this.indexOf(F)]||null;},beforeRender:function(){this.snapshot=this.jsonData;if(this.sortInfo){this.sort.apply(this,this.sortInfo);}
this.fireEvent("beforerender",this,this.jsonData);},onLoad:function(el,o){this.fireEvent("load",this,this.jsonData,o);},onLoadException:function(el,o){this.fireEvent("loadexception",this,o);},filter:function(G,H){if(this.jsonData){var data=[];var ss=this.snapshot;if(typeof H=="string"){var vlen=H.length;if(vlen==0){this.clearFilter();return;}
H=H.toLowerCase();for(var i=0,len=ss.length;i<len;i++){var o=ss[i];if(o[G].substr(0,vlen).toLowerCase()==H){data.push(o);}}}else if(H.exec){for(var i=0,len=ss.length;i<len;i++){var o=ss[i];if(H.test(o[G])){data.push(o);}}}else {return;}
this.jsonData=data;this.refresh();}},filterBy:function(fn,I){if(this.jsonData){var data=[];var ss=this.snapshot;for(var i=0,len=ss.length;i<len;i++){var o=ss[i];if(fn.call(I||this,o)){data.push(o);}}
this.jsonData=data;this.refresh();}},clearFilter:function(){if(this.snapshot&&this.jsonData!=this.snapshot){this.jsonData=this.snapshot;this.refresh();}},sort:function(J,K,L){this.sortInfo=Array.prototype.slice.call(arguments,0);if(this.jsonData){var p=J;var dsc=K&&K.toLowerCase()=="desc";var f=function(o1,o2){var v1=L?L(o1[p]):o1[p];var v2=L?L(o2[p]):o2[p];;if(v1<v2){return dsc?+1:-1;}else if(v1>v2){return dsc?-1:+1;}else {return 0;}};this.jsonData.sort(f);this.refresh();if(this.jsonData!=this.snapshot){this.snapshot.sort(f);}}}});
Roo.ColorPalette=function(A){Roo.ColorPalette.superclass.constructor.call(this,A);this.addEvents({select:true});if(this.handler){this.on("select",this.handler,this.scope,true);}};Roo.extend(Roo.ColorPalette,Roo.Component,{itemCls:"x-color-palette",value:null,clickEvent:'click',ctype:"Roo.ColorPalette",allowReselect:false,colors:["000000","993300","333300","003300","003366","000080","333399","333333","800000","FF6600","808000","008000","008080","0000FF","666699","808080","FF0000","FF9900","99CC00","339966","33CCCC","3366FF","800080","969696","FF00FF","FFCC00","FFFF00","00FF00","00FFFF","00CCFF","993366","C0C0C0","FF99CC","FFCC99","FFFF99","CCFFCC","CCFFFF","99CCFF","CC99FF","FFFFFF"],onRender:function(B,C){var t=new Roo.MasterTemplate('<tpl><a href="#" class="color-{0}" hidefocus="on"><em><span style="background:#{0}" unselectable="on">&#160;</span></em></a></tpl>');var c=this.colors;for(var i=0,len=c.length;i<len;i++){t.add([c[i]]);}var el=document.createElement("div");el.className=this.itemCls;t.overwrite(el);B.dom.insertBefore(el,C);this.el=Roo.get(el);this.el.on(this.clickEvent,this.handleClick,this,{delegate:"a"});if(this.clickEvent!='click'){this.el.on('click',Roo.emptyFn,this,{delegate:"a",preventDefault:true});}},afterRender:function(){Roo.ColorPalette.superclass.afterRender.call(this);if(this.value){var s=this.value;this.value=null;this.select(s);}},handleClick:function(e,t){e.preventDefault();if(!this.disabled){var c=t.className.match(/(?:^|\s)color-(.{6})(?:\s|$)/)[1];this.select(c.toUpperCase());}},select:function(D){D=D.replace("#","");if(D!=this.value||this.allowReselect){var el=this.el;if(this.value){el.child("a.color-"+this.value).removeClass("x-color-palette-sel");}
el.child("a.color-"+D).addClass("x-color-palette-sel");this.value=D;this.fireEvent("select",this,D);}}});
Roo.DatePicker=function(A){Roo.DatePicker.superclass.constructor.call(this,A);this.value=A&&A.value?A.value.clearTime():new Date().clearTime();this.addEvents({select:true});if(this.handler){this.on("select",this.handler,this.scope||this);}if(!this.disabledDatesRE&&this.disabledDates){var dd=this.disabledDates;var re="(?:";for(var i=0;i<dd.length;i++){re+=dd[i];if(i!=dd.length-1)re+="|";}
this.disabledDatesRE=new RegExp(re+")");}};Roo.extend(Roo.DatePicker,Roo.Component,{todayText:"Today",okText:"&#160;OK&#160;",cancelText:"Cancel",todayTip:"{0} (Spacebar)",minDate:null,maxDate:null,minText:"This date is before the minimum date",maxText:"This date is after the maximum date",format:"m/d/y",disabledDays:null,disabledDaysText:"",disabledDatesRE:null,disabledDatesText:"",constrainToViewport:true,monthNames:Date.monthNames,dayNames:Date.dayNames,nextText:'Next Month (Control+Right)',prevText:'Previous Month (Control+Left)',monthYearText:'Choose a month (Control+Up/Down to move years)',startDay:0,showClear:false,setValue:function(B){var C=this.value;this.value=B.clearTime(true);if(this.el){this.update(this.value);}},getValue:function(){return this.value;},focus:function(){if(this.el){this.update(this.activeDate);}},onRender:function(D,E){var m=['<table cellspacing="0">','<tr><td class="x-date-left"><a href="#" title="',this.prevText,'">&#160;</a></td><td class="x-date-middle" align="center"></td><td class="x-date-right"><a href="#" title="',this.nextText,'">&#160;</a></td></tr>','<tr><td colspan="3"><table class="x-date-inner" cellspacing="0"><thead><tr>'];var dn=this.dayNames;for(var i=0;i<7;i++){var d=this.startDay+i;if(d>6){d=d-7;}
...
this.items=[];this.bodyEl.setStyle("position","relative");this.active=null;this.activateDelegate=this.activate.createDelegate(this);this.addEvents({"tabchange":true,"beforetabchange":true});Roo.EventManager.onWindowResize(this.onResize,this);this.cpad=this.el.getPadding("lr");this.hiddenCount=0;Roo.TabPanel.superclass.constructor.call(this);};Roo.extend(Roo.TabPanel,Roo.util.Observable,{tabPosition:"top",currentTabWidth:0,minTabWidth:40,maxTabWidth:250,preferredTabWidth:175,resizeTabs:false,monitorResize:true,addTab:function(id,C,D,E){var F=new Roo.TabPanelItem(this,id,C,E);this.addTabItem(F);if(D){F.setContent(D);}return F;},getTab:function(id){return this.items[id];},hideTab:function(id){var t=this.items[id];if(!t.isHidden()){t.setHidden(true);this.hiddenCount++;this.autoSizeTabs();}},unhideTab:function(id){var t=this.items[id];if(t.isHidden()){t.setHidden(false);this.hiddenCount--;this.autoSizeTabs();}},addTabItem:function(G){this.items[G.id]=G;this.items.push(G);if(this.resizeTabs){G.setWidth(this.currentTabWidth||this.preferredTabWidth);this.autoSizeTabs();}else {G.autoSize();}},removeTab:function(id){var H=this.items;var I=H[id];if(!I)return;var J=H.indexOf(I);if(this.active==I&&H.length>1){var newTab=this.getNextAvailable(J);if(newTab)newTab.activate();}
this.stripEl.dom.removeChild(I.pnode.dom);if(I.bodyEl.dom.parentNode==this.bodyEl.dom){this.bodyEl.dom.removeChild(I.bodyEl.dom);}
H.splice(J,1);delete this.items[I.id];I.fireEvent("close",I);I.purgeListeners();this.autoSizeTabs();},getNextAvailable:function(K){var L=this.items;var M=K;while(M<L.length){var G=L[++M];if(G&&!G.isHidden()){return G;}}
M=K;while(M>=0){var G=L[--M];if(G&&!G.isHidden()){return G;}}return null;},disableTab:function(id){var N=this.items[id];if(N&&this.active!=N){N.disable();}},enableTab:function(id){var O=this.items[id];O.enable();},activate:function(id){var P=this.items[id];if(!P){return null;}if(P==this.active||P.disabled){return P;}var e={};this.fireEvent("beforetabchange",this,e,P);if(e.cancel!==true&&!P.disabled){if(this.active){this.active.hide();}
this.active=this.items[id];this.active.show();this.fireEvent("tabchange",this,this.active);}return P;},getActiveTab:function(){return this.active;},syncHeight:function(Q){var R=(Q||this.el.getHeight())-this.el.getBorderWidth("tb")-this.el.getPadding("tb");var bm=this.bodyEl.getMargins();var S=R-(this.stripWrap.getHeight()||0)-(bm.top+bm.bottom);this.bodyEl.setHeight(S);return S;},onResize:function(){if(this.monitorResize){this.autoSizeTabs();}},beginUpdate:function(){this.updating=true;},endUpdate:function(){this.updating=false;this.autoSizeTabs();},autoSizeTabs:function(){var T=this.items.length;var U=T-this.hiddenCount;if(!this.resizeTabs||T<1||U<1||this.updating)return;var w=Math.max(this.el.getWidth()-this.cpad,10);var V=Math.floor(w/U);var b=this.stripBody;if(b.getWidth()>w){var tabs=this.items;this.setTabWidth(Math.max(V,this.minTabWidth)-2);if(V<this.minTabWidth){}}else {if(this.currentTabWidth<this.preferredTabWidth){this.setTabWidth(Math.min(V,this.preferredTabWidth)-2);}}},getCount:function(){return this.items.length;},setTabWidth:function(W){this.currentTabWidth=W;for(var i=0,len=this.items.length;i<len;i++){if(!this.items[i].isHidden())this.items[i].setWidth(W);}},destroy:function(X){Roo.EventManager.removeResizeListener(this.onResize,this);for(var i=0,len=this.items.length;i<len;i++){this.items[i].purgeListeners();}if(X===true){this.el.update("");this.el.remove();}}});Roo.TabPanelItem=function(Y,id,Z,a){this.tabPanel=Y;this.id=id;this.disabled=false;this.text=Z;this.loaded=false;this.closable=a;this.bodyEl=Roo.get(Y.createItemBody(Y.bodyEl.dom,id));this.bodyEl.setVisibilityMode(Roo.Element.VISIBILITY);this.bodyEl.setStyle("display","block");this.bodyEl.setStyle("zoom","1");this.hideAction();var d=Y.createStripElements(Y.stripEl.dom,Z,a);this.el=Roo.get(d.el,true);this.inner=Roo.get(d.inner,true);this.textEl=Roo.get(this.el.dom.firstChild.firstChild.firstChild,true);this.pnode=Roo.get(d.el.parentNode,true);this.el.on("mousedown",this.onTabMouseDown,this);this.el.on("click",this.onTabClick,this);if(a){var c=Roo.get(d.close,true);c.dom.title=this.closeText;c.addClassOnOver("close-over");c.on("click",this.closeClick,this);}
...
this.fireEvent("activate",this.tabPanel,this);},isActive:function(){return this.tabPanel.getActiveTab()==this;},hide:function(){this.pnode.removeClass("on");this.hideAction();this.fireEvent("deactivate",this.tabPanel,this);},hideAction:function(){this.bodyEl.hide();this.bodyEl.setStyle("position","absolute");this.bodyEl.setLeft("-20000px");this.bodyEl.setTop("-20000px");},showAction:function(){this.bodyEl.setStyle("position","relative");this.bodyEl.setTop("");this.bodyEl.setLeft("");this.bodyEl.show();},setTooltip:function(f){if(Roo.QuickTips&&Roo.QuickTips.isEnabled()){this.textEl.dom.qtip=f;this.textEl.dom.removeAttribute('title');}else {this.textEl.dom.title=f;}},onTabClick:function(e){e.preventDefault();this.tabPanel.activate(this.id);},onTabMouseDown:function(e){e.preventDefault();this.tabPanel.activate(this.id);},getWidth:function(){return this.inner.getWidth();},setWidth:function(g){var h=g-this.pnode.getPadding("lr");this.inner.setWidth(h);this.textEl.setWidth(h-this.inner.getPadding("lr"));this.pnode.setWidth(g);},setHidden:function(j){this.hidden=j;this.pnode.setStyle("display",j?"none":"");},isHidden:function(){return this.hidden;},getText:function(){return this.text;},autoSize:function(){this.textEl.setWidth(1);this.setWidth(this.textEl.dom.scrollWidth+this.pnode.getPadding("lr")+this.inner.getPadding("lr"));},setText:function(k){this.text=k;this.textEl.update(k);this.setTooltip(k);if(!this.tabPanel.resizeTabs){this.autoSize();}},activate:function(){this.tabPanel.activate(this.id);},disable:function(){if(this.tabPanel.active!=this){this.disabled=true;this.pnode.addClass("disabled");}},enable:function(){this.disabled=false;this.pnode.removeClass("disabled");},setContent:function(l,m){this.bodyEl.update(l,m);},getUpdateManager:function(){return this.bodyEl.getUpdateManager();},setUrl:function(n,o,p){if(this.refreshDelegate){this.un('activate',this.refreshDelegate);}
this.refreshDelegate=this._handleRefresh.createDelegate(this,[n,o,p]);this.on("activate",this.refreshDelegate);return this.bodyEl.getUpdateManager();},_handleRefresh:function(q,r,s){if(!s||!this.loaded){var updater=this.bodyEl.getUpdateManager();updater.update(q,r,this._setLoaded.createDelegate(this));}},refresh:function(){if(this.refreshDelegate){this.loaded=false;this.refreshDelegate();}},_setLoaded:function(){this.loaded=true;},closeClick:function(e){var o={};e.stopEvent();this.fireEvent("beforeclose",this,o);if(o.cancel!==true){this.tabPanel.removeTab(this.id);}},closeText:"Close this tab"});Roo.TabPanel.prototype.createStrip=function(u){var v=document.createElement("div");v.className="x-tabs-wrap";u.appendChild(v);return v;};Roo.TabPanel.prototype.createStripList=function(x){x.innerHTML='<div class="x-tabs-strip-wrap"><table class="x-tabs-strip" cellspacing="0" cellpadding="0" border="0"><tbody><tr></tr></tbody></table></div>';return x.firstChild.firstChild.firstChild.firstChild;};Roo.TabPanel.prototype.createBody=function(y){var z=document.createElement("div");Roo.id(z,"tab-body");Roo.fly(z).addClass("x-tabs-body");y.appendChild(z);return z;};Roo.TabPanel.prototype.createItemBody=function(AA,id){var AB=Roo.getDom(id);if(!AB){AB=document.createElement("div");AB.id=id;}
Roo.fly(AB).addClass("x-tabs-item-body");AA.insertBefore(AB,AA.firstChild);return AB;};Roo.TabPanel.prototype.createStripElements=function(AC,AD,AE){var td=document.createElement("td");AC.appendChild(td);if(AE){td.className="x-tabs-closable";if(!this.closeTpl){this.closeTpl=new Roo.Template('<a href="#" class="x-tabs-right"><span class="x-tabs-left"><em class="x-tabs-inner">'+'<span unselectable="on"'+(this.disableTooltips?'':' title="{text}"')+' class="x-tabs-text">{text}</span>'+'<div unselectable="on" class="close-icon">&#160;</div></em></span></a>');}var el=this.closeTpl.overwrite(td,{"text":AD});var close=el.getElementsByTagName("div")[0];var inner=el.getElementsByTagName("em")[0];return {"el":el,"close":close,"inner":inner};}else {if(!this.tabTpl){this.tabTpl=new Roo.Template('<a href="#" class="x-tabs-right"><span class="x-tabs-left"><em class="x-tabs-inner">'+'<span unselectable="on"'+(this.disableTooltips?'':' title="{text}"')+' class="x-tabs-text">{text}</span></em></span></a>');}var el=this.tabTpl.overwrite(td,{"text":AD});var inner=el.getElementsByTagName("em")[0];return {"el":el,"inner":inner};}};
Roo.Button=function(A,B){if(!B){B=A;A=B.renderTo||false;}
Roo.apply(this,B);this.addEvents({"click":true,"toggle":true,'mouseover':true,'mouseout':true,'render':true});if(this.menu){this.menu=Roo.menu.MenuMgr.get(this.menu);}if(A){this.render(A);}
Roo.util.Observable.call(this);};Roo.extend(Roo.Button,Roo.util.Observable,{hidden:false,disabled:false,pressed:false,tabIndex:undefined,enableToggle:false,menu:undefined,menuAlign:"tl-bl?",iconCls:undefined,type:'button',menuClassTarget:'tr',clickEvent:'click',handleMouseEvents:true,tooltipType:'qtip',render:function(C){var D;if(this.hideParent){this.parentEl=Roo.get(C);}if(!this.dhconfig){if(!this.template){if(!Roo.Button.buttonTemplate){Roo.Button.buttonTemplate=new Roo.Template('<table border="0" cellpadding="0" cellspacing="0" class="x-btn-wrap"><tbody><tr>','<td class="x-btn-left"><i>&#160;</i></td><td class="x-btn-center"><em unselectable="on"><button class="x-btn-text" type="{1}">{0}</button></em></td><td class="x-btn-right"><i>&#160;</i></td>',"</tr></tbody></table>");}
this.template=Roo.Button.buttonTemplate;}
D=this.template.append(C,[this.text||'&#160;',this.type],true);var btnEl=D.child("button:first");btnEl.on('focus',this.onFocus,this);btnEl.on('blur',this.onBlur,this);if(this.cls){D.addClass(this.cls);}if(this.icon){btnEl.setStyle('background-image','url('+this.icon+')');}if(this.iconCls){btnEl.addClass(this.iconCls);if(!this.cls){D.addClass(this.text?'x-btn-text-icon':'x-btn-icon');}}if(this.tabIndex!==undefined){btnEl.dom.tabIndex=this.tabIndex;}if(this.tooltip){if(typeof this.tooltip=='object'){Roo.QuickTips.tips(Roo.apply({target:btnEl.id},this.tooltip));}else {btnEl.dom[this.tooltipType]=this.tooltip;}}}else {D=Roo.DomHelper.append(Roo.get(C).dom,this.dhconfig,true);}
this.el=D;if(this.id){this.el.dom.id=this.el.id=this.id;}if(this.menu){this.el.child(this.menuClassTarget).addClass("x-btn-with-menu");this.menu.on("show",this.onMenuShow,this);this.menu.on("hide",this.onMenuHide,this);}
D.addClass("x-btn");if(Roo.isIE&&!Roo.isIE7){this.autoWidth.defer(1,this);}else {this.autoWidth();}if(this.handleMouseEvents){D.on("mouseover",this.onMouseOver,this);D.on("mouseout",this.onMouseOut,this);D.on("mousedown",this.onMouseDown,this);}
D.on(this.clickEvent,this.onClick,this);if(this.hidden){this.hide();}if(this.disabled){this.disable();}
Roo.ButtonToggleMgr.register(this);if(this.pressed){this.el.addClass("x-btn-pressed");}if(this.repeat){var repeater=new Roo.util.ClickRepeater(D,typeof this.repeat=="object"?this.repeat:{});repeater.on("click",this.onClick,this);}
this.fireEvent('render',this);},getEl:function(){return this.el;},destroy:function(){Roo.ButtonToggleMgr.unregister(this);this.el.removeAllListeners();this.purgeListeners();this.el.remove();},autoWidth:function(){if(this.el){this.el.setWidth("auto");if(Roo.isIE7&&Roo.isStrict){var ib=this.el.child('button');if(ib&&ib.getWidth()>20){ib.clip();ib.setWidth(Roo.util.TextMetrics.measure(ib,this.text).width+ib.getFrameWidth('lr'));}}if(this.minWidth){if(this.hidden){this.el.beginMeasure();}if(this.el.getWidth()<this.minWidth){this.el.setWidth(this.minWidth);}if(this.hidden){this.el.endMeasure();}}}},setHandler:function(E,F){this.handler=E;this.scope=F;},setText:function(G){this.text=G;if(this.el){this.el.child("td.x-btn-center button.x-btn-text").update(G);}
this.autoWidth();},getText:function(){return this.text;},show:function(){this.hidden=false;if(this.el){this[this.hideParent?'parentEl':'el'].setStyle("display","");}},hide:function(){this.hidden=true;if(this.el){this[this.hideParent?'parentEl':'el'].setStyle("display","none");}},setVisible:function(H){if(H){this.show();}else {this.hide();}},toggle:function(I){I=I===undefined?!this.pressed:I;if(I!=this.pressed){if(I){this.el.addClass("x-btn-pressed");this.pressed=true;this.fireEvent("toggle",this,true);}else {this.el.removeClass("x-btn-pressed");this.pressed=false;this.fireEvent("toggle",this,false);}if(this.toggleHandler){this.toggleHandler.call(this.scope||this,this,I);}}},focus:function(){this.el.child('button:first').focus();},disable:function(){if(this.el){this.el.addClass("x-btn-disabled");}
...
this.fireEvent("click",this,e);if(this.handler){this.el.removeClass("x-btn-over");this.handler.call(this.scope||this,this,e);}}},onMouseOver:function(e){if(!this.disabled){this.el.addClass("x-btn-over");this.fireEvent('mouseover',this,e);}},onMouseOut:function(e){if(!e.within(this.el,true)){this.el.removeClass("x-btn-over");this.fireEvent('mouseout',this,e);}},onFocus:function(e){if(!this.disabled){this.el.addClass("x-btn-focus");}},onBlur:function(e){this.el.removeClass("x-btn-focus");},onMouseDown:function(e){if(!this.disabled&&e.button==0){this.el.addClass("x-btn-click");Roo.get(document).on('mouseup',this.onMouseUp,this);}},onMouseUp:function(e){if(e.button==0){this.el.removeClass("x-btn-click");Roo.get(document).un('mouseup',this.onMouseUp,this);}},onMenuShow:function(e){this.el.addClass("x-btn-menu-active");},onMenuHide:function(e){this.el.removeClass("x-btn-menu-active");}});Roo.ButtonToggleMgr=function(){var J={};function K(L,M){if(M){var g=J[L.toggleGroup];for(var i=0,l=g.length;i<l;i++){if(g[i]!=L){g[i].toggle(false);}}}}return {register:function(N){if(!N.toggleGroup){return;}var g=J[N.toggleGroup];if(!g){g=J[N.toggleGroup]=[];}
g.push(N);N.on("toggle",K);},unregister:function(O){if(!O.toggleGroup){return;}var g=J[O.toggleGroup];if(g){g.remove(O);O.un("toggle",K);}}};}();
Roo.SplitButton=function(A,B){Roo.SplitButton.superclass.constructor.call(this,A,B);this.addEvents({"arrowclick":true});};Roo.extend(Roo.SplitButton,Roo.Button,{render:function(C){var D=new Roo.Template('<table cellspacing="0" class="x-btn-menu-wrap x-btn"><tr><td>','<table cellspacing="0" class="x-btn-wrap x-btn-menu-text-wrap"><tbody>','<tr><td class="x-btn-left"><i>&#160;</i></td><td class="x-btn-center"><button class="x-btn-text" type="{1}">{0}</button></td></tr>',"</tbody></table></td><td>",'<table cellspacing="0" class="x-btn-wrap x-btn-menu-arrow-wrap"><tbody>','<tr><td class="x-btn-center"><button class="x-btn-menu-arrow-el" type="button">&#160;</button></td><td class="x-btn-right"><i>&#160;</i></td></tr>',"</tbody></table></td></tr></table>");var E=D.append(C,[this.text,this.type],true);var F=E.child("button");if(this.cls){E.addClass(this.cls);}if(this.icon){F.setStyle('background-image','url('+this.icon+')');}if(this.iconCls){F.addClass(this.iconCls);if(!this.cls){E.addClass(this.text?'x-btn-text-icon':'x-btn-icon');}}
this.el=E;if(this.handleMouseEvents){E.on("mouseover",this.onMouseOver,this);E.on("mouseout",this.onMouseOut,this);E.on("mousedown",this.onMouseDown,this);E.on("mouseup",this.onMouseUp,this);}
E.on(this.clickEvent,this.onClick,this);if(this.tooltip){if(typeof this.tooltip=='object'){Roo.QuickTips.tips(Roo.apply({target:F.id},this.tooltip));}else {F.dom[this.tooltipType]=this.tooltip;}}if(this.arrowTooltip){E.child("button:nth(2)").dom[this.tooltipType]=this.arrowTooltip;}if(this.hidden){this.hide();}if(this.disabled){this.disable();}if(this.pressed){this.el.addClass("x-btn-pressed");}if(Roo.isIE&&!Roo.isIE7){this.autoWidth.defer(1,this);}else {this.autoWidth();}if(this.menu){this.menu.on("show",this.onMenuShow,this);this.menu.on("hide",this.onMenuHide,this);}
...
Roo.Resizable.superclass.constructor.call(this);};Roo.extend(Roo.Resizable,Roo.util.Observable,{resizeChild:false,adjustments:[0,0],minWidth:5,minHeight:5,maxWidth:10000,maxHeight:10000,enabled:true,animate:false,duration:.35,dynamic:false,handles:false,multiDirectional:false,disableTrackOver:false,easing:'easeOutStrong',widthIncrement:0,heightIncrement:0,pinned:false,width:null,height:null,preserveRatio:false,transparent:false,minX:0,minY:0,draggable:false,constrainTo:undefined,resizeRegion:undefined,resizeTo:function(C,D){this.el.setSize(C,D);this.updateChildSize();this.fireEvent("resize",this,C,D,null);},startSizing:function(e,E){this.fireEvent("beforeresize",this,e);if(this.enabled){if(!this.overlay){this.overlay=this.el.createProxy({tag:"div",cls:"x-resizable-overlay",html:"&#160;"});this.overlay.unselectable();this.overlay.enableDisplayMode("block");this.overlay.on("mousemove",this.onMouseMove,this);this.overlay.on("mouseup",this.onMouseUp,this);}
this.overlay.setStyle("cursor",E.el.getStyle("cursor"));this.resizing=true;this.startBox=this.el.getBox();this.startPoint=e.getXY();this.offsets=[(this.startBox.x+this.startBox.width)-this.startPoint[0],(this.startBox.y+this.startBox.height)-this.startPoint[1]];this.overlay.setSize(Roo.lib.Dom.getViewWidth(true),Roo.lib.Dom.getViewHeight(true));this.overlay.show();if(this.constrainTo){var ct=Roo.get(this.constrainTo);this.resizeRegion=ct.getRegion().adjust(ct.getFrameWidth('t'),ct.getFrameWidth('l'),-ct.getFrameWidth('b'),-ct.getFrameWidth('r'));}
this.proxy.setStyle('visibility','hidden');this.proxy.show();this.proxy.setBox(this.startBox);if(!this.dynamic){this.proxy.setStyle('visibility','visible');}}},onMouseDown:function(F,e){if(this.enabled){e.stopEvent();this.activeHandle=F;this.startSizing(e,F);}},onMouseUp:function(e){var G=this.resizeElement();this.resizing=false;this.handleOut();this.overlay.hide();this.proxy.hide();this.fireEvent("resize",this,G.width,G.height,e);},updateChildSize:function(){if(this.resizeChild){var el=this.el;var child=this.resizeChild;var adj=this.adjustments;if(el.dom.offsetWidth){var b=el.getSize(true);child.setSize(b.width+adj[0],b.height+adj[1]);}if(Roo.isIE){setTimeout(function(){if(el.dom.offsetWidth){var b=el.getSize(true);child.setSize(b.width+adj[0],b.height+adj[1]);}},10);}}},snap:function(H,I,J){if(!I||!H)return H;var K=H;var m=H%I;if(m>0){if(m>(I/2)){K=H+(I-m);}else {K=H-m;}}return Math.max(J,K);},resizeElement:function(){var L=this.proxy.getBox();if(this.updateBox){this.el.setBox(L,false,this.animate,this.duration,null,this.easing);}else {this.el.setSize(L.width,L.height,this.animate,this.duration,null,this.easing);}
this.updateChildSize();if(!this.dynamic){this.proxy.hide();}return L;},constrain:function(v,M,m,mx){if(v-M<m){M=v-m;}else if(v-M>mx){M=mx-v;}return M;},onMouseMove:function(e){if(this.enabled){try{if(this.resizeRegion&&!this.resizeRegion.contains(e.getPoint())){return;}var curSize=this.curSize||this.startBox;var x=this.startBox.x,y=this.startBox.y;var ox=x,oy=y;var w=curSize.width,h=curSize.height;var ow=w,oh=h;var mw=this.minWidth,mh=this.minHeight;var mxw=this.maxWidth,mxh=this.maxHeight;var wi=this.widthIncrement;var hi=this.heightIncrement;var eventXY=e.getXY();var diffX=-(this.startPoint[0]-Math.max(this.minX,eventXY[0]));var diffY=-(this.startPoint[1]-Math.max(this.minY,eventXY[1]));var pos=this.activeHandle.position;switch(pos){case "east":w+=diffX;w=Math.min(Math.max(mw,w),mxw);break;case "south":h+=diffY;h=Math.min(Math.max(mh,h),mxh);break;case "southeast":w+=diffX;h+=diffY;w=Math.min(Math.max(mw,w),mxw);h=Math.min(Math.max(mh,h),mxh);break;case "north":diffY=this.constrain(h,diffY,mh,mxh);y+=diffY;h-=diffY;break;case "west":diffX=this.constrain(w,diffX,mw,mxw);x+=diffX;w-=diffX;break;case "northeast":w+=diffX;w=Math.min(Math.max(mw,w),mxw);diffY=this.constrain(h,diffY,mh,mxh);y+=diffY;h-=diffY;break;case "northwest":diffX=this.constrain(w,diffX,mw,mxw);diffY=this.constrain(h,diffY,mh,mxh);y+=diffY;h-=diffY;x+=diffX;w-=diffX;break;case "southwest":diffX=this.constrain(w,diffX,mw,mxw);h+=diffY;h=Math.min(Math.max(mh,h),mxh);x+=diffX;w-=diffX;break;}var sw=this.snap(w,wi,mw);var sh=this.snap(h,hi,mh);if(sw!=w||sh!=h){switch(pos){case "northeast":y-=sh-h;break;case "north":y-=sh-h;break;case "southwest":x-=sw-w;break;case "west":x-=sw-w;break;case "northwest":x-=sw-w;y-=sh-h;break;}
...
this.field.show();this.field.on("blur",this.onBlur,this);if(this.field.grow){this.field.on("autosize",this.el.sync,this.el,{delay:1});}},onSpecialKey:function(D,e){if(this.completeOnEnter&&e.getKey()==e.ENTER){e.stopEvent();this.completeEdit();}else if(this.cancelOnEsc&&e.getKey()==e.ESC){this.cancelEdit();}else {this.fireEvent('specialkey',D,e);}},startEdit:function(el,E){if(this.editing){this.completeEdit();}
this.boundEl=Roo.get(el);var v=E!==undefined?E:this.boundEl.dom.innerHTML;if(!this.rendered){this.render(this.parentEl||document.body);}if(this.fireEvent("beforestartedit",this,this.boundEl,v)===false){return;}
...
this.fireEvent("startedit",this.boundEl,this.startValue);},deferredFocus:function(){if(this.editing){this.field.focus();}},cancelEdit:function(G){if(this.editing){this.setValue(this.startValue);if(G!==true){this.hide();}}},onBlur:function(){if(this.allowBlur!==true&&this.editing){this.completeEdit();}},onHide:function(){if(this.editing){this.completeEdit();return;}
...
this.fireEvent("resize",this,D,E);return this;},setContentSize:function(w,h){h+=this.getHeaderFooterHeight()+this.body.getMargins("tb");w+=this.body.getMargins("lr")+this.bwrap.getMargins("lr")+this.centerBg.getPadding("lr");h+=this.body.getPadding("tb")+this.bwrap.getBorderWidth("tb")+this.body.getBorderWidth("tb")+this.el.getBorderWidth("tb");w+=this.body.getPadding("lr")+this.bwrap.getBorderWidth("lr")+this.body.getBorderWidth("lr")+this.bwrap.getPadding("lr")+this.el.getBorderWidth("lr");if(this.tabs){h+=this.tabs.stripWrap.getHeight()+this.tabs.bodyEl.getMargins("tb")+this.tabs.bodyEl.getPadding("tb");w+=this.tabs.bodyEl.getMargins("lr")+this.tabs.bodyEl.getPadding("lr");}
this.resizeTo(w,h);return this;},addKeyListener:function(F,fn,G){var H,I,J,K;if(typeof F=="object"&&!(F instanceof Array)){H=F["key"];I=F["shift"];J=F["ctrl"];K=F["alt"];}else {H=F;}var L=function(M,e){if((!I||e.shiftKey)&&(!J||e.ctrlKey)&&(!K||e.altKey)){var k=e.getKey();if(H instanceof Array){for(var i=0,len=H.length;i<len;i++){if(H[i]==k){fn.call(G||window,M,k,e);return;}}}else {if(k==H){fn.call(G||window,M,k,e);}}}};this.on("keydown",L);return this;},getTabs:function(){if(!this.tabs){this.el.addClass("x-dlg-auto-tabs");this.body.addClass(this.tabPosition=="bottom"?"x-tabs-bottom":"x-tabs-top");this.tabs=new Roo.TabPanel(this.body.dom,this.tabPosition=="bottom");}return this.tabs;},addButton:function(M,N,O){var dh=Roo.DomHelper;if(!this.footer){this.footer=dh.append(this.bwrap,{tag:"div",cls:"x-dlg-ft"},true);}if(!this.btnContainer){var tb=this.footer.createChild({cls:"x-dlg-btns x-dlg-btns-"+this.buttonAlign,html:'<table cellspacing="0"><tbody><tr></tr></tbody></table><div class="x-clear"></div>'},null,true);this.btnContainer=tb.firstChild.firstChild.firstChild;}var P={handler:N,scope:O,minWidth:this.minButtonWidth,hideParent:true};if(typeof M=="string"){P.text=M;}else {if(M.tag){P.dhconfig=M;}else {Roo.apply(P,M);}}var fc=false;if((typeof(P.position)!='undefined')&&P.position<this.btnContainer.childNodes.length-1){P.position=Math.max(0,P.position);fc=this.btnContainer.childNodes[P.position];}var Q=new Roo.Button(fc?this.btnContainer.insertBefore(document.createElement("td"),fc):this.btnContainer.appendChild(document.createElement("td")),P);this.syncBodyHeight();if(!this.buttons){this.buttons=[];}
this.buttons.push(Q);return Q;},setDefaultButton:function(R){this.defaultButton=R;return this;},getHeaderFooterHeight:function(S){var T=0;if(this.header){T+=this.header.getHeight();}if(this.footer){var fm=this.footer.getMargins();T+=(this.footer.getHeight()+fm.top+fm.bottom);}
T+=this.bwrap.getPadding("tb")+this.bwrap.getBorderWidth("tb");T+=this.centerBg.getPadding("tb");return T;},syncBodyHeight:function(){var bd=this.body,cb=this.centerBg,bw=this.bwrap;var U=this.size.height-this.getHeaderFooterHeight(false);bd.setHeight(U-bd.getMargins("tb"));var hh=this.header.getHeight();var h=this.size.height-hh;cb.setHeight(h);bw.setLeftTop(cb.getPadding("l"),hh+cb.getPadding("t"));bw.setHeight(h-cb.getPadding("tb"));bw.setWidth(this.el.getWidth(true)-cb.getPadding("lr"));bd.setWidth(bw.getWidth(true));if(this.tabs){this.tabs.syncHeight();if(Roo.isIE){this.tabs.el.repaint();}}},restoreState:function(){var V=Roo.state.Manager.get(this.stateId||(this.el.id+"-state"));if(V&&V.width){this.xy=[V.x,V.y];this.resizeTo(V.width,V.height);}return this;},beforeShow:function(){this.expand();if(this.fixedcenter){this.xy=this.el.getCenterXY(true);}if(this.modal){Roo.get(document.body).addClass("x-body-masked");this.mask.setSize(Roo.lib.Dom.getViewWidth(true),Roo.lib.Dom.getViewHeight(true));this.mask.show();}
this.constrainXY();},animShow:function(){var b=Roo.get(this.animateTarget,true).getBox();this.proxy.setSize(b.width,b.height);this.proxy.setLocation(b.x,b.y);this.proxy.show();this.proxy.setBounds(this.xy[0],this.xy[1],this.size.width,this.size.height,true,.35,this.showEl.createDelegate(this));},show:function(W){if(this.fireEvent("beforeshow",this)===false){return;}if(this.syncHeightBeforeShow){this.syncBodyHeight();}else if(this.firstShow){this.firstShow=false;this.syncBodyHeight();}
this.animateTarget=W||this.animateTarget;if(!this.el.isVisible()){this.beforeShow();if(this.animateTarget){this.animShow();}else {this.showEl();}}return this;},showEl:function(){this.proxy.hide();this.el.setXY(this.xy);this.el.show();this.adjustAssets(true);this.toFront();this.focus();if(Roo.isIE){this.el.repaint();}
this.fireEvent("show",this);},focus:function(){if(this.defaultButton){this.defaultButton.focus();}else {this.focusEl.focus();}},constrainXY:function(){if(this.constraintoviewport!==false){if(!this.viewSize){if(this.container){var s=this.container.getSize();this.viewSize=[s.width,s.height];}else {this.viewSize=[Roo.lib.Dom.getViewWidth(),Roo.lib.Dom.getViewHeight()];}}var s=Roo.get(this.container||document).getScroll();var x=this.xy[0],y=this.xy[1];var w=this.size.width,h=this.size.height;var vw=this.viewSize[0],vh=this.viewSize[1];var moved=false;if(x+w>vw+s.left){x=vw-w;moved=true;}if(y+h>vh+s.top){y=vh-h;moved=true;}if(x<s.left){x=s.left;moved=true;}if(y<s.top){y=s.top;moved=true;}if(moved){this.xy=[x,y];if(this.isVisible()){this.el.setLocation(x,y);this.adjustAssets();}}}},onDrag:function(){if(!this.proxyDrag){this.xy=this.el.getXY();this.adjustAssets();}},adjustAssets:function(X){var x=this.xy[0],y=this.xy[1];var w=this.size.width,h=this.size.height;if(X===true){if(this.shadow){this.shadow.show(this.el);}if(this.shim){this.shim.show();}}if(this.shadow&&this.shadow.isVisible()){this.shadow.show(this.el);}if(this.shim&&this.shim.isVisible()){this.shim.setBounds(x,y,w,h);}},adjustViewport:function(w,h){if(!w||!h){w=Roo.lib.Dom.getViewWidth();h=Roo.lib.Dom.getViewHeight();}
this.viewSize=[w,h];if(this.modal&&this.mask.isVisible()){this.mask.setSize(w,h);this.mask.setSize(Roo.lib.Dom.getViewWidth(true),Roo.lib.Dom.getViewHeight(true));}if(this.isVisible()){this.constrainXY();}},destroy:function(Y){if(this.isVisible()){this.animateTarget=null;this.hide();}
Roo.EventManager.removeResizeListener(this.adjustViewport,this);if(this.tabs){this.tabs.destroy(Y);}
Roo.destroy(this.shim,this.proxy,this.resizer,this.close,this.mask);if(this.dd){this.dd.unreg();}if(this.buttons){for(var i=0,len=this.buttons.length;i<len;i++){this.buttons[i].destroy();}}
this.el.removeAllListeners();if(Y===true){this.el.update("");this.el.remove();}
...
this.refreshSize();this.adjustAssets();this.focus();this.fireEvent("move",this,this.xy[0],this.xy[1]);},toFront:function(){Roo.DialogManager.bringToFront(this);return this;},toBack:function(){Roo.DialogManager.sendToBack(this);return this;},center:function(){var xy=this.el.getCenterXY(true);this.moveTo(xy[0],xy[1]);return this;},moveTo:function(x,y){this.xy=[x,y];if(this.isVisible()){this.el.setXY(this.xy);this.adjustAssets();}return this;},alignTo:function(Z,a,c){this.xy=this.el.getAlignToXY(Z,a,c);if(this.isVisible()){this.el.setXY(this.xy);this.adjustAssets();}return this;},anchorTo:function(el,d,f,g){var j=function(){this.alignTo(el,d,f);};Roo.EventManager.onWindowResize(j,this);var tm=typeof g;if(tm!='undefined'){Roo.EventManager.on(window,'scroll',j,this,{buffer:tm=='number'?g:50});}
j.call(this);return this;},isVisible:function(){return this.el.isVisible();},animHide:function(l){var b=Roo.get(this.animateTarget).getBox();this.proxy.show();this.proxy.setBounds(this.xy[0],this.xy[1],this.size.width,this.size.height);this.el.hide();this.proxy.setBounds(b.x,b.y,b.width,b.height,true,.35,this.hideEl.createDelegate(this,[l]));},hide:function(m){if(this.fireEvent("beforehide",this)===false){return;}if(this.shadow){this.shadow.hide();}if(this.shim){this.shim.hide();}if(this.animateTarget){this.animHide(m);}else {this.el.hide();this.hideEl(m);}return this;},hideEl:function(n){this.proxy.hide();if(this.modal){this.mask.hide();Roo.get(document.body).removeClass("x-body-masked");}
this.fireEvent("hide",this);if(typeof n=="function"){n();}},hideAction:function(){this.setLeft("-10000px");this.setTop("-10000px");this.setStyle("visibility","hidden");},refreshSize:function(){this.size=this.el.getSize();this.xy=this.el.getXY();Roo.state.Manager.set(this.stateId||this.el.id+"-state",this.el.getBox());},setZIndex:function(o){if(this.modal){this.mask.setStyle("z-index",o);}if(this.shim){this.shim.setStyle("z-index",++o);}if(this.shadow){this.shadow.setZIndex(++o);}
this.el.setStyle("z-index",++o);if(this.proxy){this.proxy.setStyle("z-index",++o);}if(this.resizer){this.resizer.proxy.setStyle("z-index",++o);}
this.lastZIndex=o;},getEl:function(){return this.el;}});Roo.DialogManager=function(){var p={};var q=[];var r=null;var t=function(d1,d2){return (!d1._lastAccess||d1._lastAccess<d2._lastAccess)?-1:1;};var u=function(){q.sort(t);var v=Roo.DialogManager.zseed;for(var i=0,len=q.length;i<len;i++){var dlg=q[i];if(dlg){dlg.setZIndex(v+(i*10));}}};return {zseed:9000,register:function(AD){p[AD.id]=AD;q.push(AD);},unregister:function(AE){delete p[AE.id];var i=0;var AF=0;if(!q.indexOf){for(i=0,AF=q.length;i<AF;i++){if(q[i]==AE){q.splice(i,1);return;}}}else {i=q.indexOf(AE);if(i!=-1){q.splice(i,1);}}},get:function(id){return typeof id=="object"?id:p[id];},bringToFront:function(AG){AG=this.get(AG);if(AG!=r){r=AG;AG._lastAccess=new Date().getTime();u();}return AG;},sendToBack:function(AH){AH=this.get(AH);AH._lastAccess=-(new Date().getTime());u();return AH;},hideAll:function(){for(var id in p){if(p[id]&&typeof p[id]!="function"&&p[id].isVisible()){p[id].hide();}}}};}();Roo.LayoutDialog=function(el,v){var z=v;if(typeof(v)=='undefined'){z=Roo.apply({},el);el=Roo.get(document.documentElement||document.body).createChild();}
z.autoTabs=false;Roo.LayoutDialog.superclass.constructor.call(this,el,z);this.body.setStyle({overflow:"hidden",position:"relative"});this.layout=new Roo.BorderLayout(this.body.dom,z);this.layout.monitorWindowResize=false;this.el.addClass("x-dlg-auto-layout");this.center=Roo.BasicDialog.prototype.center;this.on("show",this.layout.layout,this.layout,true);if(z.items){var xitems=z.items;delete z.items;Roo.each(xitems,this.addxtype,this);}};Roo.extend(Roo.LayoutDialog,Roo.BasicDialog,{endUpdate:function(){this.layout.endUpdate();},beginUpdate:function(){this.layout.beginUpdate();},getLayout:function(){return this.layout;},showEl:function(){Roo.LayoutDialog.superclass.showEl.apply(this,arguments);if(Roo.isIE7){this.layout.layout();}},syncBodyHeight:function(){Roo.LayoutDialog.superclass.syncBodyHeight.call(this);if(this.layout){this.layout.layout();}},addxtype:function(c){return this.layout.addxtype(c);}});
...
Roo.QuickTips=function(){var el,A,B,C,tm,D,E,F={},esc,removeCls=null,bdLeft,bdRight;var ce,bd,xy,dd;var G=false,H=true,I=false;var J=1,K=1,L=1,M=[];var N=function(e){if(H){return;}var t=e.getTarget();if(!t||t.nodeType!==1||t==document||t==document.body){return;}if(ce&&t==ce.el){clearTimeout(K);return;}if(t&&F[t.id]){F[t.id].el=t;J=S.defer(tm.showDelay,tm,[F[t.id]]);return;}var W,et=Roo.fly(t);var ns=D.namespace;if(tm.interceptTitles&&t.title){W=t.title;t.qtip=W;t.removeAttribute("title");e.preventDefault();}else {W=t.qtip||et.getAttributeNS(ns,D.attribute);}if(W){J=S.defer(tm.showDelay,tm,[{el:t,text:W,width:et.getAttributeNS(ns,D.width),autoHide:et.getAttributeNS(ns,D.hide)!="user",title:et.getAttributeNS(ns,D.title),cls:et.getAttributeNS(ns,D.cls)}]);}};var O=function(e){clearTimeout(J);var t=e.getTarget();if(t&&ce&&ce.el==t&&(tm.autoHide&&ce.autoHide!==false)){K=setTimeout(U,tm.hideDelay);}};var P=function(e){if(H){return;}
xy=e.getXY();xy[1]+=18;if(tm.trackMouse&&ce){el.setXY(xy);}};var Q=function(e){clearTimeout(J);clearTimeout(K);if(!e.within(el)){if(tm.hideOnClick){U();tm.disable();tm.enable.defer(100,tm);}}};var R=function(){return 2;};var S=function(o){if(H){return;}
clearTimeout(L);ce=o;if(removeCls){el.removeClass(removeCls);removeCls=null;}if(ce.cls){el.addClass(ce.cls);removeCls=ce.cls;}if(ce.title){C.update(ce.title);C.show();}else {C.update('');C.hide();}
el.dom.style.width=tm.maxWidth+'px';B.update(o.text);var p=R(),w=ce.width;if(!w){var td=B.dom;var aw=Math.max(td.offsetWidth,td.clientWidth,td.scrollWidth);if(aw>tm.maxWidth){w=tm.maxWidth;}else if(aw<tm.minWidth){w=tm.minWidth;}else {w=aw;}}
el.setWidth(parseInt(w,10)+p);if(ce.autoHide===false){E.setDisplayed(true);if(dd){dd.unlock();}}else {E.setDisplayed(false);if(dd){dd.lock();}}if(xy){el.avoidY=xy[1]-18;el.setXY(xy);}if(tm.animate){el.setOpacity(.1);el.setStyle("visibility","visible");el.fadeIn({callback:T});}else {T();}};var T=function(){if(ce){el.show();esc.enable();if(tm.autoDismiss&&ce.autoHide!==false){L=setTimeout(U,tm.autoDismissDelay);}}};var U=function(W){clearTimeout(L);clearTimeout(K);ce=null;if(el.isVisible()){esc.disable();if(W!==true&&tm.animate){el.fadeOut({callback:V});}else {V();}}};var V=function(){el.hide();if(removeCls){el.removeClass(removeCls);removeCls=null;}};return {minWidth:40,maxWidth:300,interceptTitles:false,trackMouse:false,hideOnClick:true,showDelay:500,hideDelay:200,autoHide:true,autoDismiss:true,autoDismissDelay:5000,animate:false,title:'',text:'',cls:'',width:null,init:function(){tm=Roo.QuickTips;D=tm.tagConfig;if(!I){if(!Roo.isReady){Roo.onReady(Roo.QuickTips.init,Roo.QuickTips);return;}
el=new Roo.Layer({cls:"x-tip",shadow:"drop",shim:true,constrain:true,shadowOffset:4});el.fxDefaults={stopFx:true};el.update('<div class="x-tip-bd"><div class="x-tip-close"></div><h3></h3><div class="x-tip-bd-inner"></div><div class="x-clear"></div></div>');C=el.child('h3');C.enableDisplayMode("block");A=el.child('div.x-tip-bd');B=el.child('div.x-tip-bd-inner');E=el.child('div.x-tip-close');E.enableDisplayMode("block");E.on("click",U);var d=Roo.get(document);d.on("mousedown",Q);d.on("mouseover",N);d.on("mouseout",O);d.on("mousemove",P);esc=d.addKeyListener(27,U);esc.disable();if(Roo.dd.DD){dd=el.initDD("default",null,{onDrag:function(){el.sync();}});dd.setHandleElId(C.id);dd.lock();}
I=true;}
this.enable();},register:function(X){var cs=X instanceof Array?X:arguments;for(var i=0,len=cs.length;i<len;i++){var c=cs[i];var target=c.target;if(target){if(target instanceof Array){for(var j=0,jlen=target.length;j<jlen;j++){F[target[j]]=c;}}else {F[typeof target=='string'?target:Roo.id(target)]=c;}}}},unregister:function(el){delete F[Roo.id(el)];},enable:function(){if(I&&H){M.pop();if(M.length<1){H=false;}}},disable:function(){H=true;clearTimeout(J);clearTimeout(K);clearTimeout(L);if(ce){U(true);}
M.push(1);},isEnabled:function(){return !H;},tagConfig:{namespace:"ext",attribute:"qtip",width:"width",target:"target",title:"qtitle",hide:"hide",cls:"qclass"}};}();Roo.QuickTips.tips=Roo.QuickTips.register;
...
this.id=this.el.id;this.addEvents({"beforeload":true,"load":true,"textchange":true,"beforeexpand":true,"beforecollapse":true,"expand":true,"disabledchange":true,"collapse":true,"beforeclick":true,"checkchange":true,"click":true,"dblclick":true,"contextmenu":true,"beforechildrenrendered":true,"startdrag":true,"enddrag":true,"dragdrop":true,"beforenodedrop":true,"nodedrop":true,"nodedragover":true});if(this.singleExpand){this.on("beforeexpand",this.restrictExpand,this);}};Roo.extend(Roo.tree.TreePanel,Roo.data.Tree,{rootVisible:true,animate:Roo.enableFx,lines:true,enableDD:false,hlDrop:Roo.enableFx,renderer:false,rendererTip:false,restrictExpand:function(D){var p=D.parentNode;if(p){if(p.expandedChild&&p.expandedChild.parentNode==p){p.expandedChild.collapse();}
p.expandedChild=D;}},setRootNode:function(E){Roo.tree.TreePanel.superclass.setRootNode.call(this,E);if(!this.rootVisible){E.ui=new Roo.tree.RootTreeNodeUI(E);}return E;},getEl:function(){return this.el;},getLoader:function(){return this.loader;},expandAll:function(){this.root.expand(true);},collapseAll:function(){this.root.collapse(true);},getSelectionModel:function(){if(!this.selModel){this.selModel=new Roo.tree.DefaultSelectionModel();}return this.selModel;},getChecked:function(a,F){F=F||this.root;var r=[];var f=function(){if(this.attributes.checked){r.push(!a?this:(a=='id'?this.id:this.attributes[a]));}}
F.cascade(f);return r;},expandPath:function(G,H,I){H=H||"id";var J=G.split(this.pathSeparator);var K=this.root;if(K.attributes[H]!=J[1]){if(I){I(false,null);}return;}var L=1;var f=function(){if(++L==J.length){if(I){I(true,K);}return;}var c=K.findChild(H,J[L]);if(!c){if(I){I(false,K);}return;}
K=c;c.expand(false,false,f);};K.expand(false,false,f);},selectPath:function(M,N,O){N=N||"id";var P=M.split(this.pathSeparator);var v=P.pop();if(P.length>0){var f=function(Q,R){if(Q&&R){var n=R.findChild(N,v);if(n){n.select();if(O){O(true,n);}}else if(O){O(false,n);}}else {if(O){O(false,n);}}};this.expandPath(P.join(this.pathSeparator),N,f);}else {this.root.select();if(O){O(true,this.root);}}},getTreeEl:function(){return this.el;},render:function(){if(this.innerCt){return this;}
...
this.select(B);},select:function(C){var D=this.selNode;if(D!=C&&this.fireEvent('beforeselect',this,C,D)!==false){if(D){D.ui.onSelectedChange(false);}
this.selNode=C;C.ui.onSelectedChange(true);this.fireEvent("selectionchange",this,C,D);}return C;},unselect:function(E){if(this.selNode==E){this.clearSelections();}},clearSelections:function(){var n=this.selNode;if(n){n.ui.onSelectedChange(false);this.selNode=null;this.fireEvent("selectionchange",this,null);}return n;},getSelectedNode:function(){return this.selNode;},isSelected:function(F){return this.selNode==F;},selectPrevious:function(){var s=this.selNode||this.lastSelNode;if(!s){return null;}var ps=s.previousSibling;if(ps){if(!ps.isExpanded()||ps.childNodes.length<1){return this.select(ps);}else {var lc=ps.lastChild;while(lc&&lc.isExpanded()&&lc.childNodes.length>0){lc=lc.lastChild;}return this.select(lc);}}else if(s.parentNode&&(this.tree.rootVisible||!s.parentNode.isRoot)){return this.select(s.parentNode);}return null;},selectNext:function(){var s=this.selNode||this.lastSelNode;if(!s){return null;}if(s.firstChild&&s.isExpanded()){return this.select(s.firstChild);}else if(s.nextSibling){return this.select(s.nextSibling);}else if(s.parentNode){var newS=null;s.parentNode.bubble(function(){if(this.nextSibling){newS=this.getOwnerTree().selModel.select(this.nextSibling);return false;}});return newS;}return null;},onKeyDown:function(e){var s=this.selNode||this.lastSelNode;var sm=this;if(!s){return;}var k=e.getKey();switch(k){case e.DOWN:e.stopEvent();this.selectNext();break;case e.UP:e.stopEvent();this.selectPrevious();break;case e.RIGHT:e.preventDefault();if(s.hasChildNodes()){if(!s.isExpanded()){s.expand();}else if(s.firstChild){this.select(s.firstChild,e);}}break;case e.LEFT:e.preventDefault();if(s.hasChildNodes()&&s.isExpanded()){s.collapse();}else if(s.parentNode&&(this.tree.rootVisible||s.parentNode!=this.tree.getRootNode())){this.select(s.parentNode,e);}break;};}});Roo.tree.MultiSelectionModel=function(){this.selNodes=[];this.selMap={};this.addEvents({"selectionchange":true});};Roo.extend(Roo.tree.MultiSelectionModel,Roo.util.Observable,{init:function(G){this.tree=G;G.getTreeEl().on("keydown",this.onKeyDown,this);G.on("click",this.onNodeClick,this);},onNodeClick:function(H,e){this.select(H,e,e.ctrlKey);},select:function(I,e,J){if(J!==true){this.clearSelections(true);}if(this.isSelected(I)){this.lastSelNode=I;return I;}
this.selNodes.push(I);this.selMap[I.id]=I;this.lastSelNode=I;I.ui.onSelectedChange(true);this.fireEvent("selectionchange",this,this.selNodes);return I;},unselect:function(K){if(this.selMap[K.id]){K.ui.onSelectedChange(false);var sn=this.selNodes;var index=-1;if(sn.indexOf){index=sn.indexOf(K);}else {for(var i=0,len=sn.length;i<len;i++){if(sn[i]==K){index=i;break;}}}if(index!=-1){this.selNodes.splice(index,1);}delete this.selMap[K.id];this.fireEvent("selectionchange",this,this.selNodes);}},clearSelections:function(L){var sn=this.selNodes;if(sn.length>0){for(var i=0,len=sn.length;i<len;i++){sn[i].ui.onSelectedChange(false);}
this.selNodes=[];this.selMap={};if(L!==true){this.fireEvent("selectionchange",this,this.selNodes);}}},isSelected:function(M){return this.selMap[M.id]?true:false;},getSelectedNodes:function(){return this.selNodes;},onKeyDown:Roo.tree.DefaultSelectionModel.prototype.onKeyDown,selectNext:Roo.tree.DefaultSelectionModel.prototype.selectNext,selectPrevious:Roo.tree.DefaultSelectionModel.prototype.selectPrevious});
Roo.tree.TreeNode=function(A){A=A||{};if(typeof A=="string"){A={text:A};}
this.childrenRendered=false;this.rendered=false;Roo.tree.TreeNode.superclass.constructor.call(this,A);this.expanded=A.expanded===true;this.isTarget=A.isTarget!==false;this.draggable=A.draggable!==false&&A.allowDrag!==false;this.allowChildren=A.allowChildren!==false&&A.allowDrop!==false;this.text=A.text;this.disabled=A.disabled===true;this.addEvents({"textchange":true,"beforeexpand":true,"beforecollapse":true,"expand":true,"disabledchange":true,"collapse":true,"beforeclick":true,"checkchange":true,"click":true,"dblclick":true,"contextmenu":true,"beforechildrenrendered":true});var B=this.attributes.uiProvider||Roo.tree.TreeNodeUI;this.ui=new B(this);};Roo.extend(Roo.tree.TreeNode,Roo.data.Node,{preventHScroll:true,isExpanded:function(){return this.expanded;},getUI:function(){return this.ui;},setFirstChild:function(C){var of=this.firstChild;Roo.tree.TreeNode.superclass.setFirstChild.call(this,C);if(this.childrenRendered&&of&&C!=of){of.renderIndent(true,true);}if(this.rendered){this.renderIndent(true,true);}},setLastChild:function(D){var ol=this.lastChild;Roo.tree.TreeNode.superclass.setLastChild.call(this,D);if(this.childrenRendered&&ol&&D!=ol){ol.renderIndent(true,true);}if(this.rendered){this.renderIndent(true,true);}},appendChild:function(){var E=Roo.tree.TreeNode.superclass.appendChild.apply(this,arguments);if(E&&this.childrenRendered){E.render();}
this.ui.updateExpandIcon();return E;},removeChild:function(F){this.ownerTree.getSelectionModel().unselect(F);Roo.tree.TreeNode.superclass.removeChild.apply(this,arguments);if(this.childrenRendered){F.ui.remove();}if(this.childNodes.length<1){this.collapse(false,false);}else {this.ui.updateExpandIcon();}if(!this.firstChild){this.childrenRendered=false;}return F;},insertBefore:function(G,H){var I=Roo.tree.TreeNode.superclass.insertBefore.apply(this,arguments);if(I&&H&&this.childrenRendered){G.render();}
this.ui.updateExpandIcon();return I;},setText:function(J){var K=this.text;this.text=J;this.attributes.text=J;if(this.rendered){this.ui.onTextChange(this,J,K);}
this.fireEvent("textchange",this,J,K);},select:function(){this.getOwnerTree().getSelectionModel().select(this);},unselect:function(){this.getOwnerTree().getSelectionModel().unselect(this);},isSelected:function(){return this.getOwnerTree().getSelectionModel().isSelected(this);},expand:function(L,M,N){if(!this.expanded){if(this.fireEvent("beforeexpand",this,L,M)===false){return;}if(!this.childrenRendered){this.renderChildren();}
this.expanded=true;if(!this.isHiddenRoot()&&(this.getOwnerTree().animate&&M!==false)||M){this.ui.animExpand(function(){this.fireEvent("expand",this);if(typeof N=="function"){N(this);}if(L===true){this.expandChildNodes(true);}}.createDelegate(this));return;}else {this.ui.expand();this.fireEvent("expand",this);if(typeof N=="function"){N(this);}}}else {if(typeof N=="function"){N(this);}}if(L===true){this.expandChildNodes(true);}},isHiddenRoot:function(){return this.isRoot&&!this.getOwnerTree().rootVisible;},collapse:function(O,P){if(this.expanded&&!this.isHiddenRoot()){if(this.fireEvent("beforecollapse",this,O,P)===false){return;}
this.expanded=false;if((this.getOwnerTree().animate&&P!==false)||P){this.ui.animCollapse(function(){this.fireEvent("collapse",this);if(O===true){this.collapseChildNodes(true);}}.createDelegate(this));return;}else {this.ui.collapse();this.fireEvent("collapse",this);}}if(O===true){var cs=this.childNodes;for(var i=0,len=cs.length;i<len;i++){cs[i].collapse(true,false);}}},delayedExpand:function(Q){if(!this.expandProcId){this.expandProcId=this.expand.defer(Q,this);}},cancelExpand:function(){if(this.expandProcId){clearTimeout(this.expandProcId);}
this.expandProcId=false;},toggle:function(){if(this.expanded){this.collapse();}else {this.expand();}},ensureVisible:function(R){var S=this.getOwnerTree();S.expandPath(this.parentNode.getPath(),false,function(){S.getTreeEl().scrollChildIntoView(this.ui.anchor);Roo.callback(R);}.createDelegate(this));},expandChildNodes:function(T){var cs=this.childNodes;for(var i=0,len=cs.length;i<len;i++){cs[i].expand(T);}},collapseChildNodes:function(U){var cs=this.childNodes;for(var i=0,len=cs.length;i<len;i++){cs[i].collapse(U);}},disable:function(){this.disabled=true;this.unselect();if(this.rendered&&this.ui.onDisableChange){this.ui.onDisableChange(this,true);}
...
this.fireEvent("disabledchange",this,false);},renderChildren:function(V){if(V!==false){this.fireEvent("beforechildrenrendered",this);}var cs=this.childNodes;for(var i=0,len=cs.length;i<len;i++){cs[i].render(true);}
this.childrenRendered=true;},sort:function(fn,W){Roo.tree.TreeNode.superclass.sort.apply(this,arguments);if(this.childrenRendered){var cs=this.childNodes;for(var i=0,len=cs.length;i<len;i++){cs[i].render(true);}}},render:function(X){this.ui.render(X);if(!this.rendered){this.rendered=true;if(this.expanded){this.expanded=false;this.expand(false,false);}}},renderIndent:function(Y,Z){if(Z){this.ui.childIndent=null;}
this.ui.renderIndent();if(Y===true&&this.childrenRendered){var cs=this.childNodes;for(var i=0,len=cs.length;i<len;i++){cs[i].renderIndent(true,Z);}}}});
Roo.tree.AsyncTreeNode=function(A){this.loaded=false;this.loading=false;Roo.tree.AsyncTreeNode.superclass.constructor.apply(this,arguments);this.addEvents({'beforeload':true,'load':true});};Roo.extend(Roo.tree.AsyncTreeNode,Roo.tree.TreeNode,{expand:function(B,C,D){if(this.loading){var timer;var f=function(){if(!this.loading){clearInterval(timer);this.expand(B,C,D);}}.createDelegate(this);timer=setInterval(f,200);return;}if(!this.loaded){if(this.fireEvent("beforeload",this)===false){return;}
this.loading=true;this.ui.beforeLoad(this);var loader=this.loader||this.attributes.loader||this.getOwnerTree().getLoader();if(loader){loader.load(this,this.loadComplete.createDelegate(this,[B,C,D]));return;}}
Roo.tree.AsyncTreeNode.superclass.expand.call(this,B,C,D);},isLoading:function(){return this.loading;},loadComplete:function(E,F,G){this.loading=false;this.loaded=true;this.ui.afterLoad(this);this.fireEvent("load",this);this.expand(E,F,G);},isLoaded:function(){return this.loaded;},hasChildNodes:function(){if(!this.isLeaf()&&!this.loaded){return true;}else {return Roo.tree.AsyncTreeNode.superclass.hasChildNodes.call(this);}},reload:function(H){this.collapse(false,false);while(this.firstChild){this.removeChild(this.firstChild);}
this.childrenRendered=false;this.loaded=false;if(this.isHiddenRoot()){this.expanded=false;}
this.expand(false,false,H);}});
Roo.tree.TreeNodeUI=function(A){this.node=A;this.rendered=false;this.animating=false;this.emptyIcon=Roo.BLANK_IMAGE_URL;};Roo.tree.TreeNodeUI.prototype={removeChild:function(B){if(this.rendered){this.ctNode.removeChild(B.ui.getEl());}},beforeLoad:function(){this.addClass("x-tree-node-loading");},afterLoad:function(){this.removeClass("x-tree-node-loading");},onTextChange:function(C,D,E){if(this.rendered){this.textNode.innerHTML=D;}},onDisableChange:function(F,G){this.disabled=G;if(G){this.addClass("x-tree-node-disabled");}else {this.removeClass("x-tree-node-disabled");}},onSelectedChange:function(H){if(H){this.focus();this.addClass("x-tree-selected");}else {this.removeClass("x-tree-selected");}},onMove:function(I,J,K,L,M,N){this.childIndent=null;if(this.rendered){var targetNode=L.ui.getContainer();if(!targetNode){this.holder=document.createElement("div");this.holder.appendChild(this.wrap);return;}var insertBefore=N?N.ui.getEl():null;if(insertBefore){targetNode.insertBefore(this.wrap,insertBefore);}else {targetNode.appendChild(this.wrap);}
this.node.renderIndent(true);}},addClass:function(O){if(this.elNode){Roo.fly(this.elNode).addClass(O);}},removeClass:function(P){if(this.elNode){Roo.fly(this.elNode).removeClass(P);}},remove:function(){if(this.rendered){this.holder=document.createElement("div");this.holder.appendChild(this.wrap);}},fireEvent:function(){return this.node.fireEvent.apply(this.node,arguments);},initEvents:function(){this.node.on("move",this.onMove,this);var E=Roo.EventManager;var a=this.anchor;var el=Roo.fly(a,'_treeui');if(Roo.isOpera){el.setStyle("text-decoration","none");}
...
this.fireEvent("dblclick",this.node,e);},onCheckChange:function(){var R=this.checkbox.checked;this.node.attributes.checked=R;this.fireEvent('checkchange',this.node,R);},ecClick:function(e){if(!this.animating&&this.node.hasChildNodes()){this.node.toggle();}},startDrop:function(){this.dropping=true;},endDrop:function(){setTimeout(function(){this.dropping=false;}.createDelegate(this),50);},expand:function(){this.updateExpandIcon();this.ctNode.style.display="";},focus:function(){if(!this.node.preventHScroll){try{this.anchor.focus();}catch(e){}}else if(!Roo.isIE){try{var noscroll=this.node.getOwnerTree().getTreeEl().dom;var l=noscroll.scrollLeft;this.anchor.focus();noscroll.scrollLeft=l;}catch(e){}}},toggleCheck:function(S){var cb=this.checkbox;if(cb){cb.checked=(S===undefined?!cb.checked:S);}},blur:function(){try{this.anchor.blur();}catch(e){}},animExpand:function(T){var ct=Roo.get(this.ctNode);ct.stopFx();if(!this.node.hasChildNodes()){this.updateExpandIcon();this.ctNode.style.display="";Roo.callback(T);return;}
this.animating=true;this.updateExpandIcon();ct.slideIn('t',{callback:function(){this.animating=false;Roo.callback(T);},scope:this,duration:this.node.ownerTree.duration||.25});},highlight:function(){var U=this.node.getOwnerTree();Roo.fly(this.wrap).highlight(U.hlColor||"C3DAF9",{endColor:U.hlBaseColor});},collapse:function(){this.updateExpandIcon();this.ctNode.style.display="none";},animCollapse:function(V){var ct=Roo.get(this.ctNode);ct.enableDisplayMode('block');ct.stopFx();this.animating=true;this.updateExpandIcon();ct.slideOut('t',{callback:function(){this.animating=false;Roo.callback(V);},scope:this,duration:this.node.ownerTree.duration||.25});},getContainer:function(){return this.ctNode;},getEl:function(){return this.wrap;},appendDDGhost:function(W){W.appendChild(this.elNode.cloneNode(true));},getDDRepairXY:function(){return Roo.lib.Dom.getXY(this.iconNode);},onRender:function(){this.render();},render:function(X){var n=this.node,a=n.attributes;var Y=n.parentNode?n.parentNode.ui.getContainer():n.ownerTree.innerCt.dom;if(!this.rendered){this.rendered=true;this.renderElements(n,a,Y,X);if(a.qtip){if(this.textNode.setAttributeNS){this.textNode.setAttributeNS("ext","qtip",a.qtip);if(a.qtipTitle){this.textNode.setAttributeNS("ext","qtitle",a.qtipTitle);}}else {this.textNode.setAttribute("ext:qtip",a.qtip);if(a.qtipTitle){this.textNode.setAttribute("ext:qtitle",a.qtipTitle);}}}else if(a.qtipCfg){a.qtipCfg.target=Roo.id(this.textNode);Roo.QuickTips.register(a.qtipCfg);}
this.initEvents();if(!this.node.expanded){this.updateExpandIcon();}}else {if(X===true){Y.appendChild(this.wrap);}}},renderElements:function(n,a,Z,b){this.indentMarkup=n.parentNode?n.parentNode.ui.getChildIndent():'';var t=n.getOwnerTree();var c=t.renderer?t.renderer(n.attributes):Roo.util.Format.htmlEncode(n.text);var d=t.rendererTip?t.rendererTip(n.attributes):c;var cb=typeof a.checked=='boolean';var f=a.href?a.href:Roo.isGecko?"":"#";var g=['<li class="x-tree-node"><div class="x-tree-node-el ',a.cls,'">','<span class="x-tree-node-indent">',this.indentMarkup,"</span>",'<img src="',this.emptyIcon,'" class="x-tree-ec-icon" />','<img src="',a.icon||this.emptyIcon,'" class="x-tree-node-icon',(a.icon?" x-tree-node-inline-icon":""),(a.iconCls?" "+a.iconCls:""),'" unselectable="on" />',cb?('<input class="x-tree-node-cb" type="checkbox" '+(a.checked?'checked="checked" />':' />')):'','<a hidefocus="on" href="',f,'" tabIndex="1" ',a.hrefTarget?' target="'+a.hrefTarget+'"':"",'><span unselectable="on" qtip="',d,'">',c,"</span></a></div>",'<ul class="x-tree-node-ct" style="display:none;"></ul>',"</li>"];if(b!==true&&n.nextSibling&&n.nextSibling.ui.getEl()){this.wrap=Roo.DomHelper.insertHtml("beforeBegin",n.nextSibling.ui.getEl(),g.join(""));}else {this.wrap=Roo.DomHelper.insertHtml("beforeEnd",Z,g.join(""));}
this.elNode=this.wrap.childNodes[0];this.ctNode=this.wrap.childNodes[1];var cs=this.elNode.childNodes;this.indentNode=cs[0];this.ecNode=cs[1];this.iconNode=cs[2];var h=3;if(cb){this.checkbox=cs[3];h++;}
this.anchor=cs[h];this.textNode=cs[h].firstChild;},getAnchor:function(){return this.anchor;},getTextEl:function(){return this.textNode;},getIconEl:function(){return this.iconNode;},isChecked:function(){return this.checkbox?this.checkbox.checked:false;},updateExpandIcon:function(){if(this.rendered){var n=this.node,c1,c2;var P=n.isLast()?"x-tree-elbow-end":"x-tree-elbow";var hasChild=n.hasChildNodes();if(hasChild){if(n.expanded){P+="-minus";c1="x-tree-node-collapsed";c2="x-tree-node-expanded";}else {P+="-plus";c1="x-tree-node-expanded";c2="x-tree-node-collapsed";}if(this.wasLeaf){this.removeClass("x-tree-node-leaf");this.wasLeaf=false;}if(this.c1!=c1||this.c2!=c2){Roo.fly(this.elNode).replaceClass(c1,c2);this.c1=c1;this.c2=c2;}}else {if(!this.wasLeaf){Roo.fly(this.elNode).replaceClass("x-tree-node-expanded","x-tree-node-leaf");delete this.c1;delete this.c2;this.wasLeaf=true;}}var ecc="x-tree-ec-icon "+P;if(this.ecc!=ecc){this.ecNode.className=ecc;this.ecc=ecc;}}},getChildIndent:function(){if(!this.childIndent){var g=[];var p=this.node;while(p){if(!p.isRoot||(p.isRoot&&p.ownerTree.rootVisible)){if(!p.isLast()){g.unshift('<img src="'+this.emptyIcon+'" class="x-tree-elbow-line" />');}else {g.unshift('<img src="'+this.emptyIcon+'" class="x-tree-icon" />');}}
p=p.parentNode;}
...
this.focusEl=el.createChild({tag:"a",cls:"x-menu-focus",href:"#",onclick:"return false;",tabIndex:"-1"});var ul=el.createChild({tag:"ul",cls:"x-menu-list"});ul.on("click",this.onClick,this);ul.on("mouseover",this.onMouseOver,this);ul.on("mouseout",this.onMouseOut,this);this.items.each(function(C){var li=document.createElement("li");li.className="x-menu-list-item";ul.dom.appendChild(li);C.render(li,this);},this);this.ul=ul;this.autoWidth();},autoWidth:function(){var el=this.el,ul=this.ul;if(!el){return;}var w=this.width;if(w){el.setWidth(w);}else if(Roo.isIE){el.setWidth(this.minWidth);var t=el.dom.offsetWidth;el.setWidth(ul.getWidth()+el.getFrameWidth("lr"));}},delayAutoWidth:function(){if(this.rendered){if(!this.awTask){this.awTask=new Roo.util.DelayedTask(this.autoWidth,this);}
this.awTask.delay(20);}},findTargetItem:function(e){var t=e.getTarget(".x-menu-list-item",this.ul,true);if(t&&t.menuItemId){return this.items.get(t.menuItemId);}},onClick:function(e){var t;if(t=this.findTargetItem(e)){t.onClick(e);this.fireEvent("click",this,t,e);}},setActiveItem:function(C,D){if(C!=this.activeItem){if(this.activeItem){this.activeItem.deactivate();}
this.activeItem=C;C.activate(D);}else if(D){C.expandMenu();}},tryActivate:function(E,F){var G=this.items;for(var i=E,len=G.length;i>=0&&i<len;i+=F){var C=G.get(i);if(!C.disabled&&C.canActivate){this.setActiveItem(C,false);return C;}}return false;},onMouseOver:function(e){var t;if(t=this.findTargetItem(e)){if(t.canActivate&&!t.disabled){this.setActiveItem(t,true);}}
...
C[g].push(T);T.on("beforecheckchange",K);}},unregisterCheckable:function(U){var g=U.group;if(g){C[g].remove(U);U.un("beforecheckchange",K);}}};}();
Roo.menu.BaseItem=function(A){Roo.menu.BaseItem.superclass.constructor.call(this,A);this.addEvents({click:true,activate:true,deactivate:true});if(this.handler){this.on("click",this.handler,this.scope,true);}};Roo.extend(Roo.menu.BaseItem,Roo.Component,{canActivate:false,activeClass:"x-menu-item-active",hideOnClick:true,hideDelay:100,ctype:"Roo.menu.BaseItem",actionMode:"container",render:function(B,C){this.parentMenu=C;Roo.menu.BaseItem.superclass.render.call(this,B);this.container.menuItemId=this.id;},onRender:function(D,E){this.el=Roo.get(this.el);D.dom.appendChild(this.el.dom);},onClick:function(e){if(!this.disabled&&this.fireEvent("click",this,e)!==false&&this.parentMenu.fireEvent("itemclick",this,e)!==false){this.handleClick(e);}else {e.stopEvent();}},activate:function(){if(this.disabled){return false;}var li=this.container;li.addClass(this.activeClass);this.region=li.getRegion().adjust(2,2,-2,-2);this.fireEvent("activate",this);return true;},deactivate:function(){this.container.removeClass(this.activeClass);this.fireEvent("deactivate",this);},shouldDeactivate:function(e){return !this.region||!this.region.contains(e.getPoint());},handleClick:function(e){if(this.hideOnClick){this.parentMenu.hide.defer(this.hideDelay,this.parentMenu,[true]);}},expandMenu:function(F){},hideMenu:function(){}});
Roo.menu.Adapter=function(A,B){Roo.menu.Adapter.superclass.constructor.call(this,B);this.component=A;};Roo.extend(Roo.menu.Adapter,Roo.menu.BaseItem,{canActivate:true,onRender:function(C,D){this.component.render(C);this.el=this.component.getEl();},activate:function(){if(this.disabled){return false;}
...
Roo.menu.Item=function(A){Roo.menu.Item.superclass.constructor.call(this,A);if(this.menu){this.menu=Roo.menu.MenuMgr.get(this.menu);}};Roo.extend(Roo.menu.Item,Roo.menu.BaseItem,{itemCls:"x-menu-item",canActivate:true,showDelay:200,hideDelay:200,ctype:"Roo.menu.Item",onRender:function(B,C){var el=document.createElement("a");el.hideFocus=true;el.unselectable="on";el.href=this.href||"#";if(this.hrefTarget){el.target=this.hrefTarget;}
...
Roo.menu.Item.superclass.handleClick.apply(this,arguments);},activate:function(E){if(Roo.menu.Item.superclass.activate.apply(this,arguments)){this.focus();if(E){this.expandMenu();}}return true;},shouldDeactivate:function(e){if(Roo.menu.Item.superclass.shouldDeactivate.call(this,e)){if(this.menu&&this.menu.isVisible()){return !this.menu.getEl().getRegion().contains(e.getPoint());}return true;}return false;},deactivate:function(){Roo.menu.Item.superclass.deactivate.apply(this,arguments);this.hideMenu();},expandMenu:function(F){if(!this.disabled&&this.menu){clearTimeout(this.hideTimer);delete this.hideTimer;if(!this.menu.isVisible()&&!this.showTimer){this.showTimer=this.deferExpand.defer(this.showDelay,this,[F]);}else if(this.menu.isVisible()&&F){this.menu.tryActivate(0,1);}}},deferExpand:function(G){delete this.showTimer;this.menu.show(this.container,this.parentMenu.subMenuAlign||"tl-tr?",this.parentMenu);if(G){this.menu.tryActivate(0,1);}},hideMenu:function(){clearTimeout(this.showTimer);delete this.showTimer;if(!this.hideTimer&&this.menu&&this.menu.isVisible()){this.hideTimer=this.deferHide.defer(this.hideDelay,this);}},deferHide:function(){delete this.hideTimer;this.menu.hide();}});
...
Roo.menu.CheckItem.superclass.destroy.apply(this,arguments);},setChecked:function(B,C){if(this.checked!=B&&this.fireEvent("beforecheckchange",this,B)!==false){if(this.container){this.container[B?"addClass":"removeClass"]("x-menu-item-checked");}
this.checked=B;if(C!==true){this.fireEvent("checkchange",this,B);}}},handleClick:function(e){if(!this.disabled&&!(this.checked&&this.group)){this.setChecked(!this.checked);}
Roo.menu.CheckItem.superclass.handleClick.apply(this,arguments);}});
Roo.menu.DateItem=function(A){Roo.menu.DateItem.superclass.constructor.call(this,new Roo.DatePicker(A),A);this.picker=this.component;this.addEvents({select:true});this.picker.on("render",function(B){B.getEl().swallowEvent("click");B.container.addClass("x-menu-date-item");});this.picker.on("select",this.onSelect,this);};Roo.extend(Roo.menu.DateItem,Roo.menu.Adapter,{onSelect:function(B,C){this.fireEvent("select",this,C,B);Roo.menu.DateItem.superclass.handleClick.call(this);}});
...
this.fireEvent("blur",this);},isValid:function(E){if(this.disabled){return true;}var F=this.preventMark;this.preventMark=E===true;var v=this.validateValue(this.processValue(this.getRawValue()));this.preventMark=F;return v;},validate:function(){if(this.disabled||this.validateValue(this.processValue(this.getRawValue()))){this.clearInvalid();return true;}return false;},processValue:function(G){return G;},validateValue:function(H){return true;},markInvalid:function(I){if(!this.rendered||this.preventMark){return;}
this.el.addClass(this.invalidClass);I=I||this.invalidText;switch(this.msgTarget){case 'qtip':this.el.dom.qtip=I;this.el.dom.qclass='x-form-invalid-tip';if(Roo.QuickTips){Roo.QuickTips.enable();}break;case 'title':this.el.dom.title=I;break;case 'under':if(!this.errorEl){var elp=this.el.findParent('.x-form-element',5,true);this.errorEl=elp.createChild({cls:'x-form-invalid-msg'});this.errorEl.setWidth(elp.getWidth(true)-20);}
this.errorEl.update(I);Roo.form.Field.msgFx[this.msgFx].show(this.errorEl,this);break;case 'side':if(!this.errorIcon){var elp=this.el.findParent('.x-form-element',5,true);this.errorIcon=elp.createChild({cls:'x-form-invalid-icon'});}
this.alignErrorIcon();this.errorIcon.dom.qtip=I;this.errorIcon.dom.qclass='x-form-invalid-tip';this.errorIcon.show();this.on('resize',this.alignErrorIcon,this);break;default:var t=Roo.getDom(this.msgTarget);t.innerHTML=I;t.style.display=this.msgDisplay;break;}
this.fireEvent('invalid',this,I);},alignErrorIcon:function(){this.errorIcon.alignTo(this.el,'tl-tr',[2,0]);},clearInvalid:function(){if(!this.rendered||this.preventMark){return;}
...
Roo.form.TextArea.superclass.onRender.call(this,ct,B);if(this.grow){this.textSizeEl=Roo.DomHelper.append(document.body,{tag:"pre",cls:"x-form-grow-sizer"});if(this.preventScrollbars){this.el.setStyle("overflow","hidden");}
...
this.ddMatch=new RegExp(re+")");}};Roo.extend(Roo.form.DateField,Roo.form.TriggerField,{format:"m/d/y",altFormats:"m/d/Y|m-d-y|m-d-Y|m/d|m-d|md|mdy|mdY|d",disabledDays:null,disabledDaysText:"Disabled",disabledDates:null,disabledDatesText:"Disabled",minValue:null,maxValue:null,minText:"The date in this field must be equal to or after {0}",maxText:"The date in this field must be equal to or before {0}",invalidText:"{0} is not a valid date - it must be in the format {1}",triggerClass:'x-form-date-trigger',useIso:false,defaultAutoCreate:{tag:"input",type:"text",size:"10",autocomplete:"off"},hiddenField:false,onRender:function(ct,B){Roo.form.DateField.superclass.onRender.call(this,ct,B);if(this.useIso){this.el.dom.removeAttribute('name');this.hiddenField=this.el.insertSibling({tag:'input',type:'hidden',name:this.name},'before',true);this.hiddenField.value=this.formatDate(this.value,'Y-m-d');this.hiddenName=this.name;}},validateValue:function(C){C=this.formatDate(C);if(!Roo.form.DateField.superclass.validateValue.call(this,C)){return false;}if(C.length<1){return true;}var D=C;C=this.parseDate(C);if(!C){this.markInvalid(String.format(this.invalidText,D,this.format));return false;}var E=C.getTime();if(this.minValue&&E<this.minValue.getTime()){this.markInvalid(String.format(this.minText,this.formatDate(this.minValue)));return false;}if(this.maxValue&&E>this.maxValue.getTime()){this.markInvalid(String.format(this.maxText,this.formatDate(this.maxValue)));return false;}if(this.disabledDays){var day=C.getDay();for(var i=0;i<this.disabledDays.length;i++){if(day===this.disabledDays[i]){this.markInvalid(this.disabledDaysText);return false;}}}var F=this.formatDate(C);if(this.ddMatch&&this.ddMatch.test(F)){this.markInvalid(String.format(this.disabledDatesText,F));return false;}return true;},validateBlur:function(){return !this.menu||!this.menu.isVisible();},getValue:function(){return this.hiddenField?this.hiddenField.value:this.parseDate(Roo.form.DateField.superclass.getValue.call(this))||"";},setValue:function(G){if(this.hiddenField){this.hiddenField.value=this.formatDate(this.parseDate(G),'Y-m-d');}
Roo.form.DateField.superclass.setValue.call(this,this.formatDate(this.parseDate(G)));},parseDate:function(H){if(!H||H instanceof Date){return H;}var v=Date.parseDate(H,this.format);if(!v&&this.altFormats){if(!this.altFormatsArray){this.altFormatsArray=this.altFormats.split("|");}for(var i=0,len=this.altFormatsArray.length;i<len&&!v;i++){v=Date.parseDate(H,this.altFormatsArray[i]);}}return v;},formatDate:function(I,J){return (!I||!(I instanceof Date))?I:I.dateFormat(J||this.format);},menuListeners:{select:function(m,d){this.setValue(d);this.fireEvent('select',this,d);},show:function(){this.onFocus();},hide:function(){this.focus.defer(10,this);var ml=this.menuListeners;this.menu.un("select",ml.select,this);this.menu.un("show",ml.show,this);this.menu.un("hide",ml.hide,this);}},onTriggerClick:function(){if(this.disabled){return;}if(this.menu==null){this.menu=new Roo.menu.DateMenu();}
...
Roo.form.ComboBox=function(A){Roo.form.ComboBox.superclass.constructor.call(this,A);this.addEvents({'expand':true,'collapse':true,'beforeselect':true,'select':true,'beforequery':true});if(this.transform){this.allowDomMove=false;var s=Roo.getDom(this.transform);if(!this.hiddenName){this.hiddenName=s.name;}if(!this.store){this.mode='local';var d=[],opts=s.options;for(var i=0,len=opts.length;i<len;i++){var o=opts[i];var value=(Roo.isIE?o.getAttributeNode('value').specified:o.hasAttribute('value'))?o.value:o.text;if(o.selected){this.value=value;}
d.push([value,o.text]);}
...
this.tb=this.toolbars.BODY;this.tb.el.show();this.rendered=true;B.on('editorevent',this.updateToolbar,this);},updateToolbar:function(){if(!this.editor.activated){this.editor.onFirstFocus();return;}var G=this.editor.getAllAncestors();var ty=Roo.form.HtmlEditor.ToolbarContext.types;var H=G.length?(G[0]?G[0]:G[1]):this.editor.doc.body;H=H?H:this.editor.doc.body;H=H.tagName.length?H:this.editor.doc.body;var tn=H.tagName.toUpperCase();H=typeof(ty[tn])!='undefined'?H:this.editor.doc.body;tn=H.tagName.toUpperCase();if(this.tb.name==tn){return;}
this.tb.el.hide();this.tb=this.toolbars[tn];this.tb.el.show();this.tb.fields.each(function(e){e.setValue(H.getAttribute(e.name));});this.tb.selectedNode=H;Roo.menu.MenuMgr.hideAll();},onDestroy:function(){if(this.rendered){this.tb.items.each(function(I){if(I.menu){I.menu.removeAll();if(I.menu.el){I.menu.el.destroy();}}
...
this.fireEvent('rendered',this);return this;},addButton:function(E,F,G){var bc={handler:F,scope:G,minWidth:this.minButtonWidth,hideParent:true};if(typeof E=="string"){bc.text=E;}else {Roo.apply(bc,E);}var H=new Roo.Button(null,bc);this.buttons.push(H);return H;},addxtype:function(){var ar=Array.prototype.slice.call(arguments,0);var I=false;for(var i=0;i<ar.length;i++){if(!ar[i]){continue;}if(Roo.form[ar[i].xtype]){ar[i].form=this;var fe=Roo.factory(ar[i],Roo.form);if(!I){I=fe;}
...
this.end();continue;}
this.add(fe);}if(ar[i].xtype=='Button'){this.addButton(ar[i]);this.allItems.push(fe);continue;}if(ar[i].xtype=='end'){alert('end is not supported on xtype any more, use items');}}return I;},startMonitoring:function(){if(!this.bound){this.bound=true;Roo.TaskMgr.start({run:this.bindHandler,interval:this.monitorPoll||200,scope:this});}},stopMonitoring:function(){this.bound=false;},bindHandler:function(){if(!this.bound){return false;}var J=true;this.items.each(function(f){if(!f.isValid(true)){J=false;return false;}});for(var i=0,len=this.buttons.length;i<len;i++){var H=this.buttons[i];if(H.formBind===true&&H.disabled===J){H.setDisabled(!J);}}
this.fireEvent('clientvalidation',this,J);}});Roo.Form=Roo.form.Form;
Roo.form.Action=function(A,B){this.form=A;this.options=B||{};};Roo.form.Action.CLIENT_INVALID='client';Roo.form.Action.SERVER_INVALID='server';Roo.form.Action.CONNECT_FAILURE='connect';Roo.form.Action.LOAD_FAILURE='load';Roo.form.Action.prototype={type:'default',failureType:undefined,response:undefined,result:undefined,run:function(C){},success:function(D){},handleResponse:function(E){},failure:function(F){this.response=F;this.failureType=Roo.form.Action.CONNECT_FAILURE;this.form.afterAction(this,false);},processResponse:function(G){this.response=G;if(!G.responseText){return true;}
this.result=this.handleResponse(G);return this.result;},getUrl:function(H){var I=this.options.url||this.form.url||this.form.el.dom.action;if(H){var p=this.getParams();if(p){I+=(I.indexOf('?')!=-1?'&':'?')+p;}}return I;},getMethod:function(){return (this.options.method||this.form.method||this.form.el.dom.method||'POST').toUpperCase();},getParams:function(){var bp=this.form.baseParams;var p=this.options.params;if(p){if(typeof p=="object"){p=Roo.urlEncode(Roo.applyIf(p,bp));}else if(typeof p=='string'&&bp){p+='&'+Roo.urlEncode(bp);}}else if(bp){p=Roo.urlEncode(bp);}return p;},createCallback:function(){return {success:this.success,failure:this.failure,scope:this,timeout:(this.form.timeout*1000),upload:this.form.fileUpload?this.success:undefined};}};Roo.form.Action.Submit=function(J,K){Roo.form.Action.Submit.superclass.constructor.call(this,J,K);};Roo.extend(Roo.form.Action.Submit,Roo.form.Action,{type:'submit',run:function(){var o=this.options;var L=this.getMethod();var M=L=='POST';if(o.clientValidation===false||this.form.isValid()){Roo.Ajax.request(Roo.apply(this.createCallback(),{form:this.form.el.dom,url:this.getUrl(!M),method:L,params:M?this.getParams():null,isUpload:this.form.fileUpload}));}else if(o.clientValidation!==false){this.failureType=Roo.form.Action.CLIENT_INVALID;this.form.afterAction(this,false);}},success:function(N){var O=this.processResponse(N);if(O===true||O.success){this.form.afterAction(this,true);return;}if(O.errors){this.form.markInvalid(O.errors);this.failureType=Roo.form.Action.SERVER_INVALID;}
this.form.afterAction(this,false);},handleResponse:function(P){if(this.form.errorReader){var rs=this.form.errorReader.read(P);var errors=[];if(rs.records){for(var i=0,len=rs.records.length;i<len;i++){var r=rs.records[i];errors[i]=r.data;}}if(errors.length<1){errors=null;}return {success:rs.success,errors:errors};}var Q=false;try{Q=Roo.decode(P.responseText);}catch(e){ret={success:false,errorMsg:"Failed to read server message: "+response.responseText,errors:[]};}return Q;}});Roo.form.Action.Load=function(R,S){Roo.form.Action.Load.superclass.constructor.call(this,R,S);this.reader=this.form.reader;};Roo.extend(Roo.form.Action.Load,Roo.form.Action,{type:'load',run:function(){Roo.Ajax.request(Roo.apply(this.createCallback(),{method:this.getMethod(),url:this.getUrl(false),params:this.getParams()}));},success:function(T){var U=this.processResponse(T);if(U===true||!U.success||!U.data){this.failureType=Roo.form.Action.LOAD_FAILURE;this.form.afterAction(this,false);return;}
this.form.clearInvalid();this.form.setValues(U.data);this.form.afterAction(this,true);},handleResponse:function(V){if(this.form.reader){var rs=this.form.reader.read(V);var data=rs.records&&rs.records[0]?rs.records[0].data:null;return {success:rs.success,data:data};}return Roo.decode(V.responseText);}});Roo.form.Action.ACTION_TYPES={'load':Roo.form.Action.Load,'submit':Roo.form.Action.Submit};
Roo.form.Layout=function(A){var B=[];if(A.items){B=A.items;delete A.items;}
Roo.form.Layout.superclass.constructor.call(this,A);this.stack=[];Roo.each(B,this.addxtype,this);};Roo.extend(Roo.form.Layout,Roo.Component,{clear:true,labelSeparator:':',hideLabels:false,defaultAutoCreate:{tag:'div',cls:'x-form-ct'},isLayout:true,onRender:function(ct,C){if(this.el){this.el=Roo.get(this.el);}else {var cfg=this.getAutoCreate();this.el=ct.createChild(cfg,C);}if(this.style){this.el.applyStyles(this.style);}if(this.labelAlign){this.el.addClass('x-form-label-'+this.labelAlign);}if(this.hideLabels){this.labelStyle="display:none";this.elementStyle="padding-left:0;";}else {if(typeof this.labelWidth=='number'){this.labelStyle="width:"+this.labelWidth+"px;";this.elementStyle="padding-left:"+((this.labelWidth+(typeof this.labelPad=='number'?this.labelPad:5))+'px')+";";}if(this.labelAlign=='top'){this.labelStyle="width:auto;";this.elementStyle="padding-left:0;";}}var D=this.stack;var E=D.length;if(E>0){if(!this.fieldTpl){var t=new Roo.Template('<div class="x-form-item {5}">','<label for="{0}" style="{2}">{1}{4}</label>','<div class="x-form-element" id="x-form-el-{0}" style="{3}">','</div>','</div><div class="x-form-clear-left"></div>');t.disableFormats=true;t.compile();Roo.form.Layout.prototype.fieldTpl=t;}for(var i=0;i<E;i++){if(D[i].isFormField){this.renderField(D[i]);}else {this.renderComponent(D[i]);}}}if(this.clear){this.el.createChild({cls:'x-form-clear'});}},renderField:function(f){f.fieldEl=Roo.get(this.fieldTpl.append(this.el,[f.id,f.fieldLabel,f.labelStyle||this.labelStyle||'',this.elementStyle||'',typeof f.labelSeparator=='undefined'?this.labelSeparator:f.labelSeparator,f.itemCls||this.itemCls||''],true).getPrevSibling());},renderComponent:function(c){c.render(c.isLayout?this.el:this.el.createChild());},addxtype:function(o){o.form=this.form;var fe=Roo.factory(o,Roo.form);this.form.allItems.push(fe);this.stack.push(fe);if(fe.isFormField){this.form.items.add(fe);}return fe;}});Roo.form.Column=function(F){Roo.form.Column.superclass.constructor.call(this,F);};Roo.extend(Roo.form.Column,Roo.form.Layout,{defaultAutoCreate:{tag:'div',cls:'x-form-ct x-form-column'},onRender:function(ct,G){Roo.form.Column.superclass.onRender.call(this,ct,G);if(this.width){this.el.setWidth(this.width);}}});Roo.form.Row=function(H){Roo.form.Row.superclass.constructor.call(this,H);};Roo.extend(Roo.form.Row,Roo.form.Layout,{defaultAutoCreate:{tag:'div',cls:'x-form-ct x-form-row'},padWidth:20,onRender:function(ct,I){if(!this.rowTpl){var t=new Roo.Template('<div class="x-form-item {5}" style="float:left;width:{6}px">','<label for="{0}" style="{2}">{1}{4}</label>','<div class="x-form-element" id="x-form-el-{0}" style="{3}">','</div>','</div>');t.disableFormats=true;t.compile();Roo.form.Layout.prototype.rowTpl=t;}
this.fieldTpl=this.rowTpl;var J=100;if((this.labelAlign!='top')){if(typeof this.labelWidth=='number'){J=this.labelWidth}
this.padWidth=20+J;}
Roo.form.Column.superclass.onRender.call(this,ct,I);if(this.width){this.el.setWidth(this.width);}if(this.height){this.el.setHeight(this.height);}},renderField:function(f){f.fieldEl=this.fieldTpl.append(this.el,[f.id,f.fieldLabel,f.labelStyle||this.labelStyle||'',this.elementStyle||'',typeof f.labelSeparator=='undefined'?this.labelSeparator:f.labelSeparator,f.itemCls||this.itemCls||'',f.width?f.width+this.padWidth:160+this.padWidth],true);}});Roo.form.FieldSet=function(K){Roo.form.FieldSet.superclass.constructor.call(this,K);};Roo.extend(Roo.form.FieldSet,Roo.form.Layout,{defaultAutoCreate:{tag:'fieldset',cn:{tag:'legend'}},onRender:function(ct,L){Roo.form.FieldSet.superclass.onRender.call(this,ct,L);if(this.legend){this.setLegend(this.legend);}},setLegend:function(M){if(this.rendered){this.el.child('legend').update(M);}}});
...
this.isDirtyFlag=false;this.addEvents({"drop":true});};Roo.extend(Roo.DDView,Roo.View,{isFormField:true,reset:Roo.emptyFn,clearInvalid:Roo.form.Field.prototype.clearInvalid,validate:function(){return true;},destroy:function(){this.purgeListeners();this.getEl.removeAllListeners();this.getEl().remove();if(this.dragZone){if(this.dragZone.destroy){this.dragZone.destroy();}}if(this.dropZone){if(this.dropZone.destroy){this.dropZone.destroy();}}},getName:function(){return this.name;},setValue:function(v){if(!this.store){throw "DDView.setValue(). DDView must be constructed with a valid Store";}var D={};D[this.store.reader.meta.root]=v?[].concat(v):[];this.store.proxy=new Roo.data.MemoryProxy(D);this.store.load();},getValue:function(){var E='(';this.store.each(function(F){E+=F.id+',';});return E.substr(0,E.length-1)+')';},getIds:function(){var i=0,F=new Array(this.store.getCount());this.store.each(function(G){F[i++]=G.id;});return F;},isDirty:function(){return this.isDirtyFlag;},getTargetFromEvent:function(e){var G=e.getTarget();while((G!==null)&&(G.parentNode!=this.el.dom)){G=G.parentNode;}if(!G){G=this.el.dom.lastChild||this.el.dom;}return G;},getDragData:function(e){var H=this.findItemFromChild(e.getTarget());if(H){this.handleSelection(e);var selNodes=this.getSelectedNodes();var dragData={source:this,copy:this.copy||(this.allowCopy&&e.ctrlKey),nodes:selNodes,records:[]};var selectedIndices=this.getSelectedIndexes();for(var i=0;i<selectedIndices.length;i++){dragData.records.push(this.store.getAt(selectedIndices[i]));}if(selNodes.length==1){dragData.ddel=H.cloneNode(true);}else {var div=document.createElement('div');div.className='multi-proxy';for(var i=0,len=selNodes.length;i<len;i++){div.appendChild(selNodes[i].cloneNode(true));}
dragData.ddel=div;}return dragData;}return false;},setDraggable:function(I){if(I instanceof Array){Roo.each(I,this.setDraggable,this);return;}if(this.dragZone){this.dragZone.addToGroup(I);}else {this.dragZone=new Roo.dd.DragZone(this.getEl(),{containerScroll:true,ddGroup:I});if(!this.multiSelect){this.singleSelect=true;}
this.dragZone.getDragData=this.getDragData.createDelegate(this);}},setDroppable:function(J){if(J instanceof Array){Roo.each(J,this.setDroppable,this);return;}if(this.dropZone){this.dropZone.addToGroup(J);}else {this.dropZone=new Roo.dd.DropZone(this.getEl(),{containerScroll:true,ddGroup:J});this.dropZone.getTargetFromEvent=this.getTargetFromEvent.createDelegate(this);this.dropZone.onNodeEnter=this.onNodeEnter.createDelegate(this);this.dropZone.onNodeOver=this.onNodeOver.createDelegate(this);this.dropZone.onNodeOut=this.onNodeOut.createDelegate(this);this.dropZone.onNodeDrop=this.onNodeDrop.createDelegate(this);}},getDropPoint:function(e,n,dd){if(n==this.el.dom){return "above";}var t=Roo.lib.Dom.getY(n),b=t+n.offsetHeight;var c=t+(b-t)/2;var y=Roo.lib.Event.getPageY(e);if(y<=c){return "above";}else {return "below";}},onNodeEnter:function(n,dd,e,K){return false;},onNodeOver:function(n,dd,e,L){var pt=this.getDropPoint(e,n,dd);var M=this.dropNotAllowed;if(pt){var targetElClass;if(pt=="above"){M=n.previousSibling?"x-tree-drop-ok-between":"x-tree-drop-ok-above";targetElClass="x-view-drag-insert-above";}else {M=n.nextSibling?"x-tree-drop-ok-between":"x-tree-drop-ok-below";targetElClass="x-view-drag-insert-below";}if(this.lastInsertClass!=targetElClass){Roo.fly(n).replaceClass(this.lastInsertClass,targetElClass);this.lastInsertClass=targetElClass;}}return M;},onNodeOut:function(n,dd,e,N){this.removeDropIndicators(n);},onNodeDrop:function(n,dd,e,O){if(this.fireEvent("drop",this,n,dd,e,O)===false){return false;}var pt=this.getDropPoint(e,n,dd);var P=(n==this.el.dom)?this.nodes.length:n.nodeIndex;if(pt=="below"){P++;}for(var i=0;i<O.records.length;i++){var r=O.records[i];var dup=this.store.getById(r.id);if(dup&&(dd!=this.dragZone)){Roo.fly(this.getNode(this.store.indexOf(dup))).frame("red",1);}else {if(O.copy){this.store.insert(P++,r.copy());}else {O.source.isDirtyFlag=true;r.store.remove(r);this.store.insert(P++,r);}
this.isDirtyFlag=true;}}
this.dragZone.cachedTarget=null;return true;},removeDropIndicators:function(n){if(n){Roo.fly(n).removeClass(["x-view-drag-insert-above","x-view-drag-insert-below"]);this.lastInsertClass="_noclass";}},setDeletable:function(Q){if(!this.singleSelect&&!this.multiSelect){this.singleSelect=true;}var c=this.getContextMenu();this.contextMenu.on("itemclick",function(R){switch(R.id){case "delete":this.remove(this.getSelectedIndexes());break;}},this);this.contextMenu.add({icon:Q,id:"delete",text:'Delete'});},getContextMenu:function(){if(!this.contextMenu){this.contextMenu=new Roo.menu.Menu({id:this.id+"-contextmenu"});this.el.on("contextmenu",this.showContextMenu,this);}return this.contextMenu;},disableContextMenu:function(){if(this.contextMenu){this.el.un("contextmenu",this.showContextMenu,this);}},showContextMenu:function(e,R){R=this.findItemFromChild(e.getTarget());if(R){e.stopEvent();this.select(this.getNode(R),this.multiSelect&&e.ctrlKey,true);this.contextMenu.showAt(e.getXY());}},remove:function(S){S=[].concat(S);for(var i=0;i<S.length;i++){var rec=this.store.getAt(S[i]);this.store.remove(rec);}},onDblClick:function(e){var T=this.findItemFromChild(e.getTarget());if(T){if(this.fireEvent("dblclick",this,this.indexOf(T),T,e)===false){return false;}if(this.dragGroup){var targets=Roo.dd.DragDropMgr.getRelated(this.dragZone,true);while(targets.indexOf(this.dropZone)>-1){targets.remove(this.dropZone);}if(targets.length==1){this.dragZone.cachedTarget=null;var el=Roo.get(targets[0].getEl());var box=el.getBox(true);targets[0].onNodeDrop(el.dom,{target:el.dom,xy:[box.x,box.y+box.height-1]},null,this.getDragData(e));}}}},handleSelection:function(e){this.dragZone.cachedTarget=null;var U=this.findItemFromChild(e.getTarget());if(!U){this.clearSelections(true);return;}if(U&&(this.multiSelect||this.singleSelect)){if(this.multiSelect&&e.shiftKey&&(!e.ctrlKey)&&this.lastSelection){this.select(this.getNodes(this.indexOf(this.lastSelection),U.nodeIndex),false);}else if(this.isSelected(this.getNode(U))&&e.ctrlKey){this.unselect(U);}else {this.select(U,this.multiSelect&&e.ctrlKey);this.lastSelection=U;}}},onItemClick:function(V,W,e){if(this.fireEvent("beforeclick",this,W,V,e)===false){return false;}return true;},unselect:function(X,Y){var Z=this.getNode(X);if(Z&&this.isSelected(Z)){if(this.fireEvent("beforeselect",this,Z,this.selections)!==false){Roo.fly(Z).removeClass(this.selectedClass);this.selections.remove(Z);if(!Y){this.fireEvent("selectionchange",this,this.selections);}}}}});
...
this.id=this.el.id;this.el.addClass("x-layout-container");this.monitorWindowResize=true;this.regions={};this.addEvents({"layout":true,"regionresized":true,"regioncollapsed":true,"regionexpanded":true});this.updating=false;Roo.EventManager.onWindowResize(this.onWindowResize,this,true);};Roo.extend(Roo.LayoutManager,Roo.util.Observable,{isUpdating:function(){return this.updating;},beginUpdate:function(){this.updating=true;},endUpdate:function(C){this.updating=false;if(!C){this.layout();}},layout:function(){},onRegionResized:function(D,E){this.fireEvent("regionresized",D,E);this.layout();},onRegionCollapsed:function(F){this.fireEvent("regioncollapsed",F);},onRegionExpanded:function(G){this.fireEvent("regionexpanded",G);},getViewSize:function(){var H;if(this.el.dom!=document.body){H=this.el.getSize();}else {H={width:Roo.lib.Dom.getViewWidth(),height:Roo.lib.Dom.getViewHeight()};}
H.width-=this.el.getBorderWidth("lr")-this.el.getPadding("lr");H.height-=this.el.getBorderWidth("tb")-this.el.getPadding("tb");return H;},getEl:function(){return this.el;},getRegion:function(I){return this.regions[I.toLowerCase()];},onWindowResize:function(){if(this.monitorWindowResize){this.layout();}}});
Roo.BorderLayout=function(A,B){B=B||{};Roo.BorderLayout.superclass.constructor.call(this,A,B);this.factory=B.factory||Roo.BorderLayout.RegionFactory;for(var i=0,len=this.factory.validRegions.length;i<len;i++){var target=this.factory.validRegions[i];if(B[target]){this.addRegion(target,B[target]);}}};Roo.extend(Roo.BorderLayout,Roo.LayoutManager,{addRegion:function(C,D){if(!this.regions[C]){var r=this.factory.create(C,this,D);this.bindRegion(C,r);}return this.regions[C];},bindRegion:function(E,r){this.regions[E]=r;r.on("visibilitychange",this.layout,this);r.on("paneladded",this.layout,this);r.on("panelremoved",this.layout,this);r.on("invalidated",this.layout,this);r.on("resized",this.onRegionResized,this);r.on("collapsed",this.onRegionCollapsed,this);r.on("expanded",this.onRegionExpanded,this);},layout:function(){if(this.updating)return;var F=this.getViewSize();var w=F.width;var h=F.height;var G=w;var H=h;var I=0;var J=0;var rs=this.regions;var K=rs["north"];var L=rs["south"];var M=rs["west"];var N=rs["east"];var O=rs["center"];if(K&&K.isVisible()){var b=K.getBox();var m=K.getMargins();b.width=w-(m.left+m.right);b.x=m.left;b.y=m.top;I=b.height+b.y+m.bottom;H-=I;K.updateBox(this.safeBox(b));}if(L&&L.isVisible()){var b=L.getBox();var m=L.getMargins();b.width=w-(m.left+m.right);b.x=m.left;var totalHeight=(b.height+m.top+m.bottom);b.y=h-totalHeight+m.top;H-=totalHeight;L.updateBox(this.safeBox(b));}if(M&&M.isVisible()){var b=M.getBox();var m=M.getMargins();b.height=H-(m.top+m.bottom);b.x=m.left;b.y=I+m.top;var totalWidth=(b.width+m.left+m.right);J+=totalWidth;G-=totalWidth;M.updateBox(this.safeBox(b));}if(N&&N.isVisible()){var b=N.getBox();var m=N.getMargins();b.height=H-(m.top+m.bottom);var totalWidth=(b.width+m.left+m.right);b.x=w-totalWidth+m.left;b.y=I+m.top;G-=totalWidth;N.updateBox(this.safeBox(b));}if(O){var m=O.getMargins();var centerBox={x:J+m.left,y:I+m.top,width:G-(m.left+m.right),height:H-(m.top+m.bottom)};O.updateBox(this.safeBox(centerBox));}
this.el.repaint();this.fireEvent("layout",this);},safeBox:function(P){P.width=Math.max(0,P.width);P.height=Math.max(0,P.height);return P;},add:function(Q,R){Q=Q.toLowerCase();return this.regions[Q].add(R);},remove:function(S,T){S=S.toLowerCase();return this.regions[S].remove(T);},findPanel:function(U){var rs=this.regions;for(var S in rs){if(typeof rs[S]!="function"){var p=rs[S].getPanel(U);if(p){return p;}}}return null;},showPanel:function(V){var rs=this.regions;for(var S in rs){var r=rs[S];if(typeof r!="function"){if(r.hasPanel(V)){return r.showPanel(V);}}}return null;},restoreState:function(W){if(!W){W=Roo.state.Manager;}var sm=new Roo.LayoutStateManager();sm.init(this,W);},batchAdd:function(X){this.beginUpdate();for(var rname in X){var lr=this.regions[rname];if(lr){this.addTypedPanels(lr,X[rname]);}}
this.endUpdate();},addTypedPanels:function(lr,ps){if(typeof ps=='string'){lr.add(new Roo.ContentPanel(ps));}else if(ps instanceof Array){for(var i=0,len=ps.length;i<len;i++){this.addTypedPanels(lr,ps[i]);}}else if(!ps.events){var el=ps.el;delete ps.el;lr.add(new Roo.ContentPanel(el||Roo.id(),ps));}else {lr.add(ps);}},addxtype:function(Y){if(!Y.xtype.match(/Panel$/)){return false;}var Z=false;var a=Y.region;delete Y.region;var c=[];if(Y.items){c=Y.items;delete Y.items;}switch(Y.xtype){case 'ContentPanel':if(Y.autoCreate){Z=new Roo[Y.xtype](Y);}else {var el=this.el.createChild();Z=new Roo[Y.xtype](el,Y);}
this.add(a,Z);break;case 'TreePanel':Y.el=this.el.createChild();Z=new Roo[Y.xtype](Y);this.add(a,Z);break;case 'NestedLayoutPanel':var el=this.el.createChild();var clayout=Y.layout;delete Y.layout;clayout.items=clayout.items||[];c=clayout.items;if(a=='center'&&this.active&&this.getRegion('center').panels.length<1){Y.background=false;}var layout=new Roo.BorderLayout(el,clayout);Z=new Roo[Y.xtype](layout,Y);this.add(a,Z);break;case 'GridPanel':var el=this.el.createChild();var grid=new Roo.grid[Y.grid.xtype](el,Y.grid);delete Y.grid;if(a=='center'&&this.active){Y.background=false;}
Z=new Roo[Y.xtype](grid,Y);this.add(a,Z);if(Y.background){Z.on('activate',function(gp){if(!gp.grid.rendered){gp.grid.render();}});}else {grid.render();}break;default:alert("Can not add '"+Y.xtype+"' to BorderLayout");return;}
this.beginUpdate();Roo.each(c,function(i){Z.addxtype(i);});this.endUpdate();return Z;}});Roo.BorderLayout.create=function(d,e){var f=new Roo.BorderLayout(e||document.body,d);f.beginUpdate();var g=Roo.BorderLayout.RegionFactory.validRegions;for(var j=0,jlen=g.length;j<jlen;j++){var lr=g[j];if(f.regions[lr]&&d[lr].panels){var r=f.regions[lr];var ps=d[lr].panels;f.addTypedPanels(r,ps);}}
f.endUpdate();return f;};Roo.BorderLayout.RegionFactory={validRegions:["north","south","east","west","center"],create:function(k,l,n){k=k.toLowerCase();if(n.lightweight||n.basic){return new Roo.BasicLayoutRegion(l,n,k);}switch(k){case "north":return new Roo.NorthLayoutRegion(l,n);case "south":return new Roo.SouthLayoutRegion(l,n);case "east":return new Roo.EastLayoutRegion(l,n);case "west":return new Roo.WestLayoutRegion(l,n);case "center":return new Roo.CenterLayoutRegion(l,n);}throw 'Layout region "'+k+'" not supported.';}};
Roo.BasicLayoutRegion=function(A,B,C,D){this.mgr=A;this.position=C;this.events={"beforeremove":true,"invalidated":true,"visibilitychange":true,"paneladded":true,"panelremoved":true,"collapsed":true,"expanded":true,"slideshow":true,"slidehide":true,"panelactivated":true,"resized":true};this.panels=new Roo.util.MixedCollection();this.panels.getKey=this.getPanelId.createDelegate(this);this.box=null;this.activePanel=null;if(B.listeners||B.events){Roo.BasicLayoutRegion.superclass.constructor.call(this,{listeners:B.listeners||{},events:B.events||{}});}if(D!==true){this.applyConfig(B);}};Roo.extend(Roo.BasicLayoutRegion,Roo.util.Observable,{getPanelId:function(p){return p.getId();},applyConfig:function(E){this.margins=E.margins||this.margins||{top:0,left:0,right:0,bottom:0};this.config=E;},resizeTo:function(F){var el=this.el?this.el:(this.activePanel?this.activePanel.getEl():null);if(el){switch(this.position){case "east":case "west":el.setWidth(F);this.fireEvent("resized",this,F);break;case "north":case "south":el.setHeight(F);this.fireEvent("resized",this,F);break;}}},getBox:function(){return this.activePanel?this.activePanel.getEl().getBox(false,true):null;},getMargins:function(){return this.margins;},updateBox:function(G){this.box=G;var el=this.activePanel.getEl();el.dom.style.left=G.x+"px";el.dom.style.top=G.y+"px";this.activePanel.setSize(G.width,G.height);},getEl:function(){return this.activePanel;},isVisible:function(){return this.activePanel?true:false;},setActivePanel:function(H){H=this.getPanel(H);if(this.activePanel&&this.activePanel!=H){this.activePanel.setActiveState(false);this.activePanel.getEl().setLeftTop(-10000,-10000);}
this.activePanel=H;H.setActiveState(true);if(this.box){H.setSize(this.box.width,this.box.height);}
this.fireEvent("panelactivated",this,H);this.fireEvent("invalidated");},showPanel:function(I){if(I=this.getPanel(I)){this.setActivePanel(I);}return I;},getActivePanel:function(){return this.activePanel;},add:function(J){if(arguments.length>1){for(var i=0,len=arguments.length;i<len;i++){this.add(arguments[i]);}return null;}if(this.hasPanel(J)){this.showPanel(J);return J;}var el=J.getEl();if(el.dom.parentNode!=this.mgr.el.dom){this.mgr.el.dom.appendChild(el.dom);}if(J.setRegion){J.setRegion(this);}
this.panels.add(J);el.setStyle("position","absolute");if(!J.background){this.setActivePanel(J);if(this.config.initialSize&&this.panels.getCount()==1){this.resizeTo(this.config.initialSize);}}
this.fireEvent("paneladded",this,J);return J;},hasPanel:function(K){if(typeof K=="object"){K=K.getId();}return this.getPanel(K)?true:false;},remove:function(L,M){L=this.getPanel(L);if(!L){return null;}var e={};this.fireEvent("beforeremove",this,L,e);if(e.cancel===true){return null;}var N=L.getId();this.panels.removeKey(N);return L;},getPanel:function(id){if(typeof id=="object"){return id;}return this.panels.get(id);},getPosition:function(){return this.position;}});
...
this.duration=c.duration||.30;this.slideDuration=c.slideDuration||.45;this.config=c;if(c.collapsed){this.collapse(true);}if(c.hidden){this.hide();}},isVisible:function(){return this.visible;},setCollapsedTitle:function(D){D=D||"&#160;";if(this.collapsedTitleTextEl){this.collapsedTitleTextEl.innerHTML=D;}},getBox:function(){var b;if(!this.collapsed){b=this.el.getBox(false,true);}else {b=this.collapsedEl.getBox(false,true);}return b;},getMargins:function(){return this.collapsed?this.cmargins:this.margins;},highlight:function(){this.el.addClass("x-layout-panel-dragover");},unhighlight:function(){this.el.removeClass("x-layout-panel-dragover");},updateBox:function(E){this.box=E;if(!this.collapsed){this.el.dom.style.left=E.x+"px";this.el.dom.style.top=E.y+"px";this.updateBody(E.width,E.height);}else {this.collapsedEl.dom.style.left=E.x+"px";this.collapsedEl.dom.style.top=E.y+"px";this.collapsedEl.setSize(E.width,E.height);}if(this.tabs){this.tabs.autoSizeTabs();}},updateBody:function(w,h){if(w!==null){this.el.setWidth(w);w-=this.el.getBorderWidth("rl");if(this.config.adjustments){w+=this.config.adjustments[0];}}if(h!==null){this.el.setHeight(h);h=this.titleEl&&this.titleEl.isDisplayed()?h-(this.titleEl.getHeight()||0):h;h-=this.el.getBorderWidth("tb");if(this.config.adjustments){h+=this.config.adjustments[1];}
...
this.visible=true;this.fireEvent("visibilitychange",this,true);},closeClicked:function(){if(this.activePanel){this.remove(this.activePanel);}},collapseClick:function(e){if(this.isSlid){e.stopPropagation();this.slideIn();}else {e.stopPropagation();this.slideOut();}},collapse:function(F){if(this.collapsed)return;this.collapsed=true;if(this.split){this.split.el.hide();}if(this.config.animate&&F!==true){this.fireEvent("invalidated",this);this.animateCollapse();}else {this.el.setLocation(-20000,-20000);this.el.hide();this.collapsedEl.show();this.fireEvent("collapsed",this);this.fireEvent("invalidated",this);}},animateCollapse:function(){},expand:function(e,G){if(e)e.stopPropagation();if(!this.collapsed||this.el.hasActiveFx())return;if(this.isSlid){this.afterSlideIn();G=true;}
this.collapsed=false;if(this.config.animate&&G!==true){this.animateExpand();}else {this.el.show();if(this.split){this.split.el.show();}
...
this.tabs=ts;ts.resizeTabs=this.config.resizeTabs===true;ts.minTabWidth=this.config.minTabWidth||40;ts.maxTabWidth=this.config.maxTabWidth||250;ts.preferredTabWidth=this.config.preferredTabWidth||150;ts.monitorResize=false;ts.bodyEl.setStyle("overflow",this.config.autoScroll?"auto":"hidden");ts.bodyEl.addClass('x-layout-tabs-body');this.panels.each(this.initPanelAsTab,this);},initPanelAsTab:function(H){var ti=this.tabs.addTab(H.getEl().id,H.getTitle(),null,this.config.closeOnTab&&H.isClosable());if(H.tabTip!==undefined){ti.setTooltip(H.tabTip);}
ti.on("activate",function(){this.setActivePanel(H);},this);if(this.config.closeOnTab){ti.on("beforeclose",function(t,e){e.cancel=true;this.remove(H);},this);}return ti;},updatePanelTitle:function(I,J){if(this.activePanel==I){this.updateTitle(J);}if(this.tabs){var ti=this.tabs.getTab(I.getEl().id);ti.setText(J);if(I.tabTip!==undefined){ti.setTooltip(I.tabTip);}}},updateTitle:function(K){if(this.titleTextEl&&!this.config.title){this.titleTextEl.innerHTML=(typeof K!="undefined"&&K.length>0?K:"&#160;");}},setActivePanel:function(L){L=this.getPanel(L);if(this.activePanel&&this.activePanel!=L){this.activePanel.setActiveState(false);}
this.activePanel=L;L.setActiveState(true);if(this.panelSize){L.setSize(this.panelSize.width,this.panelSize.height);}if(this.closeBtn){this.closeBtn.setVisible(!this.config.closeOnTab&&!this.isSlid&&L.isClosable());}
this.updateTitle(L.getTitle());if(this.tabs){this.fireEvent("invalidated",this);}
this.fireEvent("panelactivated",this,L);},showPanel:function(M){if(M=this.getPanel(M)){if(this.tabs){var tab=this.tabs.getTab(M.getEl().id);if(tab.isHidden()){this.tabs.unhideTab(tab.id);}
tab.activate();}else {this.setActivePanel(M);}}return M;},getActivePanel:function(){return this.activePanel;},validateVisibility:function(){if(this.panels.getCount()<1){this.updateTitle("&#160;");this.closeBtn.hide();this.hide();}else {if(!this.isVisible()){this.show();}}},add:function(N){if(arguments.length>1){for(var i=0,len=arguments.length;i<len;i++){this.add(arguments[i]);}return null;}if(this.hasPanel(N)){this.showPanel(N);return N;}
N.setRegion(this);this.panels.add(N);if(this.panels.getCount()==1&&!this.config.alwaysShowTabs){this.bodyEl.dom.appendChild(N.getEl().dom);if(N.background!==true){this.setActivePanel(N);}
this.fireEvent("paneladded",this,N);return N;}if(!this.tabs){this.initTabs();}else {this.initPanelAsTab(N);}if(N.background!==true){this.tabs.activate(N.getEl().id);}
this.fireEvent("paneladded",this,N);return N;},hidePanel:function(O){if(this.tabs&&(O=this.getPanel(O))){this.tabs.hideTab(O.getEl().id);}},unhidePanel:function(P){if(this.tabs&&(P=this.getPanel(P))){this.tabs.unhideTab(P.getEl().id);}},clearPanels:function(){while(this.panels.getCount()>0){this.remove(this.panels.first());}},remove:function(Q,R){Q=this.getPanel(Q);if(!Q){return null;}var e={};this.fireEvent("beforeremove",this,Q,e);if(e.cancel===true){return null;}
...
Q.setRegion(null);if(this.activePanel==Q){this.activePanel=null;}if(this.config.autoDestroy!==false&&R!==true){try{Q.destroy();}catch(e){}}
this.fireEvent("panelremoved",this,Q);return Q;},getTabs:function(){return this.tabs;},createTool:function(T,U){var V=Roo.DomHelper.append(T,{tag:"div",cls:"x-layout-tools-button",children:[{tag:"div",cls:"x-layout-tools-button-inner "+U,html:"&#160;"}]},true);V.addClassOnOver("x-layout-tools-button-over");return V;}});
...
this.rendered=true;this.fireEvent('render',this);return this;},reconfigure:function(D,E){if(this.loadMask){this.loadMask.destroy();this.loadMask=new Roo.LoadMask(this.container,Roo.apply({store:D},this.loadMask));}
this.view.bind(D,E);this.dataSource=D;this.colModel=E;this.view.refresh(true);},onKeyDown:function(e){this.fireEvent("keydown",e);},destroy:function(F,G){if(this.loadMask){this.loadMask.destroy();}var c=this.container;c.removeAllListeners();this.view.destroy();this.colModel.purgeListeners();if(!G){this.purgeListeners();}
c.update("");if(F===true){c.remove();}},processEvent:function(H,e){this.fireEvent(H,e);var t=e.getTarget();var v=this.view;var I=v.findHeaderIndex(t);if(I!==false){this.fireEvent("header"+H,this,I,e);}else {var row=v.findRowIndex(t);var cell=v.findCellIndex(t);if(row!==false){this.fireEvent("row"+H,this,row,e);if(cell!==false){this.fireEvent("cell"+H,this,row,cell,e);}}}},onClick:function(e){this.processEvent("click",e);},onContextMenu:function(e,t){this.processEvent("contextmenu",e);},onDblClick:function(e){this.processEvent("dblclick",e);},walkCells:function(J,K,L,fn,M){var cm=this.colModel,N=cm.getColumnCount();var ds=this.dataSource,O=ds.getCount(),P=true;if(L<0){if(K<0){J--;P=false;}while(J>=0){if(!P){K=N-1;}
P=false;while(K>=0){if(fn.call(M||this,J,K,cm)===true){return [J,K];}
K--;}
J--;}}else {if(K>=N){J++;P=false;}while(J<O){if(!P){K=0;}
P=false;while(K<N){if(fn.call(M||this,J,K,cm)===true){return [J,K];}
K++;}
J++;}}return null;},getSelections:function(){return this.selModel.getSelections();},autoSize:function(){if(this.rendered){this.view.layout();if(this.view.adjustForScroll){this.view.adjustForScroll();}}},getGridEl:function(){return this.container;},stopEditing:function(){},getSelectionModel:function(){if(!this.selModel){this.selModel=new Roo.grid.RowSelectionModel();}return this.selModel;},getDataSource:function(){return this.dataSource;},getColumnModel:function(){return this.colModel;},getView:function(){if(!this.view){this.view=new Roo.grid.GridView(this.viewConfig);}return this.view;},getDragDropText:function(){var Q=this.selModel.getCount();return String.format(this.ddText,Q,Q==1?'':'s');}});Roo.grid.Grid.prototype.ddText="{0} selected row{1}";
Roo.grid.AbstractGridView=function(){this.grid=null;this.events={"beforerowremoved":true,"beforerowsinserted":true,"beforerefresh":true,"rowremoved":true,"rowsinserted":true,"rowupdated":true,"refresh":true};Roo.grid.AbstractGridView.superclass.constructor.call(this);};Roo.extend(Roo.grid.AbstractGridView,Roo.util.Observable,{rowClass:"x-grid-row",cellClass:"x-grid-cell",tdClass:"x-grid-td",hdClass:"x-grid-hd",splitClass:"x-grid-hd-split",init:function(A){this.grid=A;var B=this.grid.getGridEl().id;this.colSelector="#"+B+" ."+this.cellClass+"-";this.tdSelector="#"+B+" ."+this.tdClass+"-";this.hdSelector="#"+B+" ."+this.hdClass+"-";this.splitSelector="#"+B+" ."+this.splitClass+"-";},getColumnRenderers:function(){var C=[];var cm=this.grid.colModel;var D=cm.getColumnCount();for(var i=0;i<D;i++){C[i]=cm.getRenderer(i);}return C;},getColumnIds:function(){var E=[];var cm=this.grid.colModel;var F=cm.getColumnCount();for(var i=0;i<F;i++){E[i]=cm.getColumnId(i);}return E;},getDataIndexes:function(){if(!this.indexMap){this.indexMap=this.buildIndexMap();}return this.indexMap.colToData;},getColumnIndexByDataIndex:function(G){if(!this.indexMap){this.indexMap=this.buildIndexMap();}return this.indexMap.dataToCol[G];},setCSSStyle:function(H,I,J){var K="#"+this.grid.id+" .x-grid-col-"+H;Roo.util.CSS.updateRule(K,I,J);},generateRules:function(cm){var L=[],M=this.grid.id+'-cssrules';Roo.util.CSS.removeStyleSheet(M);for(var i=0,len=cm.getColumnCount();i<len;i++){var B=cm.getColumnId(i);L.push(this.colSelector,B," {\n",cm.config[i].css,"}\n",this.tdSelector,B," {\n}\n",this.hdSelector,B," {\n}\n",this.splitSelector,B," {\n}\n");}return Roo.util.CSS.createStyleSheet(L.join(""),M);}});
...
h=h.prevSibling;}return null;},positionIndicator:function(h,n,e){var x=Roo.lib.Event.getPageX(e);var r=Roo.lib.Dom.getRegion(n.firstChild);var px,pt,py=r.top+this.proxyOffsets[1];if((r.right-x)<=(r.right-r.left)/2){px=r.right+this.view.borderWidth;pt="after";}else {px=r.left;pt="before";}var D=this.view.getCellIndex(h);var E=this.view.getCellIndex(n);if(this.grid.colModel.isFixed(E)){return false;}var F=this.grid.colModel.isLocked(E);if(pt=="after"){E++;}if(D<E){E--;}if(D==E&&(F==this.grid.colModel.isLocked(D))){return false;}
...
this.proxyBottom.setLeftTop(px,py+this.proxyTop.dom.offsetHeight+this.bottomOffset);this.proxyBottom.show();return pt;},onNodeEnter:function(n,dd,e,G){if(G.header!=n){this.positionIndicator(G.header,n,e);}},onNodeOver:function(n,dd,e,H){var I=false;if(H.header!=n){I=this.positionIndicator(H.header,n,e);}if(!I){this.proxyTop.hide();this.proxyBottom.hide();}return I?this.dropAllowed:this.dropNotAllowed;},onNodeOut:function(n,dd,e,J){this.proxyTop.hide();this.proxyBottom.hide();},onNodeDrop:function(n,dd,e,K){var h=K.header;if(h!=n){var cm=this.grid.colModel;var x=Roo.lib.Event.getPageX(e);var r=Roo.lib.Dom.getRegion(n.firstChild);var pt=(r.right-x)<=((r.right-r.left)/2)?"after":"before";var D=this.view.getCellIndex(h);var E=this.view.getCellIndex(n);var F=cm.isLocked(E);if(pt=="after"){E++;}if(D<E){E--;}if(D==E&&(F==cm.isLocked(D))){return false;}
cm.setLocked(D,F,true);cm.moveColumn(D,E);this.grid.fireEvent("columnmove",D,E);return true;}return false;}});
...
this.cm=cm;},init:function(B){Roo.grid.GridView.superclass.init.call(this,B);this.bind(B.dataSource,B.colModel);B.on("headerclick",this.handleHeaderClick,this);if(B.trackMouseOver){B.on("mouseover",this.onRowOver,this);B.on("mouseout",this.onRowOut,this);}
B.cancelTextSelection=function(){};this.gridId=B.id;var C=this.templates||{};if(!C.master){C.master=new Roo.Template('<div class="x-grid" hidefocus="true">','<div class="x-grid-topbar"></div>','<div class="x-grid-scroller"><div></div></div>','<div class="x-grid-locked">','<div class="x-grid-header">{lockedHeader}</div>','<div class="x-grid-body">{lockedBody}</div>',"</div>",'<div class="x-grid-viewport">','<div class="x-grid-header">{header}</div>','<div class="x-grid-body">{body}</div>',"</div>",'<div class="x-grid-bottombar"></div>','<a href="#" class="x-grid-focus" tabIndex="-1"></a>','<div class="x-grid-resize-proxy">&#160;</div>',"</div>");C.master.disableformats=true;}if(!C.header){C.header=new Roo.Template('<table border="0" cellspacing="0" cellpadding="0">','<tbody><tr class="x-grid-hd-row">{cells}</tr></tbody>',"</table>{splits}");C.header.disableformats=true;}
C.header.compile();if(!C.hcell){C.hcell=new Roo.Template('<td class="x-grid-hd x-grid-td-{id} {cellId}"><div title="{title}" class="x-grid-hd-inner x-grid-hd-{id}">','<div class="x-grid-hd-text" unselectable="on">{value}<img class="x-grid-sort-icon" src="',Roo.BLANK_IMAGE_URL,'" /></div>',"</div></td>");C.hcell.disableFormats=true;}
C.hcell.compile();if(!C.hsplit){C.hsplit=new Roo.Template('<div class="x-grid-split {splitId} x-grid-split-{id}" style="{style}" unselectable="on">&#160;</div>');C.hsplit.disableFormats=true;}
C.hsplit.compile();if(!C.body){C.body=new Roo.Template('<table border="0" cellspacing="0" cellpadding="0">',"<tbody>{rows}</tbody>","</table>");C.body.disableFormats=true;}
C.body.compile();if(!C.row){C.row=new Roo.Template('<tr class="x-grid-row {alt}">{cells}</tr>');C.row.disableFormats=true;}
C.row.compile();if(!C.cell){C.cell=new Roo.Template('<td class="x-grid-col x-grid-td-{id} {cellId} {css}" tabIndex="0">','<div class="x-grid-col-{id} x-grid-cell-inner"><div class="x-grid-cell-text" unselectable="on" {attr}>{value}</div></div>',"</td>");C.cell.disableFormats=true;}
C.cell.compile();this.templates=C;},onColWidthChange:function(){this.updateColumns.apply(this,arguments);},onHeaderChange:function(){this.updateHeaders.apply(this,arguments);},onHiddenChange:function(){this.handleHiddenChange.apply(this,arguments);},onColumnMove:function(){this.handleColumnMove.apply(this,arguments);},onColumnLock:function(){this.handleLockChange.apply(this,arguments);},onDataChange:function(){this.refresh();this.updateHeaderSortState();},onClear:function(){this.refresh();},onUpdate:function(ds,D){this.refreshRow(D);},refreshRow:function(E){var ds=this.ds,F;if(typeof E=='number'){F=E;E=ds.getAt(F);}else {F=ds.indexOf(E);}
this.insertRows(ds,F,F,true);this.onRemove(ds,E,F+1,true);this.syncRowHeights(F,F);this.layout();this.fireEvent("rowupdated",this,F,E);},onAdd:function(ds,G,H){this.insertRows(ds,H,H+(G.length-1));},onRemove:function(ds,I,J,K){if(K!==true){this.fireEvent("beforerowremoved",this,J,I);}var bt=this.getBodyTable(),lt=this.getLockedTable();if(bt.rows[J]){bt.firstChild.removeChild(bt.rows[J]);}if(lt.rows[J]){lt.firstChild.removeChild(lt.rows[J]);}if(K!==true){this.stripeRows(J);this.syncRowHeights(J,J);this.layout();this.fireEvent("rowremoved",this,J,I);}},onLoad:function(){this.scrollToTop();},scrollToTop:function(){if(this.scroller){this.scroller.dom.scrollTop=0;this.syncScroll();}},getHeaderPanel:function(L){if(L){this.headerPanel.show();}return this.headerPanel;},getFooterPanel:function(M){if(M){this.footerPanel.show();}return this.footerPanel;},initElements:function(){var E=Roo.Element;var el=this.grid.getGridEl().dom.firstChild;var cs=el.childNodes;this.el=new E(el);this.headerPanel=new E(el.firstChild);this.headerPanel.enableDisplayMode("block");this.scroller=new E(cs[1]);this.scrollSizer=new E(this.scroller.dom.firstChild);this.lockedWrap=new E(cs[2]);this.lockedHd=new E(this.lockedWrap.dom.firstChild);this.lockedBody=new E(this.lockedWrap.dom.childNodes[1]);this.mainWrap=new E(cs[3]);this.mainHd=new E(this.mainWrap.dom.firstChild);this.mainBody=new E(this.mainWrap.dom.childNodes[1]);this.footerPanel=new E(cs[4]);this.footerPanel.enableDisplayMode("block");this.focusEl=new E(cs[5]);this.focusEl.swallowEvent("click",true);this.resizeProxy=new E(cs[6]);this.headerSelector=String.format('#{0} td.x-grid-hd, #{1} td.x-grid-hd',this.lockedHd.id,this.mainHd.id);this.splitterSelector=String.format('#{0} div.x-grid-split, #{1} div.x-grid-split',this.idToCssName(this.lockedHd.id),this.idToCssName(this.mainHd.id));},idToCssName:function(s){return s.replace(/[^a-z0-9]+/ig,'-');},getHeaderCell:function(N){return Roo.DomQuery.select(this.headerSelector)[N];},getHeaderCellMeasure:function(O){return this.getHeaderCell(O).firstChild;},getHeaderCellText:function(P){return this.getHeaderCell(P).firstChild.firstChild;},getLockedTable:function(){return this.lockedBody.dom.firstChild;},getBodyTable:function(){return this.mainBody.dom.firstChild;},getLockedRow:function(Q){return this.getLockedTable().rows[Q];},getRow:function(R){return this.getBodyTable().rows[R];},getRowComposite:function(S){if(!this.rowEl){this.rowEl=new Roo.CompositeElementLite();}var T=[],U,V;if(U=this.getLockedRow(S)){T.push(U);}if(V=this.getRow(S)){T.push(V);}
this.rowEl.elements=T;return this.rowEl;},getCell:function(W,X){var Y=this.cm.getLockedCount();var Z;if(X<Y){Z=this.lockedBody.dom.firstChild;}else {Z=this.mainBody.dom.firstChild;X-=Y;}return Z.rows[W].childNodes[X];},getCellText:function(a,b){return this.getCell(a,b).firstChild.firstChild;},getCellBox:function(c){var b=this.fly(c).getBox();if(Roo.isOpera){b.y=c.offsetTop+this.mainBody.getY();}return b;},getCellIndex:function(d){var id=String(d.className).match(this.cellRE);if(id){return parseInt(id[1],10);}return 0;},findHeaderIndex:function(n){var r=Roo.fly(n).findParent("td."+this.hdClass,6);return r?this.getCellIndex(r):false;},findHeaderCell:function(n){var r=Roo.fly(n).findParent("td."+this.hdClass,6);return r?r:false;},findRowIndex:function(n){if(!n){return false;}var r=Roo.fly(n).findParent("tr."+this.rowClass,6);return r?r.rowIndex:false;},findCellIndex:function(e){var f=this.el.dom;while(e&&e!=f){if(this.findRE.test(e.className)){return this.getCellIndex(e);}
e=e.parentNode;}return false;},getColumnId:function(g){return this.cm.getColumnId(g);},getSplitters:function(){if(this.splitterSelector){return Roo.DomQuery.select(this.splitterSelector);}else {return null;}},getSplitter:function(k){return this.getSplitters()[k];},onRowOver:function(e,t){var o;if((o=this.findRowIndex(t))!==false){this.getRowComposite(o).addClass("x-grid-row-over");}},onRowOut:function(e,t){var p;if((p=this.findRowIndex(t))!==false&&p!==this.findRowIndex(e.getRelatedTarget())){this.getRowComposite(p).removeClass("x-grid-row-over");}},renderHeaders:function(){var cm=this.cm;var ct=this.templates.hcell,ht=this.templates.header,st=this.templates.hsplit;var cb=[],lb=[],sb=[],q=[],p={};for(var i=0,len=cm.getColumnCount();i<len;i++){p.cellId="x-grid-hd-0-"+i;p.splitId="x-grid-csplit-0-"+i;p.id=cm.getColumnId(i);p.title=cm.getColumnTooltip(i)||"";p.value=cm.getColumnHeader(i)||"";p.style=(this.grid.enableColumnResize===false||!cm.isResizable(i)||cm.isFixed(i))?'cursor:default':'';if(!cm.isLocked(i)){cb[cb.length]=ct.apply(p);sb[sb.length]=st.apply(p);}else {lb[lb.length]=ct.apply(p);q[q.length]=st.apply(p);}}return [ht.apply({cells:lb.join(""),splits:q.join("")}),ht.apply({cells:cb.join(""),splits:sb.join("")})];},updateHeaders:function(){var u=this.renderHeaders();this.lockedHd.update(u[0]);this.mainHd.update(u[1]);},focusRow:function(v){var x=this.scroller.dom.scrollLeft;this.focusCell(v,0,false);this.scroller.dom.scrollLeft=x;},focusCell:function(y,z,AA){var el=this.ensureVisible(y,z,AA);this.focusEl.alignTo(el,"tl-tl");if(Roo.isGecko){this.focusEl.focus();}else {this.focusEl.focus.defer(1,this.focusEl);}},ensureVisible:function(AB,AC,AD){if(typeof AB!="number"){AB=AB.rowIndex;}if(AB<0&&AB>=this.ds.getCount()){return;}
AC=(AC!==undefined?AC:0);var cm=this.grid.colModel;while(cm.isHidden(AC)){AC++;}var el=this.getCell(AB,AC);if(!el){return;}var c=this.scroller.dom;var AE=parseInt(el.offsetTop,10);var AF=parseInt(el.offsetLeft,10);var AG=AE+el.offsetHeight;var AH=AF+el.offsetWidth;var ch=c.clientHeight-this.mainHd.dom.offsetHeight;var AI=parseInt(c.scrollTop,10);var AJ=parseInt(c.scrollLeft,10);var AK=AI+ch;var AL=AJ+c.clientWidth;if(AE<AI){c.scrollTop=AE;}else if(AG>AK){c.scrollTop=AG-ch;}if(AD!==false){if(AF<AJ){c.scrollLeft=AF;}else if(AH>AL){c.scrollLeft=AH-c.clientWidth;}}return el;},updateColumns:function(){this.grid.stopEditing();var cm=this.grid.colModel,AM=this.getColumnIds();var AN=0;for(var i=0,len=cm.getColumnCount();i<len;i++){var w=cm.getColumnWidth(i);this.css.updateRule(this.colSelector+this.idToCssName(AM[i]),"width",(w-this.borderWidth)+"px");this.css.updateRule(this.hdSelector+this.idToCssName(AM[i]),"width",(w-this.borderWidth)+"px");}
this.updateSplitters();},generateRules:function(cm){var AO=[],AP=this.idToCssName(this.grid.id)+'-cssrules';Roo.util.CSS.removeStyleSheet(AP);for(var i=0,len=cm.getColumnCount();i<len;i++){var cid=cm.getColumnId(i);var align='';if(cm.config[i].align){align='text-align:'+cm.config[i].align+';';}var hidden='';if(cm.isHidden(i)){hidden='display:none;';}var width="width:"+(cm.getColumnWidth(i)-this.borderWidth)+"px;";AO.push(this.colSelector,cid," {\n",cm.config[i].css,align,width,"\n}\n",this.hdSelector,cid," {\n",align,width,"}\n",this.tdSelector,cid," {\n",hidden,"\n}\n",this.splitSelector,cid," {\n",hidden,"\n}\n");}return Roo.util.CSS.createStyleSheet(AO.join(""),AP);},updateSplitters:function(){var cm=this.cm,s=this.getSplitters();if(s){var AN=0,Y=true;for(var i=0,len=cm.getColumnCount();i<len;i++){if(cm.isHidden(i))continue;var w=cm.getColumnWidth(i);if(!cm.isLocked(i)&&Y){AN=0;Y=false;}
AN+=w;s[i].style.left=(AN-this.splitOffset)+"px";}}},handleHiddenChange:function(AQ,AR,AS){if(AS){this.hideColumn(AR);}else {this.unhideColumn(AR);}},hideColumn:function(AT){var AU=this.getColumnId(AT);this.css.updateRule(this.tdSelector+this.idToCssName(AU),"display","none");this.css.updateRule(this.splitSelector+this.idToCssName(AU),"display","none");if(Roo.isSafari){this.updateHeaders();}
this.updateSplitters();this.layout();},unhideColumn:function(AV){var AW=this.getColumnId(AV);this.css.updateRule(this.tdSelector+this.idToCssName(AW),"display","");this.css.updateRule(this.splitSelector+this.idToCssName(AW),"display","");if(Roo.isSafari){this.updateHeaders();}
this.updateSplitters();this.layout();},insertRows:function(dm,AX,AY,AZ){if(AX==0&&AY==dm.getCount()-1){this.refresh();}else {if(!AZ){this.fireEvent("beforerowsinserted",this,AX,AY);}var s=this.getScrollState();var markup=this.renderRows(AX,AY);this.bufferRows(markup[0],this.getLockedTable(),AX);this.bufferRows(markup[1],this.getBodyTable(),AX);this.restoreScroll(s);if(!AZ){this.fireEvent("rowsinserted",this,AX,AY);this.syncRowHeights(AX,AY);this.stripeRows(AX);this.layout();}}},bufferRows:function(Aa,Ab,Ac){var Ad=null,Ae=Ab.rows,Af=Ab.tBodies[0];if(Ac<Ae.length){Ad=Ae[Ac];}var b=document.createElement("div");b.innerHTML="<table><tbody>"+Aa+"</tbody></table>";var Ag=b.firstChild.rows;for(var i=0,len=Ag.length;i<len;i++){if(Ad){Af.insertBefore(Ag[0],Ad);}else {Af.appendChild(Ag[0]);}}
b.innerHTML="";b=null;},deleteRows:function(dm,Ah,Ai){if(dm.getRowCount()<1){this.fireEvent("beforerefresh",this);this.mainBody.update("");this.lockedBody.update("");this.fireEvent("refresh",this);}else {this.fireEvent("beforerowsdeleted",this,Ah,Ai);var bt=this.getBodyTable();var Af=bt.firstChild;var Ag=bt.rows;for(var a=Ah;a<=Ai;a++){Af.removeChild(Ag[Ah]);}
this.stripeRows(Ah);this.fireEvent("rowsdeleted",this,Ah,Ai);}},updateRows:function(Aj,Ak,Al){var s=this.getScrollState();this.refresh();this.restoreScroll(s);},handleSort:function(Am,An,Ao,Ap){if(!Ap){this.refresh();}
this.updateHeaderSortState();},getScrollState:function(){var sb=this.scroller.dom;return {left:sb.scrollLeft,top:sb.scrollTop};},stripeRows:function(Aq){if(!this.grid.stripeRows||this.ds.getCount()<1){return;}
Aq=Aq||0;var Ar=this.getBodyTable().rows;var As=this.getLockedTable().rows;var At=' x-grid-row-alt ';for(var i=Aq,len=Ar.length;i<len;i++){var AB=Ar[i],U=As[i];var isAlt=((i+1)%2==0);var hasAlt=(' '+AB.className+' ').indexOf(At)!=-1;if(isAlt==hasAlt){continue;}if(isAlt){AB.className+=" x-grid-row-alt";}else {AB.className=AB.className.replace("x-grid-row-alt","");}if(U){U.className=AB.className;}}},restoreScroll:function(Au){var sb=this.scroller.dom;sb.scrollLeft=Au.left;sb.scrollTop=Au.top;this.syncScroll();},syncScroll:function(){var sb=this.scroller.dom;var sh=this.mainHd.dom;var bs=this.mainBody.dom;var lv=this.lockedBody.dom;sh.scrollLeft=bs.scrollLeft=sb.scrollLeft;lv.scrollTop=bs.scrollTop=sb.scrollTop;},handleScroll:function(e){this.syncScroll();var sb=this.scroller.dom;this.grid.fireEvent("bodyscroll",sb.scrollLeft,sb.scrollTop);e.stopEvent();},handleWheel:function(e){var d=e.getWheelDelta();this.scroller.dom.scrollTop-=d*22;this.lockedBody.dom.scrollTop=this.mainBody.dom.scrollTop=this.scroller.dom.scrollTop;e.stopEvent();},renderRows:function(Av,Aw){var g=this.grid,cm=g.colModel,ds=g.dataSource,Ax=g.stripeRows;var Ay=cm.getColumnCount();if(ds.getCount()<1){return ["",""];}var cs=[];for(var i=0;i<Ay;i++){var name=cm.getDataIndex(i);cs[i]={name:typeof name=='undefined'?ds.fields.get(i).name:name,renderer:cm.getRenderer(i),id:cm.getColumnId(i),locked:cm.isLocked(i)};}
Av=Av||0;Aw=typeof Aw=="undefined"?ds.getCount()-1:Aw;var rs=ds.getRange(Av,Aw);return this.doRender(cs,rs,ds,Av,Ay,Ax);},doRender:Roo.isGecko?function(cs,rs,ds,Az,A0,A1){var ts=this.templates,ct=ts.cell,rt=ts.row;var A2="",A3="",cb,A4,c,p={},rp={},r,a;for(var j=0,len=rs.length;j<len;j++){r=rs[j];cb="";A4="";a=(j+Az);for(var i=0;i<A0;i++){c=cs[i];p.cellId="x-grid-cell-"+a+"-"+i;p.id=c.id;p.css=p.attr="";p.value=c.renderer(r.data[c.name],p,r,a,i,ds);if(p.value==undefined||p.value==="")p.value="&#160;";if(r.dirty&&typeof r.modified[c.name]!=='undefined'){p.css+=p.css?' x-grid-dirty-cell':'x-grid-dirty-cell';}var Aa=ct.apply(p);if(!c.locked){cb+=Aa;}else {A4+=Aa;}}var alt=[];if(A1&&((a+1)%2==0)){alt[0]="x-grid-row-alt";}if(r.dirty){alt[1]=" x-grid-dirty-row";}
rp.cells=A4;if(this.getRowClass){alt[2]=this.getRowClass(r,a);}
rp.alt=alt.join(" ");A3+=rt.apply(rp);rp.cells=cb;A2+=rt.apply(rp);}return [A3,A2];}:function(cs,rs,ds,A5,A6,A7){var ts=this.templates,ct=ts.cell,rt=ts.row;var A8=[],A9=[],cb,BA,c,p={},rp={},r,a;for(var j=0,len=rs.length;j<len;j++){r=rs[j];cb=[];BA=[];a=(j+A5);for(var i=0;i<A6;i++){c=cs[i];p.cellId="x-grid-cell-"+a+"-"+i;p.id=c.id;p.css=p.attr="";p.value=c.renderer(r.data[c.name],p,r,a,i,ds);if(p.value==undefined||p.value==="")p.value="&#160;";if(r.dirty&&typeof r.modified[c.name]!=='undefined'){p.css+=p.css?' x-grid-dirty-cell':'x-grid-dirty-cell';}var Aa=ct.apply(p);if(!c.locked){cb[cb.length]=Aa;}else {BA[BA.length]=Aa;}}var alt=[];if(A7&&((a+1)%2==0)){alt[0]="x-grid-row-alt";}if(r.dirty){alt[1]=" x-grid-dirty-row";}
rp.cells=BA;if(this.getRowClass){alt[2]=this.getRowClass(r,a);}
rp.alt=alt.join(" ");rp.cells=BA.join("");A9[A9.length]=rt.apply(rp);rp.cells=cb.join("");A8[A8.length]=rt.apply(rp);}return [A9.join(""),A8.join("")];},renderBody:function(){var BB=this.renderRows();var bt=this.templates.body;return [bt.apply({rows:BB[0]}),bt.apply({rows:BB[1]})];},refresh:function(BC){this.fireEvent("beforerefresh",this);this.grid.stopEditing();var BD=this.renderBody();this.lockedBody.update(BD[0]);this.mainBody.update(BD[1]);if(BC===true){this.updateHeaders();this.updateColumns();this.updateSplitters();this.updateHeaderSortState();}
this.syncRowHeights();this.layout();this.fireEvent("refresh",this);},handleColumnMove:function(cm,BE,BF){this.indexMap=null;var s=this.getScrollState();this.refresh(true);this.restoreScroll(s);this.afterMove(BF);},afterMove:function(BG){if(this.enableMoveAnim&&Roo.enableFx){this.fly(this.getHeaderCell(BG).firstChild).highlight(this.hlColor);}},updateCell:function(dm,BH,BI){var BJ=this.getColumnIndexByDataIndex(BI);if(typeof BJ=="undefined"){return;}var cm=this.grid.colModel;var BK=this.getCell(BH,BJ);var BL=this.getCellText(BH,BJ);var p={cellId:"x-grid-cell-"+BH+"-"+BJ,id:cm.getColumnId(BJ),css:BJ==cm.getColumnCount()-1?"x-grid-col-last":""};var BM=cm.getRenderer(BJ);var BN=BM(dm.getValueAt(BH,BI),p,BH,BJ,dm);if(typeof BN=="undefined"||BN==="")BN="&#160;";BL.innerHTML=BN;BK.className=this.cellClass+" "+this.idToCssName(p.cellId)+" "+p.css;this.syncRowHeights(BH,BH);},calcColumnWidth:function(BO,BP){var BQ=0;if(this.grid.autoSizeHeaders){var h=this.getHeaderCellMeasure(BO);BQ=Math.max(BQ,h.scrollWidth);}var tb,BR;if(this.cm.isLocked(BO)){tb=this.getLockedTable();BR=BO;}else {tb=this.getBodyTable();BR=BO-this.cm.getLockedCount();}if(tb&&tb.rows){var Ar=tb.rows;var stopIndex=Math.min(BP||Ar.length,Ar.length);for(var i=0;i<stopIndex;i++){var BK=Ar[i].childNodes[BR].firstChild;BQ=Math.max(BQ,BK.scrollWidth);}}return BQ+5;},autoSizeColumn:function(BS,BT,BU){if(this.cm.isHidden(BS)){return;}if(BT){var AW=this.cm.getColumnId(BS);this.css.updateRule(this.colSelector+this.idToCssName(AW),"width",this.grid.minColumnWidth+"px");if(this.grid.autoSizeHeaders){this.css.updateRule(this.hdSelector+this.idToCssName(AW),"width",this.grid.minColumnWidth+"px");}}var BV=this.calcColumnWidth(BS);this.cm.setColumnWidth(BS,Math.max(this.grid.minColumnWidth,BV),BU);if(!BU){this.grid.fireEvent("columnresize",BS,BV);}},autoSizeColumns:function(){var cm=this.grid.colModel;var BW=cm.getColumnCount();for(var i=0;i<BW;i++){this.autoSizeColumn(i,true,true);}if(cm.getTotalWidth()<this.scroller.dom.clientWidth){this.fitColumns();}else {this.updateColumns();this.layout();}},fitColumns:function(BX){var cm=this.grid.colModel;var BY=cm.getColumnCount();var BZ=[];var Ba=0;var i,w;for(i=0;i<BY;i++){if(!cm.isHidden(i)&&!cm.isFixed(i)){w=cm.getColumnWidth(i);BZ.push(i);BZ.push(w);Ba+=w;}}var Bb=Math.min(this.scroller.dom.clientWidth,this.el.getWidth());if(BX){Bb-=17;}var Bc=(Bb-cm.getTotalWidth())/Ba;while(BZ.length){w=BZ.pop();i=BZ.pop();cm.setColumnWidth(i,Math.floor(w+w*Bc),true);}
this.updateColumns();this.layout();},onRowSelect:function(Bd){var Be=this.getRowComposite(Bd);Be.addClass("x-grid-row-selected");},onRowDeselect:function(Bf){var Bg=this.getRowComposite(Bf);Bg.removeClass("x-grid-row-selected");},onCellSelect:function(Bh,Bi){var Bj=this.getCell(Bh,Bi);if(Bj){Roo.fly(Bj).addClass("x-grid-cell-selected");}},onCellDeselect:function(Bk,Bl){var Bm=this.getCell(Bk,Bl);if(Bm){Roo.fly(Bm).removeClass("x-grid-cell-selected");}},updateHeaderSortState:function(){var Bn=this.ds.getSortState();if(!Bn){return;}
this.sortState=Bn;var Bo=this.cm.findColumnIndex(Bn.field);if(Bo!=-1){var Ao=Bn.direction;var sc=this.sortClasses;var hds=this.el.select(this.headerSelector).removeClass(sc);hds.item(Bo).addClass(sc[Ao=="DESC"?1:0]);}},handleHeaderClick:function(g,Bp){if(this.headersDisabled){return;}var dm=g.dataSource,cm=g.colModel;if(!cm.isSortable(Bp)){return;}
g.stopEditing();dm.sort(cm.getDataIndex(Bp));},destroy:function(){if(this.colMenu){this.colMenu.removeAll();Roo.menu.MenuMgr.unregister(this.colMenu);this.colMenu.getEl().remove();delete this.colMenu;}if(this.hmenu){this.hmenu.removeAll();Roo.menu.MenuMgr.unregister(this.hmenu);this.hmenu.getEl().remove();delete this.hmenu;}if(this.grid.enableColumnMove){var dds=Roo.dd.DDM.ids['gridHeader'+this.grid.getGridEl().id];if(dds){for(var dd in dds){if(!dds[dd].config.isTarget&&dds[dd].dragElId){var elid=dds[dd].dragElId;dds[dd].unreg();Roo.get(elid).remove();}else if(dds[dd].config.isTarget){dds[dd].proxyTop.remove();dds[dd].proxyBottom.remove();dds[dd].unreg();}if(Roo.dd.DDM.locationCache[dd]){delete Roo.dd.DDM.locationCache[dd];}}delete Roo.dd.DDM.ids['gridHeader'+this.grid.getGridEl().id];}}
Roo.util.CSS.removeStyleSheet(this.idToCssName(this.grid.id)+'-cssrules');this.bind(null,null);Roo.EventManager.removeResizeListener(this.onWindowResize,this);},handleLockChange:function(){this.refresh(true);},onDenyColumnLock:function(){},onDenyColumnHide:function(){},handleHdMenuClick:function(Bq){var Br=this.hdCtxIndex;var cm=this.cm,ds=this.ds;switch(Bq.id){case "asc":ds.sort(cm.getDataIndex(Br),"ASC");break;case "desc":ds.sort(cm.getDataIndex(Br),"DESC");break;case "lock":var lc=cm.getLockedCount();if(cm.getColumnCount(true)<=lc+1){this.onDenyColumnLock();return;}if(lc!=Br){cm.setLocked(Br,true,true);cm.moveColumn(Br,lc);this.grid.fireEvent("columnmove",Br,lc);}else {cm.setLocked(Br,true);}break;case "unlock":var lc=cm.getLockedCount();if((lc-1)!=Br){cm.setLocked(Br,false,true);cm.moveColumn(Br,lc-1);this.grid.fireEvent("columnmove",Br,lc-1);}else {cm.setLocked(Br,false);}break;default:Br=cm.getIndexById(Bq.id.substr(4));if(Br!=-1){if(Bq.checked&&cm.getColumnCount(true)<=1){this.onDenyColumnHide();return false;}
cm.setHidden(Br,Bq.checked);}}return true;},beforeColMenuShow:function(){var cm=this.cm,Bs=cm.getColumnCount();this.colMenu.removeAll();for(var i=0;i<Bs;i++){this.colMenu.add(new Roo.menu.CheckItem({id:"col-"+cm.getColumnId(i),text:cm.getColumnHeader(i),checked:!cm.isHidden(i),hideOnClick:false}));}},handleHdCtx:function(g,Bt,e){e.stopEvent();var hd=this.getHeaderCell(Bt);this.hdCtxIndex=Bt;var ms=this.hmenu.items,cm=this.cm;ms.get("asc").setDisabled(!cm.isSortable(Bt));ms.get("desc").setDisabled(!cm.isSortable(Bt));if(this.grid.enableColLock!==false){ms.get("lock").setDisabled(cm.isLocked(Bt));ms.get("unlock").setDisabled(!cm.isLocked(Bt));}
this.hmenu.show(hd,"tl-bl");},handleHdOver:function(e){var hd=this.findHeaderCell(e.getTarget());if(hd&&!this.headersDisabled){if(this.grid.colModel.isSortable(this.getCellIndex(hd))){this.fly(hd).addClass("x-grid-hd-over");}}},handleHdOut:function(e){var hd=this.findHeaderCell(e.getTarget());if(hd){this.fly(hd).removeClass("x-grid-hd-over");}},handleSplitDblClick:function(e,t){var i=this.getCellIndex(t);if(this.grid.enableColumnResize!==false&&this.cm.isResizable(i)&&!this.cm.isFixed(i)){this.autoSizeColumn(i,true);this.layout();}},render:function(){var cm=this.cm;var Bu=cm.getColumnCount();if(this.grid.monitorWindowResize===true){Roo.EventManager.onWindowResize(this.onWindowResize,this,true);}var Bv=this.renderHeaders();var Bw=this.templates.body.apply({rows:""});var Bx=this.templates.master.apply({lockedBody:Bw,body:Bw,lockedHeader:Bv[0],header:Bv[1]});this.grid.getGridEl().dom.innerHTML=Bx;this.initElements();this.scroller.on("scroll",this.handleScroll,this);this.lockedBody.on("mousewheel",this.handleWheel,this);this.mainBody.on("mousewheel",this.handleWheel,this);this.mainHd.on("mouseover",this.handleHdOver,this);this.mainHd.on("mouseout",this.handleHdOut,this);this.mainHd.on("dblclick",this.handleSplitDblClick,this,{delegate:"."+this.splitClass});this.lockedHd.on("mouseover",this.handleHdOver,this);this.lockedHd.on("mouseout",this.handleHdOut,this);this.lockedHd.on("dblclick",this.handleSplitDblClick,this,{delegate:"."+this.splitClass});if(this.grid.enableColumnResize!==false&&Roo.grid.SplitDragZone){new Roo.grid.SplitDragZone(this.grid,this.lockedHd.dom,this.mainHd.dom);}
...
this.updateHeaderSortState();this.beforeInitialResize();this.layout(true);this.renderPhase2.defer(1,this);},renderPhase2:function(){this.refresh();if(this.grid.autoSizeColumns){this.autoSizeColumns();}},beforeInitialResize:function(){},onColumnSplitterMoved:function(i,w){this.userResized=true;var cm=this.grid.colModel;cm.setColumnWidth(i,w,true);var By=cm.getColumnId(i);this.css.updateRule(this.colSelector+this.idToCssName(By),"width",(w-this.borderWidth)+"px");this.css.updateRule(this.hdSelector+this.idToCssName(By),"width",(w-this.borderWidth)+"px");this.updateSplitters();this.layout();this.grid.fireEvent("columnresize",i,w);},syncRowHeights:function(Bz,B0){if(this.grid.enableRowHeightSync===true&&this.cm.getLockedCount()>0){Bz=Bz||0;var mrows=this.getBodyTable().rows;var As=this.getLockedTable().rows;var len=mrows.length-1;B0=Math.min(B0||len,len);for(var i=Bz;i<=B0;i++){var m=mrows[i],l=As[i];var h=Math.max(m.offsetHeight,l.offsetHeight);m.style.height=l.style.height=h+"px";}}},layout:function(B1,B2){var g=this.grid;var B3=g.autoHeight;var B4=16;var c=g.getGridEl(),cm=this.cm,B5=g.autoExpandColumn,gv=this;if(!c.dom.offsetWidth){if(B1){this.lockedWrap.show();this.mainWrap.show();}return;}var B6=this.cm.isLocked(0);var B7=this.headerPanel.getHeight();var B8=this.footerPanel.getHeight();if(B3){var ch=this.getBodyTable().offsetHeight+B7+B8+this.mainHd.getHeight();var newHeight=ch+c.getBorderWidth("tb");if(g.maxHeight){newHeight=Math.min(g.maxHeight,newHeight);}
c.setHeight(newHeight);}if(g.autoWidth){c.setWidth(cm.getTotalWidth()+c.getBorderWidth('lr'));}var s=this.scroller;var B9=c.getSize(true);this.el.setSize(B9.width,B9.height);this.headerPanel.setWidth(B9.width);this.footerPanel.setWidth(B9.width);var CA=this.mainHd.getHeight();var vw=B9.width;var vh=B9.height-(B7+B8);s.setSize(vw,vh);var bt=this.getBodyTable();var CB=B6?Math.max(this.getLockedTable().offsetWidth,this.lockedHd.dom.firstChild.offsetWidth):0;var CC=bt.offsetHeight;var CD=CB+bt.offsetWidth;var CE=false,CF=false;this.scrollSizer.setSize(CD,CC+CA);var lw=this.lockedWrap,mw=this.mainWrap;var lb=this.lockedBody,mb=this.mainBody;setTimeout(function(){var t=s.dom.offsetTop;var w=s.dom.clientWidth,h=s.dom.clientHeight;lw.setTop(t);lw.setSize(CB,h);mw.setLeftTop(CB,t);mw.setSize(w-CB,h);lb.setHeight(h-CA);mb.setHeight(h-CA);if(B2!==true&&!gv.userResized&&B5){var ci=cm.getIndexById(B5);if(ci<0){ci=cm.findColumnIndex(B5);}
ci=Math.max(0,ci);var expandId=cm.getColumnId(ci);var tw=cm.getTotalWidth(false);var currentWidth=cm.getColumnWidth(ci);var cw=Math.min(Math.max(((w-tw)+currentWidth-2)-(w<=s.dom.offsetWidth?0:18),g.autoExpandMin),g.autoExpandMax);if(currentWidth!=cw){cm.setColumnWidth(ci,cw,true);gv.css.updateRule(gv.colSelector+gv.idToCssName(expandId),"width",(cw-gv.borderWidth)+"px");gv.css.updateRule(gv.hdSelector+gv.idToCssName(expandId),"width",(cw-gv.borderWidth)+"px");gv.updateSplitters();gv.layout(false,true);}}if(B1){lw.show();mw.show();}},10);},onWindowResize:function(){if(!this.grid.monitorWindowResize||this.grid.autoHeight){return;}
this.layout();},appendFooter:function(CG){return null;},sortAscText:"Sort Ascending",sortDescText:"Sort Descending",lockText:"Lock Column",unlockText:"Unlock Column",columnsText:"Columns"});Roo.grid.GridView.ColumnDragZone=function(CH,hd){Roo.grid.GridView.ColumnDragZone.superclass.constructor.call(this,CH,hd,null);this.proxy.el.addClass('x-grid3-col-dd');};Roo.extend(Roo.grid.GridView.ColumnDragZone,Roo.grid.HeaderDragZone,{handleMouseDown:function(e){},callHandleMouseDown:function(e){Roo.grid.GridView.ColumnDragZone.superclass.handleMouseDown.call(this,e);}});
Roo.grid.SplitDragZone=function(A,hd,B){this.grid=A;this.view=A.getView();this.proxy=this.view.resizeProxy;Roo.grid.SplitDragZone.superclass.constructor.call(this,hd,"gridSplitters"+this.grid.getGridEl().id,{dragElId:Roo.id(this.proxy.dom),resizeFrame:false});this.setHandleElId(Roo.id(hd));this.setOuterHandleElId(Roo.id(B));this.scroll=false;};Roo.extend(Roo.grid.SplitDragZone,Roo.dd.DDProxy,{fly:Roo.Element.fly,b4StartDrag:function(x,y){this.view.headersDisabled=true;this.proxy.setHeight(this.view.mainWrap.getHeight());var w=this.cm.getColumnWidth(this.cellIndex);var C=Math.max(w-this.grid.minColumnWidth,0);this.resetConstraints();this.setXConstraint(C,1000);this.setYConstraint(0,0);this.minX=x-C;this.maxX=x+1000;this.startPos=x;Roo.dd.DDProxy.prototype.b4StartDrag.call(this,x,y);},handleMouseDown:function(e){ev=Roo.EventObject.setEvent(e);var t=this.fly(ev.getTarget());if(t.hasClass("x-grid-split")){this.cellIndex=this.view.getCellIndex(t.dom);this.split=t.dom;this.cm=this.grid.colModel;if(this.cm.isResizable(this.cellIndex)&&!this.cm.isFixed(this.cellIndex)){Roo.grid.SplitDragZone.superclass.handleMouseDown.apply(this,arguments);}}},endDrag:function(e){this.view.headersDisabled=false;var D=Math.max(this.minX,Roo.lib.Event.getPageX(e));var E=D-this.startPos;this.view.onColumnSplitterMoved(this.cellIndex,this.cm.getColumnWidth(this.cellIndex)+E);},autoOffset:function(){this.setDelta(0,0);}});
...
this.scroll=false;this.grid=A;this.ddel=document.createElement('div');this.ddel.className='x-grid-dd-wrap';};Roo.extend(Roo.grid.GridDragZone,Roo.dd.DragZone,{ddGroup:"GridDD",getDragData:function(e){var t=Roo.lib.Event.getTarget(e);var C=this.view.findRowIndex(t);if(C!==false){var sm=this.grid.selModel;if(e.hasModifier()){sm.handleMouseDown(e,t);}return {grid:this.grid,ddel:this.ddel,rowIndex:C,selections:sm.getSelections()};}return false;},onInitDrag:function(e){var D=this.dragData;this.ddel.innerHTML=this.grid.getDragDropText();this.proxy.update(this.ddel);},afterRepair:function(){this.dragging=false;},getRepairXY:function(e,E){return false;},onEndDrag:function(F,e){},onValidDrop:function(dd,e,id){this.hideProxy();},beforeInvalidDrop:function(e,id){}});
Roo.grid.ColumnModel=function(A){this.config=A;this.lookup={};for(var i=0,len=A.length;i<len;i++){var c=A[i];if(typeof c.dataIndex=="undefined"){c.dataIndex=i;}if(typeof c.renderer=="string"){c.renderer=Roo.util.Format[c.renderer];}if(typeof c.id=="undefined"){c.id=Roo.id();}if(c.editor&&c.editor.xtype){c.editor=Roo.factory(c.editor,Roo.grid);}if(c.editor&&c.editor.isFormField){c.editor=new Roo.grid.GridEditor(c.editor);}
this.lookup[c.id]=c;}
this.defaultWidth=100;this.defaultSortable=false;this.addEvents({"widthchange":true,"headerchange":true,"hiddenchange":true,"columnmoved":true,"columnlockchange":true});Roo.grid.ColumnModel.superclass.constructor.call(this);};Roo.extend(Roo.grid.ColumnModel,Roo.util.Observable,{getColumnId:function(B){return this.config[B].id;},getColumnById:function(id){return this.lookup[id];},getIndexById:function(id){for(var i=0,len=this.config.length;i<len;i++){if(this.config[i].id==id){return i;}}return -1;},findColumnIndex:function(C){for(var i=0,len=this.config.length;i<len;i++){if(this.config[i].dataIndex==C){return i;}}return -1;},moveColumn:function(D,E){var c=this.config[D];this.config.splice(D,1);this.config.splice(E,0,c);this.dataMap=null;this.fireEvent("columnmoved",this,D,E);},isLocked:function(F){return this.config[F].locked===true;},setLocked:function(G,H,I){if(this.isLocked(G)==H){return;}
this.config[G].locked=H;if(!I){this.fireEvent("columnlockchange",this,G,H);}},getTotalLockedWidth:function(){var J=0;for(var i=0;i<this.config.length;i++){if(this.isLocked(i)&&!this.isHidden(i)){this.totalWidth+=this.getColumnWidth(i);}}return J;},getLockedCount:function(){for(var i=0,len=this.config.length;i<len;i++){if(!this.isLocked(i)){return i;}}},getColumnCount:function(K){if(K===true){var c=0;for(var i=0,len=this.config.length;i<len;i++){if(!this.isHidden(i)){c++;}}return c;}return this.config.length;},getColumnsBy:function(fn,L){var r=[];for(var i=0,len=this.config.length;i<len;i++){var c=this.config[i];if(fn.call(L||this,c,i)===true){r[r.length]=c;}}return r;},isSortable:function(M){if(typeof this.config[M].sortable=="undefined"){return this.defaultSortable;}return this.config[M].sortable;},getRenderer:function(N){if(!this.config[N].renderer){return Roo.grid.ColumnModel.defaultRenderer;}return this.config[N].renderer;},setRenderer:function(O,fn){this.config[O].renderer=fn;},getColumnWidth:function(P){return this.config[P].width||this.defaultWidth;},setColumnWidth:function(Q,R,S){this.config[Q].width=R;this.totalWidth=null;if(!S){this.fireEvent("widthchange",this,Q,R);}},getTotalWidth:function(T){if(!this.totalWidth){this.totalWidth=0;for(var i=0,len=this.config.length;i<len;i++){if(T||!this.isHidden(i)){this.totalWidth+=this.getColumnWidth(i);}}}return this.totalWidth;},getColumnHeader:function(U){return this.config[U].header;},setColumnHeader:function(V,W){this.config[V].header=W;this.fireEvent("headerchange",this,V,W);},getColumnTooltip:function(X){return this.config[X].tooltip;},setColumnTooltip:function(Y,Z){this.config[Y].tooltip=Z;},getDataIndex:function(a){return this.config[a].dataIndex;},setDataIndex:function(b,d){this.config[b].dataIndex=d;},isCellEditable:function(e,f){return (this.config[e].editable||(typeof this.config[e].editable=="undefined"&&this.config[e].editor))?true:false;},getCellEditor:function(g,h){return this.config[g].editor;},setEditable:function(j,k){this.config[j].editable=k;},isHidden:function(l){return this.config[l].hidden;},isFixed:function(m){return this.config[m].fixed;},isResizable:function(n){return n>=0&&this.config[n].resizable!==false&&this.config[n].fixed!==true;},setHidden:function(o,p){this.config[o].hidden=p;this.totalWidth=null;this.fireEvent("hiddenchange",this,o,p);},setEditor:function(q,s){this.config[q].editor=s;}});Roo.grid.ColumnModel.defaultRenderer=function(t){if(typeof t=="string"&&t.length<1){return "&#160;";}return t;};Roo.grid.DefaultColumnModel=Roo.grid.ColumnModel;
...
this.rowNav=new Roo.KeyNav(this.grid.getGridEl(),{"up":function(e){if(!e.shiftKey){this.selectPrevious(e.shiftKey);}else if(this.last!==false&&this.lastActive!==false){var last=this.last;this.selectRange(this.last,this.lastActive-1);this.grid.getView().focusRow(this.lastActive);if(last!==false){this.last=last;}}else {this.selectFirstRow();}
this.fireEvent("afterselectionchange",this);},"down":function(e){if(!e.shiftKey){this.selectNext(e.shiftKey);}else if(this.last!==false&&this.lastActive!==false){var last=this.last;this.selectRange(this.last,this.lastActive+1);this.grid.getView().focusRow(this.lastActive);if(last!==false){this.last=last;}}else {this.selectFirstRow();}
this.fireEvent("afterselectionchange",this);},scope:this});var B=this.grid.view;B.on("refresh",this.onRefresh,this);B.on("rowupdated",this.onRowUpdated,this);B.on("rowremoved",this.onRemove,this);},onRefresh:function(){var ds=this.grid.dataSource,i,v=this.grid.view;var s=this.selections;s.each(function(r){if((i=ds.indexOfId(r.id))!=-1){v.onRowSelect(i);}else {s.remove(r);}});},onRemove:function(v,C,r){this.selections.remove(r);},onRowUpdated:function(v,D,r){if(this.isSelected(r)){v.onRowSelect(D);}},selectRecords:function(E,F){if(!F){this.clearSelections();}var ds=this.grid.dataSource;for(var i=0,len=E.length;i<len;i++){this.selectRow(ds.indexOf(E[i]),true);}},getCount:function(){return this.selections.length;},selectFirstRow:function(){this.selectRow(0);},selectLastRow:function(G){this.selectRow(this.grid.dataSource.getCount()-1,G);},selectNext:function(H){if(this.last!==false&&(this.last+1)<this.grid.dataSource.getCount()){this.selectRow(this.last+1,H);this.grid.getView().focusRow(this.last);}},selectPrevious:function(I){if(this.last){this.selectRow(this.last-1,I);this.grid.getView().focusRow(this.last);}},getSelections:function(){return [].concat(this.selections.items);},getSelected:function(){return this.selections.itemAt(0);},clearSelections:function(J){if(this.locked)return;if(J!==true){var ds=this.grid.dataSource;var s=this.selections;s.each(function(r){this.deselectRow(ds.indexOfId(r.id));},this);s.clear();}else {this.selections.clear();}
this.last=false;},selectAll:function(){if(this.locked)return;this.selections.clear();for(var i=0,len=this.grid.dataSource.getCount();i<len;i++){this.selectRow(i,true);}},hasSelection:function(){return this.selections.length>0;},isSelected:function(K){var r=typeof K=="number"?this.grid.dataSource.getAt(K):K;return (r&&this.selections.key(r.id)?true:false);},isIdSelected:function(id){return (this.selections.key(id)?true:false);},handleMouseDown:function(e,t){var L=this.grid.getView(),M;if(this.isLocked()||(M=L.findRowIndex(t))===false){return;};if(e.shiftKey&&this.last!==false){var last=this.last;this.selectRange(last,M,e.ctrlKey);this.last=last;L.focusRow(M);}else {var isSelected=this.isSelected(M);if(e.button!==0&&isSelected){L.focusRow(M);}else if(e.ctrlKey&&isSelected){this.deselectRow(M);}else if(!isSelected){this.selectRow(M,e.button===0&&(e.ctrlKey||e.shiftKey));L.focusRow(M);}}
this.fireEvent("afterselectionchange",this);},handleDragableRowClick:function(N,O,e){if(e.button===0&&!e.shiftKey&&!e.ctrlKey){this.selectRow(O,false);N.view.focusRow(O);this.fireEvent("afterselectionchange",this);}},selectRows:function(P,Q){if(!Q){this.clearSelections();}for(var i=0,len=P.length;i<len;i++){this.selectRow(P[i],true);}},selectRange:function(R,S,T){if(this.locked)return;if(!T){this.clearSelections();}if(R<=S){for(var i=R;i<=S;i++){this.selectRow(i,true);}}else {for(var i=R;i>=S;i--){this.selectRow(i,true);}}},deselectRange:function(U,V,W){if(this.locked)return;for(var i=U;i<=V;i++){this.deselectRow(i,W);}},selectRow:function(X,Y,Z){if(this.locked||(X<0||X>=this.grid.dataSource.getCount()))return;if(this.fireEvent("beforerowselect",this,X,Y)!==false){if(!Y||this.singleSelect){this.clearSelections();}var r=this.grid.dataSource.getAt(X);this.selections.add(r);this.last=this.lastActive=X;if(!Z){this.grid.getView().onRowSelect(X);}
this.fireEvent("rowselect",this,X,r);this.fireEvent("selectionchange",this);}},deselectRow:function(a,b){if(this.locked)return;if(this.last==a){this.last=false;}if(this.lastActive==a){this.lastActive=false;}var r=this.grid.dataSource.getAt(a);this.selections.remove(r);if(!b){this.grid.getView().onRowDeselect(a);}
this.fireEvent("rowdeselect",this,a);this.fireEvent("selectionchange",this);},restoreLast:function(){if(this._last){this.last=this._last;}},acceptsNav:function(c,d,cm){return !cm.isHidden(d)&&cm.isCellEditable(d,c);},onEditorKey:function(f,e){var k=e.getKey(),h,g=this.grid,ed=g.activeEditor;if(k==e.TAB){e.stopEvent();ed.completeEdit();if(e.shiftKey){h=g.walkCells(ed.row,ed.col-1,-1,this.acceptsNav,this);}else {h=g.walkCells(ed.row,ed.col+1,1,this.acceptsNav,this);}}else if(k==e.ENTER&&!e.ctrlKey){e.stopEvent();ed.completeEdit();if(e.shiftKey){h=g.walkCells(ed.row-1,ed.col,-1,this.acceptsNav,this);}else {h=g.walkCells(ed.row+1,ed.col,1,this.acceptsNav,this);}}else if(k==e.ESC){ed.cancelEdit();}if(h){g.startEditing(h[0],h[1]);}}});
Roo.grid.CellSelectionModel=function(A){Roo.apply(this,A);this.selection=null;this.addEvents({"beforecellselect":true,"cellselect":true,"selectionchange":true});Roo.grid.CellSelectionModel.superclass.constructor.call(this);};Roo.extend(Roo.grid.CellSelectionModel,Roo.grid.AbstractSelectionModel,{initEvents:function(){this.grid.on("mousedown",this.handleMouseDown,this);this.grid.getGridEl().on(Roo.isIE?"keydown":"keypress",this.handleKeyDown,this);var B=this.grid.view;B.on("refresh",this.onViewChange,this);B.on("rowupdated",this.onRowUpdated,this);B.on("beforerowremoved",this.clearSelections,this);B.on("beforerowsinserted",this.clearSelections,this);if(this.grid.isEditor){this.grid.on("beforeedit",this.beforeEdit,this);}},beforeEdit:function(e){this.select(e.row,e.column,false,true,e.record);},onRowUpdated:function(v,C,r){if(this.selection&&this.selection.record==r){v.onCellSelect(C,this.selection.cell[1]);}},onViewChange:function(){this.clearSelections(true);},getSelectedCell:function(){return this.selection?this.selection.cell:null;},clearSelections:function(D){var s=this.selection;if(s){if(D!==true){this.grid.view.onCellDeselect(s.cell[0],s.cell[1]);}
this.selection=null;this.fireEvent("selectionchange",this,null);}},hasSelection:function(){return this.selection?true:false;},handleMouseDown:function(e,t){var v=this.grid.getView();if(this.isLocked()){return;};var E=v.findRowIndex(t);var F=v.findCellIndex(t);if(E!==false&&F!==false){this.select(E,F);}},select:function(G,H,I,J,r){if(this.fireEvent("beforecellselect",this,G,H)!==false){this.clearSelections();r=r||this.grid.dataSource.getAt(G);this.selection={record:r,cell:[G,H]};if(!I){var v=this.grid.getView();v.onCellSelect(G,H);if(J!==true){v.focusCell(G,H);}}
this.fireEvent("cellselect",this,G,H);this.fireEvent("selectionchange",this,this.selection);}},isSelectable:function(K,L,cm){return !cm.isHidden(L);},handleKeyDown:function(e){if(!e.isNavKeyPress()){return;}var g=this.grid,s=this.selection;if(!s){e.stopEvent();var F=g.walkCells(0,0,1,this.isSelectable,this);if(F){this.select(F[0],F[1]);}return;}var sm=this;var M=function(O,P,Q){return g.walkCells(O,P,Q,sm.isSelectable,sm);};var k=e.getKey(),r=s.cell[0],c=s.cell[1];var N;switch(k){case e.TAB:if(e.shiftKey){N=M(r,c-1,-1);}else {N=M(r,c+1,1);}break;case e.DOWN:N=M(r+1,c,1);break;case e.UP:N=M(r-1,c,-1);break;case e.RIGHT:N=M(r,c+1,1);break;case e.LEFT:N=M(r,c-1,-1);break;case e.ENTER:if(g.isEditor&&!g.editing){g.startEditing(r,c);e.stopEvent();return;}break;};if(N){this.select(N[0],N[1]);e.stopEvent();}},acceptsNav:function(O,P,cm){return !cm.isHidden(P)&&cm.isCellEditable(P,O);},onEditorKey:function(Q,e){var k=e.getKey(),R,g=this.grid,ed=g.activeEditor;if(k==e.TAB){if(e.shiftKey){R=g.walkCells(ed.row,ed.col-1,-1,this.acceptsNav,this);}else {R=g.walkCells(ed.row,ed.col+1,1,this.acceptsNav,this);}
...
Roo.grid.PropertyStore.superclass.constructor.call(this);};Roo.extend(Roo.grid.PropertyStore,Roo.util.Observable,{setSource:function(o){this.source=o;this.store.removeAll();var C=[];for(var k in o){if(this.isEditableValue(o[k])){C.push(new Roo.grid.PropertyRecord({name:k,value:o[k]},k));}}
this.store.loadRecords({records:C},{},true);},onUpdate:function(ds,D,E){if(E==Roo.data.Record.EDIT){var v=D.data['value'];var oldValue=D.modified['value'];if(this.grid.fireEvent('beforepropertychange',this.source,D.id,v,oldValue)!==false){this.source[D.id]=v;D.commit();this.grid.fireEvent('propertychange',this.source,D.id,v,oldValue);}else {D.reject();}}},getProperty:function(F){return this.store.getAt(F);},isEditableValue:function(G){if(G&&G instanceof Date){return true;}else if(typeof G=='object'||typeof G=='function'){return false;}return true;},setValue:function(H,I){this.source[H]=I;this.store.getById(H).set('value',I);},getSource:function(){return this.source;}});Roo.grid.PropertyColumnModel=function(J,K){this.grid=J;var g=Roo.grid;g.PropertyColumnModel.superclass.constructor.call(this,[{header:this.nameText,sortable:true,dataIndex:'name',id:'name'},{header:this.valueText,resizable:false,dataIndex:'value',id:'value'}]);this.store=K;this.bselect=Roo.DomHelper.append(document.body,{tag:'select',style:'display:none',cls:'x-grid-editor',children:[{tag:'option',value:'true',html:'true'},{tag:'option',value:'false',html:'false'}]});Roo.id(this.bselect);var f=Roo.form;this.editors={'date':new g.GridEditor(new f.DateField({selectOnFocus:true})),'string':new g.GridEditor(new f.TextField({selectOnFocus:true})),'number':new g.GridEditor(new f.NumberField({selectOnFocus:true,style:'text-align:left;'})),'int':new g.GridEditor(new f.NumberField({selectOnFocus:true,allowDecimals:false,style:'text-align:left;'})),'boolean':new g.GridEditor(new f.Field({el:this.bselect,selectOnFocus:true}))};this.renderCellDelegate=this.renderCell.createDelegate(this);this.renderPropDelegate=this.renderProp.createDelegate(this);};Roo.extend(Roo.grid.PropertyColumnModel,Roo.grid.ColumnModel,{nameText:'Name',valueText:'Value',dateFormat:'m/j/Y',renderDate:function(L){return L.dateFormat(this.dateFormat);},renderBool:function(M){return M?'true':'false';},isCellEditable:function(N,O){return N==1;},getRenderer:function(P){return P==1?this.renderCellDelegate:this.renderPropDelegate;},renderProp:function(v){return this.getPropertyName(v);},renderCell:function(Q){var rv=Q;if(Q instanceof Date){rv=this.renderDate(Q);}else if(typeof Q=='boolean'){rv=this.renderBool(Q);}return Roo.util.Format.htmlEncode(rv);},getPropertyName:function(R){var pn=this.grid.propertyNames;return pn&&pn[R]?pn[R]:R;},getCellEditor:function(S,T){var p=this.store.getProperty(T);var n=p.data['name'],U=p.data['value'];if(typeof(this.grid.customEditors[n])=='string'){return this.editors[this.grid.customEditors[n]];}if(typeof(this.grid.customEditors[n])!='undefined'){return this.grid.customEditors[n];}if(U instanceof Date){return this.editors['date'];}else if(typeof U=='number'){return this.editors['number'];}else if(typeof U=='boolean'){return this.editors['boolean'];}else {return this.editors['string'];}}});Roo.grid.PropertyGrid=function(V,W){W=W||{};var X=new Roo.grid.PropertyStore(this);this.store=X;var cm=new Roo.grid.PropertyColumnModel(this,X);X.store.sort('name','ASC');Roo.grid.PropertyGrid.superclass.constructor.call(this,V,Roo.apply({ds:X.store,cm:cm,enableColLock:false,enableColumnMove:false,stripeRows:false,trackMouseOver:false,clicksToEdit:1},W));this.getGridEl().addClass('x-props-grid');this.lastEditRow=null;this.on('columnresize',this.onColumnResize,this);this.addEvents({"beforepropertychange":true,"propertychange":true});this.customEditors=this.customEditors||{};};Roo.extend(Roo.grid.PropertyGrid,Roo.grid.EditorGrid,{render:function(){Roo.grid.PropertyGrid.superclass.render.call(this);this.autoSize.defer(100,this);},autoSize:function(){Roo.grid.PropertyGrid.superclass.autoSize.call(this);if(this.view){this.view.fitColumns();}},onColumnResize:function(){this.colModel.setColumnWidth(1,this.container.getWidth(true)-this.colModel.getColumnWidth(0));this.autoSize();},setSource:function(Y){this.store.setSource(Y);},getSource:function(){return this.store.getSource();}});
