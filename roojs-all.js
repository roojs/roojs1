...
Roo.EventManager.on(B,'load',cb,this);C.submit();if(D){for(var i=0,E=D.length;i<E;i++){C.removeChild(D[i]);}}}});Roo.Ajax=new Roo.data.Connection({autoAbort:false,serializeForm:function(A){return Roo.lib.Ajax.serializeForm(A);}});
...
H.push(n+' { '+I.join(' ')+'} ');}
E=H.join("\n");}if(Roo.isIE){F.appendChild(G);ss=G.styleSheet;ss.cssText=E;}else {try{G.appendChild(B.createTextNode(E));}catch(e){nrules.cssText=cssText;}
...
Roo.extend(Roo.dd.DragDrop, Roo.util.Observable , {
    ...
    setPadding: function(C, D, E, F) {
        
        if (!D && 0 !== D) {
            this.padding = [C, C, C, C];
        } else  if (!E && 0 !== E) {
            this.padding = [C, D, C, D];
        } else  {
            this.padding = [C, D, E, F];
        }
    },
    ...
    clickValidator: function(e) {
        var  C = e.getTarget();
        return  ( this.isValidHandleChild(C) &&
    ...
    removeInvalidHandleClass: function(C) {
        for (var  i=0, D=this.invalidHandleClasses.length; i<D; ++i) {
            if (this.invalidHandleClasses[i] == C) {
    ...
    isValidHandleChild: function(C) {
        ...
        D = D && !this.invalidHandleTypes[E];
        D = D && !this.invalidHandleIds[C.id];

        for (var  i=0, F=this.invalidHandleClasses.length; D && i<F; ++i) {
            D = !B.hasClass(C, this.invalidHandleClasses[i]);
        }
    ...
    setXTicks: function(C, D) {
        this.xTicks = [];
        this.xTickSize = D;

        var  E = {};

        for (var  i = this.initPageX; i >= this.minX; i = i - D) {
            if (!E[i]) {
        ...
        for (i = this.initPageX; i <= this.maxX; i = i + D) {
            if (!E[i]) {
                this.xTicks[this.xTicks.length] = i;
                E[i] = true;
            }
    ...
    setYTicks: function(C, D) {
        this.yTicks = [];
        this.yTickSize = D;

        var  E = {};

        for (var  i = this.initPageY; i >= this.minY; i = i - D) {
            if (!E[i]) {
        ...
        for (i = this.initPageY; i <= this.maxY; i = i + D) {
            if (!E[i]) {
                this.yTicks[this.yTicks.length] = i;
                E[i] = true;
            }
...
Roo.dd.DragDropMgr = function() {
    ...
    return  {
        ...
        removeDDFromGroup: function(B, C) {
            if (!this.ids[C]) {
                this.ids[C] = {};
            }
        ...
        _remove: function(B) {
            for (var  g  in  B.groups) {
                if (g && this.ids[g][B.id]) {
        ...
        getRelated: function(B, C) {
            var  D = [];
            for (var  i  in  B.groups) {
                for (j  in  this.ids[i]) {
                    var  dd = this.ids[i][j];
        ...
        handleMouseMove: function(e) {
            ...
            if (!this.dragThreshMet) {
                var  B = Math.abs(this.startX - e.getPageX());
                var  C = Math.abs(this.startY - e.getPageY());
                if (B > this.clickPixelThresh ||
        ...
        fireEvents: function(e, B) {
            ...
            for (var  i  in  this.dragOvers) {
                ...
                var  H = this.dragOvers[i];

                if (! this.isTypeOfDD(H)) {
                    continue;
                }

                if (! this.isOverTarget(pt, H, this.mode)) {
                    D.push( H );
                }
            ...
            for (var  I  in  dc.groups) {
                ...
                for (i  in  this.ids[I]) {
                    ...
                    if (J.isTarget && !J.isLocked() && J != dc) {
                        if (this.isOverTarget(pt, J, this.mode)) {
            ...
            if (this.mode) {
                ...
                for (i=0, K=D.length; i<K; ++i) {
                    dc.b4DragOut(e, D[i].id);
                    dc.onDragOut(e, D[i].id);
                }

                
                for (i=0,K=G.length; i<K; ++i) {
                    
                    dc.onDragEnter(e, G[i].id);
                }

                
                for (i=0,K=E.length; i<K; ++i) {
                    dc.b4DragOver(e, E[i].id);
                    dc.onDragOver(e, E[i].id);
                }

                
                for (i=0, K=F.length; i<K; ++i) {
                    dc.b4DragDrop(e, F[i].id);
                    dc.onDragDrop(e, F[i].id);
                }
            ...
            if (D == 1) {
                ...
                for (var  i=0; i<D; ++i) {
                    ...
                    if (dd.cursorIsOver) {
                        ...
                        if (!C ||
                            C.overlap.getArea() < dd.overlap.getArea()) {
                            C = dd;
                        }
                    }
                }
        ...
        getLocation: function(B) {
            ...
            var  el = B.getEl(), C, x1, x2, y1, y2, t, r, b, l;

            try {
                C= Roo.lib.Dom.getXY(el);
            } catch (e) { }
        ...
        isOverTarget: function(pt, B, C) {
            
            var  D = this.locationCache[B.id];
            if (!D || !this.useCache) {
                D = this.getLocation(B);
                this.locationCache[B.id] = D;
            ...
            var  E = dc.getTargetCoord(pt.x, pt.y);

            var  el = dc.getDragEl();
            var  F = new  Roo.lib.Region( E.y,
                                                   E.x + el.offsetWidth,
                                                   E.y + el.offsetHeight,
                                                   E.x );
        ...
        getElWrapper: function(id) {
            var  B = this.elementCache[id];
            if (!B || !B.el) {
                B = this.elementCache[id] =
        ...
        getScroll: function () {
            var  t, l, B=document.documentElement, db=document.body;
        ...
        getStyle: function(el, B) {
            return  Roo.fly(el).getStyle(B);
        },
        ...
        moveToEl: function (B, C) {
            var  D = Roo.lib.Dom.getXY(C);
            Roo.lib.Dom.setXY(B, D);
        },
        ...
        handleWasClicked: function(B, id) {
            if (this.isHandle(id, B.id)) {
                return  true;
            } else  {
...
Roo.extend(Roo.dd.DD, Roo.dd.DragDrop, {
    ...
    autoOffset: function(A, B) {
        var  x = A - this.startPageX;
        var  y = B - this.startPageY;
        this.setDelta(x, y);
    },

    

    setDelta: function(A, B) {
        this.deltaX = A;
        this.deltaY = B;
    },
    ...
    setDragElPos: function(A, B) {
        
        

        var  el = this.getDragEl();
        this.alignElWithMouse(el, A, B);
    },

    

    alignElWithMouse: function(el, A, B) {
        var  C = this.getTargetCoord(A, B);
        var  D = el.dom ? el : Roo.fly(el);
        if (!this.deltaSetXY) {
            var  E = [C.x, C.y];
            D.setXY(E);
            var  F = D.getLeft(true);
            var  G  = D.getTop(true);
            this.deltaSetXY = [ F - C.x, G - C.y ];
        } else  {
            D.setLeftTop(C.x + this.deltaSetXY[0], C.y + this.deltaSetXY[1]);
        }


        this.cachePosition(C.x, C.y);
        this.autoScroll(C.x, C.y, el.offsetHeight, el.offsetWidth);
        return  C;
    },

    

    cachePosition: function(A, B) {
        if (A) {
            this.lastPageX = A;
            this.lastPageY = B;
        } else  {
            var  C = Roo.lib.Dom.getXY(this.getEl());
            this.lastPageX = C[0];
            this.lastPageY = C[1];
        }
    },
    ...
    autoScroll: function(x, y, h, w) {
        ...
        if (this.scroll) {
            ...
            var  A = Roo.lib.Dom.getViewWidth();

            
            var  B = Roo.lib.Dom.getViewHeight();
            ...
            var  E = (A + st - y - this.deltaY);

            
            var  F = (B + sl - x - this.deltaX);
            ...
            var  H = (document.all) ? 80 : 30;

            
            
            if ( C > A && E < G ) {
                window.scrollTo(sl, st + H);
            }
...
Roo.extend(Roo.dd.DDProxy, Roo.dd.DD, {
    ...
    createFrame: function() {
        ...
        if (!B || !B.firstChild) {
            setTimeout( function() { A.createFrame(); }, 50 );
            return;
        }
...
Roo.dd.ScrollManager = function(){
    ...
    var  G = function(){
        if(A.dragCurrent){
            var  K = Roo.dd.ScrollManager;
            if(!K.animate){
                if(D.el.scroll(D.dir, K.increment)){
                    F();
                }
            }else {
                D.el.scroll(D.dir, K.increment, true, K.animDuration, F);
            }
        }
    };
...
Roo.extend(Roo.dd.DragSource, Roo.dd.DDProxy, {
    ...
    onDragEnter : function(e, id){
        var  A = Roo.dd.DragDropMgr.getDDById(id);
        this.cachedTarget = A;
        if(this.beforeDragEnter(A, e, id) !== false){
            if(A.isNotifyTarget){
                var  B = A.notifyEnter(this, e, this.dragData);
    ...
    beforeDragEnter : function(A, e, id){
        return  true;
    },
    ...
    onDragOver : function(e, id){
        var  A = this.cachedTarget || Roo.dd.DragDropMgr.getDDById(id);
        if(this.beforeDragOver(A, e, id) !== false){
            if(A.isNotifyTarget){
                var  B = A.notifyOver(this, e, this.dragData);
                this.proxy.setStatus(B);
            }
    ...
    beforeDragOver : function(A, e, id){
        return  true;
    },
    ...
    onDragOut : function(e, id){
        var  A = this.cachedTarget || Roo.dd.DragDropMgr.getDDById(id);
        if(this.beforeDragOut(A, e, id) !== false){
            if(A.isNotifyTarget){
                A.notifyOut(this, e, this.dragData);
            }
    ...
    onDragDrop : function(e, id){
        var  A = this.cachedTarget || Roo.dd.DragDropMgr.getDDById(id);
        if(this.beforeDragDrop(A, e, id) !== false){
            if(A.isNotifyTarget){
                if(A.notifyDrop(this, e, this.dragData)){ 
                    this.onValidDrop(A, e, id);
                }else {
                    this.onInvalidDrop(A, e, id);
                }
            }else {
                this.onValidDrop(A, e, id);
            }
    ...
    onValidDrop : function(A, e, id){
        this.hideProxy();
        if(this.afterValidDrop){
            

            this.afterValidDrop(A, e, id);
        }
    },

    
    getRepairXY : function(e, A){
        return  this.el.getXY();  
    },

    
    onInvalidDrop : function(A, e, id){
        this.beforeInvalidDrop(A, e, id);
    ...
    beforeInvalidDrop : function(A, e, id){
        return  true;
    },
    ...
    handleMouseDown : function(e){
        ...
        var  A = this.getDragData(e);
        if(A && this.onBeforeDrag(A, e) !== false){
            this.dragData = A;
            this.proxy.stop();
    ...
    onInitDrag : function(x, y){
        var  A = this.el.dom.cloneNode(true);
        A.id = Roo.id(); 
        this.proxy.update(A);
...
Roo.extend(Roo.dd.DropTarget, Roo.dd.DDTarget, {
    ...
    notifyEnter : function(dd, e, A)
    {
        this.valid = true;
        this.fireEvent('enter', dd, e, A);
        if(this.overClass){
    ...
    notifyOver : function(dd, e, A)
    {
        this.valid = true;
        this.fireEvent('over', dd, e, A);
        return  typeof(this.valid) == 'string' ? 'x-dd-drop-' + this.valid : (
            this.valid ? this.dropAllowed : this.dropNotAllowed
        );
    },
    ...
    notifyOut : function(dd, e, A)
    {
        this.fireEvent('out', dd, e, A);
        if(this.overClass){
    ...
    notifyDrop : function(dd, e, A)
    {
        this.success = false;
        this.fireEvent('drop', dd, e, A);
        return  this.success;
    }
});
...
Roo.extend(Roo.dd.DropZone, Roo.dd.DropTarget, {
    ...
    onNodeEnter : function(n, dd, e, A){
        
    },

    

    onNodeOver : function(n, dd, e, A){
        return  this.dropAllowed;
    },

    

    onNodeOut : function(n, dd, e, A){
        
    },

    

    onNodeDrop : function(n, dd, e, A){
        return  false;
    },

    

    onContainerOver : function(dd, e, A){
        return  this.dropNotAllowed;
    },

    

    onContainerDrop : function(dd, e, A){
        return  false;
    },
    ...
    notifyOver : function(dd, e, A){
        ...
        if(!n){ 
            if(this.lastOverNode){
                this.onNodeOut(this.lastOverNode, dd, e, A);
                this.lastOverNode = null;
            }
            return  this.onContainerOver(dd, e, A);
        }
        if(this.lastOverNode != n){
            ...
            this.onNodeEnter(n, dd, e, A);
            this.lastOverNode = n;
        }
        return  this.onNodeOver(n, dd, e, A);
    },
    ...
    notifyDrop : function(dd, e, A){
        ...
        var  n = this.getTargetFromEvent(e);
        return  n ?
            this.onNodeDrop(n, dd, e, A) :
            this.onContainerDrop(dd, e, A);
    },
...
Roo.data.SortTypes = {
    ...
    asInt : function(s) {
        var  A = parseInt(String(s).replace(/,/g, ""));
        if(isNaN(A)) A = 0;
    	return  A;
    }
};
...
Roo.data.Record.prototype = {
    ...
    copy : function(A) {
        return  new  this.constructor(Roo.apply({}, this.data), A || this.id);
    }
};
...
Roo.extend(Roo.data.Store, Roo.util.Observable, {
    ...
    load : function(A){
        A = A || {};
        if(this.fireEvent("beforeload", this, A) !== false){
            this.storeOptions(A);
            var  p = Roo.apply(A.params || {}, this.baseParams);
            ...
            this.proxy.load(p, this.reader, this.loadRecords, this, A);
        }
    },
    ...
    reload : function(A){
        this.load(Roo.applyIf(A||{}, this.lastOptions));
    },

    
    
    loadRecords : function(o, A, B){
        if(!o || B === false){
            if(B !== false){
                this.fireEvent("load", this, [], A);
            }
            if(A.callback){
                A.callback.call(A.scope || this, [], A, false);
            }
            return;
        }
        
        if (o.success === false) {
            this.fireEvent("loadexception", this, o, A, this.reader.jsonData);
            return;
        }
        ...
        this.fireEvent("load", this, r, A);
        if(A.callback){
            A.callback.call(A.scope || this, r, A, true);
        }
    },

    

    loadData : function(o, A){
        var  r = this.reader.readRecords(o);
        this.loadRecords(r, {add: A}, true);
    },
    ...
    setDefaultSort : function(A, B){
        this.sortInfo = {field: A, direction: B ? B.toUpperCase() : "ASC"};
    },

    

    sort : function(A, B){
        var  f = this.fields.get(A);
        if(!B){
            this.sortToggle[f.name] = this.sortToggle[f.name] || f.sortDir;
            
            if(this.multiSort || (this.sortInfo && this.sortInfo.field == f.name) ){ 
                B = (this.sortToggle[f.name] || "ASC").toggle("ASC", "DESC");
            }else {
                B = f.sortDir;
            }
        }

        this.sortToggle[f.name] = B;
        this.sortInfo = {field: f.name, direction: B};
        if(!this.remoteSort){
            this.applySort();
            this.fireEvent("datachanged", this);
        }else {
    ...
    createFilterFn : function(A, B, C){
        if(!B.exec){ 
            ...
            B = new  RegExp((C === true ? '' : '^') + Roo.escapeRe(B), "i");
        }
        return  function(r){
            return  B.test(r.data[A]);
        };
    },
    ...
    filter : function(A, B, C){
        var  fn = this.createFilterFn(A, B, C);
        return  fn ? this.filterBy(fn) : this.clearFilter();
    },

    

    filterBy : function(fn, A){
        this.snapshot = this.snapshot || this.data;
        this.data = this.queryBy(fn, A||this);
        this.fireEvent("datachanged", this);
    },
    ...
    queryBy : function(fn, A){
        var  B = this.snapshot || this.data;
        return  B.filterBy(fn, A||this);
    },

    

    collect : function(A, B, C){
        var  d = (C === true && this.snapshot) ?
                this.snapshot.items : this.data.items;
        var  v, sv, r = [], l = {};
        for(var  i = 0, D = d.length; i < D; i++){
            v = d[i].data[A];
            sv = String(v);
            if((B || !Roo.isEmpty(v)) && !l[sv]){
                l[sv] = true;
                r[r.length] = v;
            }
        }
    ...
    afterEdit : function(A){
        ...
        this.fireEvent("update", this, A, Roo.data.Record.EDIT);
    },

    
    afterReject : function(A){
        this.modified.remove(A);
        this.fireEvent("update", this, A, Roo.data.Record.REJECT);
    },

    
    afterCommit : function(A){
        this.modified.remove(A);
        this.fireEvent("update", this, A, Roo.data.Record.COMMIT);
    },
    ...
    onMetaChange : function(A, B, o){
        this.recordType = B;
        this.fields = B.prototype.fields;
        delete  this.snapshot;
        this.sortInfo = A.sortInfo || this.sortInfo;
        this.modified = [];
...
Roo.extend(Roo.data.HttpProxy, Roo.data.DataProxy, {
    ...
    load : function(A, B, C, D, E){
        if(this.fireEvent("beforeload", this, A) !== false){
            ...
            C.call(D||this, null, E, false);
        }
    },

    
    loadResponse : function(o, A, B){
        delete  this.activeRequest;
        if(!A){
            this.fireEvent("loadexception", this, o, B);
        ...
        o.request.callback.call(o.request.scope, C, o.request.arg, true);
    },
...
Roo.extend(Roo.data.ScriptTagProxy, Roo.data.DataProxy, {
    ...
    load : function(A, B, C, D, E){
        if(this.fireEvent("beforeload", this, A) !== false){
            ...
            var  p = Roo.urlEncode(Roo.apply(A, this.extraParams));

            var  F = this.url;
            F += (F.indexOf("?") != -1 ? "&" : "?") + p;
            if(this.nocache){
                F += "&_dc=" + (new  Date().getTime());
            }
            var  G = ++Roo.data.ScriptTagProxy.TRANS_ID;
            var  H = {
                id : G,
                cb : "stcCallback"+G,
                scriptId : "stcScript"+G,
                params : A,
                arg : E,
                url : F,
                callback : C,
            ...
            window[H.cb] = function(o){
                I.handleResponse(o, H);
            };

            F += String.format("&{0}={1}", this.callbackParam, H.cb);
            ...
            H.timeoutId = this.handleFailure.defer(this.timeout, this, [H]);

            var  J = document.createElement("script");
            J.setAttribute("src", F);
            J.setAttribute("type", "text/javascript");
            J.setAttribute("id", H.scriptId);
    ...
    destroyTrans : function(A, B){
        this.head.removeChild(document.getElementById(A.scriptId));
        clearTimeout(A.timeoutId);
        if(B){
            window[A.cb] = undefined;
            try{
                delete  window[A.cb];
            }catch(e){}
        }else {
            
            window[A.cb] = function(){
                window[A.cb] = undefined;
                try{
                    delete  window[A.cb];
                }catch(e){}
    ...
    handleResponse : function(o, A){
        this.trans = false;
        this.destroyTrans(A, true);
        var  B;
        try {
            B = A.reader.readRecords(o);
        }catch(e){
        ...
        this.fireEvent("load", this, o, A.arg);
        A.callback.call(A.scope||window, B, A.arg, true);
    },
...
Roo.extend(Roo.data.JsonReader, Roo.data.DataReader, {
 ...
 eval("("+B+")");
    ...
    readRecords : function(o){
        ...
        this.jsonData = o;
        var  s = this.meta, A = this.recordType,
            f = A.prototype.fields, fi = f.items, fl = f.length;
        ...
        if (!this.ef) {
            ...
            for(var  jj = 0; jj < fl; jj++){
                f = fi[jj];
                var  B = (f.mapping !== undefined && f.mapping !== null) ? f.mapping : f.name;
                this.ef[jj] = this.getJsonAccessor(B);
            }
        }

    	var  C = this.getRoot(o), c = C.length, D = c, E = true;
    	if(s.totalProperty){
	    ...
	    for(var  i = 0; i < c; i++){
	        ...
	        for(var  j = 0; j < fl; j++){
                ...
                G[f.name] = f.convert((v !== undefined) ? v : f.defaultValue);
	        }
	        var  H = new  A(G, id);
	        H.json = n;
	        F[i] = H;
...
Roo.extend(Roo.data.XmlReader, Roo.data.DataReader, {
    ...
    readRecords : function(A){
        ...
        this.xmlData = A;
        var  B = A.documentElement || A;
    	var  q = Roo.DomQuery;
    	var  C = this.recordType, D = C.prototype.fields;
    	var  E = this.meta.id;
    	var  F = 0, G = true;
    	if(this.meta.totalRecords){
    	    F = q.selectNumber(this.meta.totalRecords, B, 0);
    	}
        
        if(this.meta.success){
            var  sv = q.selectValue(this.meta.success, B, true);
            G = sv !== false && sv !== 'false';
    	}
    	var  H = [];
    	var  ns = q.select(this.meta.record, B);
        for(var  i = 0, I = ns.length; i < I; i++) {
	        var  n = ns[i];
	        var  J = {};
	        var  id = E ? q.selectValue(E, n) : undefined;
...
Roo.extend(Roo.data.ArrayReader, Roo.data.JsonReader, {
    ...
    readRecords : function(o){
        var  A = this.meta ? this.meta.id : null;
    	var  B = this.recordType, C = B.prototype.fields;
    	var  D = [];
    	var  E = o;
	    for(var  i = 0; i < E.length; i++){
		    var  n = E[i];
	        var  F = {};
	        var  id = ((A || A === 0) && n[A] !== undefined && n[A] !== "" ? n[A] : null);
	        for(var  j = 0, G = C.length; j < G; j++){
                var  f = C.items[j];
...
Roo.extend(Roo.data.Node, Roo.util.Observable, {
    ...
    appendChild : function(A){
        ...
        if(B){
            for(var  i = 0, C = B.length; i < C; i++) {
            	this.appendChild(B[i]);
            }
        }else {
            if(this.fireEvent("beforeappend", this.ownerTree, this, A) === false){
                return  false;
            }
            ...
            if(E){
                if(A.fireEvent("beforemove", A.getOwnerTree(), A, E, this, D) === false){
                    return  false;
                }
            ...
            A.parentNode = this;
            var  ps = this.childNodes[D-1];
            if(ps){
                A.previousSibling = ps;
                ps.nextSibling = A;
            }else {
            ...
            A.nextSibling = null;
            this.setLastChild(A);
            A.setOwnerTree(this.getOwnerTree());
            this.fireEvent("append", this.ownerTree, this, A, D);
            if(E){
                A.fireEvent("move", this.ownerTree, A, E, this, D);
            }
            return  A;
        }
    },
    ...
    removeChild : function(A){
        ...
        if(this.fireEvent("beforeremove", this.ownerTree, this, A) === false){
            return  false;
        }
        ...
        A.previousSibling = null;
        A.nextSibling = null;
        this.fireEvent("remove", this.ownerTree, this, A);
        return  A;
    },
    ...
    insertBefore : function(A, B){
        ...
        if(this.fireEvent("beforeinsert", this.ownerTree, this, A, B) === false){
            return  false;
        }
        ...
        if(D == this && this.childNodes.indexOf(A) < C){
            E--;
        }

        
        if(D){
            if(A.fireEvent("beforemove", A.getOwnerTree(), A, D, this, C, B) === false){
                return  false;
            }
        ...
        this.childNodes.splice(E, 0, A);
        A.parentNode = this;
        var  ps = this.childNodes[E-1];
        if(ps){
            A.previousSibling = ps;
            ps.nextSibling = A;
        }else {
        ...
        B.previousSibling = A;
        A.setOwnerTree(this.getOwnerTree());
        this.fireEvent("insert", this.ownerTree, this, A, B);
        if(D){
            A.fireEvent("move", this.ownerTree, A, D, this, E, B);
        }
        return  A;
    },
    ...
    bubble : function(fn, A, B){
        var  p = this;
        while(p){
            if(fn.call(A || p, B || p) === false){
                break;
            }
    ...
    cascade : function(fn, A, B){
        if(fn.call(A || this, B || this) !== false){
            var  cs = this.childNodes;
    ...
    eachChild : function(fn, A, B){
        var  cs = this.childNodes;
        for(var  i = 0, C = cs.length; i < C; i++) {
        	if(fn.call(A || this, B || cs[i]) === false){
        	    break;
        	}
        }
    },
    ...
    findChildBy : function(fn, A){
        var  cs = this.childNodes;
        for(var  i = 0, B = cs.length; i < B; i++) {
        	if(fn.call(A||cs[i], cs[i]) === true){
        	    return  cs[i];
        	}
        }
        return  null;
    ...
    sort : function(fn, A){
        ...
        var  B = cs.length;
        if(B > 0){
            var  C = A ? function(){fn.apply(A, arguments);} : fn;
            cs.sort(C);
            for(var  i = 0; i < B; i++){
                var  n = cs[i];
...
Roo.ComponentMgr = function(){
    ...
    return  {
        ...
        onAvailable : function(id, fn, B){
            A.on("add", function(C, o){
                if(o.id == id){
                    fn.call(B || o, o);
                    A.un("add", fn, B);
                }
...
Roo.extend(Roo.Component, Roo.util.Observable, {
    ...
    onRender : function(ct, A){
        if(this.el){
            ...
            if(this.allowDomMove !== false){
                ct.dom.insertBefore(this.el.dom, A);
            }
        }
    },
    ...
    cloneConfig : function(A){
        A = A || {};
        var  id = A.id || Roo.id();
        var  B = Roo.applyIf(A, this.initialConfig);
        B.id = id; 
...
Roo.Shadow.prototype = {
    ...
    realign : function(l, t, w, h){
        ...
        s.left = (l+a.l)+"px";
        s.top = (t+a.t)+"px";
        var  sw = (w+a.w), sh = (h+a.h), B = sw +"px", C = sh + "px";
 
        if(s.width != B || s.height != C){
            s.width = B;
            s.height = C;
            if(!Roo.isIE){
                var  cn = d.childNodes;
                var  D = Math.max(0, (sw-12))+"px";
                cn[0].childNodes[1].style.width = D;
                cn[1].childNodes[1].style.width = D;
                cn[2].childNodes[1].style.width = D;
...
Roo.Shadow.Pool = function(){
    ...
    return  {
        pull : function(){
            var  sh = p.shift();
            if(!sh){
                sh = Roo.get(Roo.DomHelper.insertHtml("beforeBegin", document.body.firstChild, A));
...
Roo.extend(Roo.BoxComponent, Roo.Component, {
    ...
    onRender : function(ct, A){
        Roo.BoxComponent.superclass.onRender.call(this, ct, A);
        if(this.resizeEl){
...
Roo.extend(Roo.SplitBar, Roo.util.Observable, {
    onStartProxyDrag : function(x, y){
        ...
        var  c1 = A - this.activeMinSize;
        var  c2 = Math.max(this.activeMaxSize - A, 0);
        if(this.orientation == Roo.SplitBar.HORIZONTAL){
    ...
    onEndProxyDrag : function(e){
        ...
        var  A = Roo.lib.Event.getXY(e);
        if(this.overlay){
            this.overlay.hide();
        }
        var  B;
        if(this.orientation == Roo.SplitBar.HORIZONTAL){
            B = this.dragSpecs.startSize + 
                (this.placement == Roo.SplitBar.LEFT ?
                    A[0] - this.dragSpecs.startPoint[0] :
                    this.dragSpecs.startPoint[0] - A[0]
                );
        }else {
            B = this.dragSpecs.startSize + 
                (this.placement == Roo.SplitBar.TOP ?
                    A[1] - this.dragSpecs.startPoint[1] :
                    this.dragSpecs.startPoint[1] - A[1]
                );
        }

        B = Math.min(Math.max(B, this.activeMinSize), this.activeMaxSize);
        if(B != this.dragSpecs.startSize){
            if(this.fireEvent('beforeapply', this, B) !== false){
...
Roo.SplitBar.createProxy = function(A){
    var  B = new  Roo.Element(document.createElement("div"));
    B.unselectable();
    var  C = 'x-splitbar-proxy';
    B.addClass(C + ' ' + (A == Roo.SplitBar.HORIZONTAL ? C +'-h' : C + '-v'));
...
Roo.SplitBar.BasicLayoutAdapter.prototype = {
    ...
    setElementSize : function(s, A, B){
        if(s.orientation == Roo.SplitBar.HORIZONTAL){
            if(!s.animate){
                s.resizingEl.setWidth(A);
                if(B){
                    B(s, A);
                }
            }else {
                s.resizingEl.setWidth(A, true, .1, B, 'easeOut');
            }
        }else {
            
            if(!s.animate){
                s.resizingEl.setHeight(A);
                if(B){
                    B(s, A);
                }
            }else {
                s.resizingEl.setHeight(A, true, .1, B, 'easeOut');
            }
        }
    }
};
...
Roo.SplitBar.AbsoluteLayoutAdapter = function(A){
...
Roo.SplitBar.AbsoluteLayoutAdapter.prototype = {
    ...
    setElementSize : function(s, A, B){
        this.basic.setElementSize(s, A, this.moveSplitter.createDelegate(this, [s]));
    },
...
Roo.extend(Roo.View, Roo.util.Observable, {
    ...
    refresh : function(){
        ...
        for(var  i = 0, C = B.length; i < C; i++){
            var  D = this.prepareData(B[i].data, i, B[i]);
            this.fireEvent("preparedata", this, D, i, B[i]);
            A[A.length] = t.apply(D);
        }

        this.el.update(A.join(""));
    ...
    onUpdate : function(ds, A){
        this.clearSelections();
        var  B = this.store.indexOf(A);
        var  n = this.nodes[B];
        this.tpl.insertBefore(n, this.prepareData(A.data));
        n.parentNode.removeChild(n);
        this.updateIndexes(B, B);
    },

    onAdd : function(ds, A, B){
        this.clearSelections();
        ...
        for(var  i = 0, C = A.length; i < C; i++){
            var  d = this.prepareData(A[i].data);
            if(n){
    ...
    onClick : function(e){
        var  A = this.findItemFromChild(e.getTarget());
        if(A){
            var  B = this.indexOf(A);
            if(this.onItemClick(A, B, e) !== false){
                this.fireEvent("click", this, B, A, e);
            }
        }else {
            this.clearSelections();
        }
    ...
    onContextMenu : function(e){
        var  A = this.findItemFromChild(e.getTarget());
        if(A){
            this.fireEvent("contextmenu", this, this.indexOf(A), A, e);
        }
    },
    ...
    onDblClick : function(e){
        var  A = this.findItemFromChild(e.getTarget());
        if(A){
            this.fireEvent("dblclick", this, this.indexOf(A), A, e);
        }
    },

    onItemClick : function(A, B, e){
        if(this.fireEvent("beforeclick", this, B, A, e) === false){
            return  false;
        }
        if(this.multiSelect || this.singleSelect){
            if(this.multiSelect && e.shiftKey && this.lastSelection){
                this.select(this.getNodes(this.indexOf(this.lastSelection), B), false);
            }else {
                this.select(A, this.multiSelect && e.ctrlKey);
    ...
    getSelectedIndexes : function(){
        var  A = [], s = this.selections;
        for(var  i = 0, B = s.length; i < B; i++){
            A.push(s[i].nodeIndex);
        }
        return  A;
    },
        ...
        if(A  instanceof  Array){
            ...
            if(E && !this.isSelected(E)){
                ...
                if(this.fireEvent("beforeselect", this, E, this.selections) !== false){
                    Roo.fly(E).addClass(this.selectedClass);
    ...
    getNodes : function(A, B){
        var  ns = this.nodes;
        A = A || 0;
        B = typeof  B == "undefined" ? ns.length - 1 : B;
        var  C = [];
        if(A <= B){
            for(var  i = A; i <= B; i++){
                C.push(ns[i]);
    ...
    indexOf : function(A){
        A = this.getNode(A);
        if(typeof  A.nodeIndex == "number"){
            return  A.nodeIndex;
        }
        var  ns = this.nodes;
        for(var  i = 0, B = ns.length; i < B; i++){
            if(ns[i] == A){
                return  i;
            }
        }
...
Roo.extend(Roo.JsonView, Roo.View, {
    ...
    refresh : function(){
        ...
        var  o = this.jsonData;
        if(o && o.length > 0){
            for(var  i = 0, B = o.length; i < B; i++){
                var  C = this.prepareData(o[i], i, o);
                A[A.length] = this.tpl.apply(C);
            }
        }else {
            A.push(this.emptyText);
        }
    ...
    render : function(el, A){
        this.clearSelections();
        this.el.update("");
        var  o;
        try{
            o = Roo.util.JSON.decode(A.responseText);
            if(this.jsonRoot){
    ...
    getNodeData : function(A){
        if(A  instanceof  Array){
            var  B = [];
            for(var  i = 0, C = A.length; i < C; i++){
                B.push(this.getNodeData(A[i]));
            }
            return  B;
        }
        return  this.jsonData[this.indexOf(A)] || null;
    },
    ...
    filter : function(A, B){
        if(this.jsonData){
            ...
            if(typeof  B == "string"){
                ...
                B = B.toLowerCase();
                for(var  i = 0, E = ss.length; i < E; i++){
                    var  o = ss[i];
                    if(o[A].substr(0, D).toLowerCase() == B){
                        C.push(o);
                    }
                }
            } else  if(B.exec){ 
    ...
    sort : function(A, B, C){
        ...
        if(this.jsonData){
            var  p = A;
            var  D = B && B.toLowerCase() == "desc";
            var  f = function(o1, o2){
                var  v1 = C ? C(o1[p]) : o1[p];
                var  v2 = C ? C(o2[p]) : o2[p];
                ;
                if(v1 < v2){
                    return  D ? +1 : -1;
                } else  if(v1 > v2){
...
Roo.extend(Roo.ColorPalette, Roo.Component, {
    ...
    onRender : function(A, B){
        ...
        var  c = this.colors;
        for(var  i = 0, C = c.length; i < C; i++){
            t.add([c[i]]);
        }
        var  el = document.createElement("div");
        el.className = this.itemCls;
        t.overwrite(el);
        A.dom.insertBefore(el, B);
        this.el = Roo.get(el);
    ...
    select : function(A){
        ...
        if(A != this.value || this.allowReselect){
            ...
            el.child("a.color-"+A).addClass("x-color-palette-sel");
            this.value = A;
...
Roo.extend(Roo.DatePicker, Roo.Component, {
    ...
    onRender : function(A, B){
        ...
        var  C = (new  Date()).dateFormat(this.format);
        
        var  D = new  Roo.Toolbar(this.el.child("td.x-date-bottom", true));
        if (this.showClear) {
            D.add( new  Roo.Toolbar.Fill());
        }

        D.add({
            text: String.format(this.todayText, C),
            tooltip: String.format(this.todayTip, C),
    ...
    update : function(A){
        ...
        if(vd && this.el){
            var  t = A.getTime();
            if(vd.getMonth() == A.getMonth() && vd.getFullYear() == A.getFullYear()){
        ...
        var  B = A.getDaysInMonth();
        var  C = A.getFirstDateOfMonth();
        var  D = C.getDay()-this.startDay;
        ...
        var  pm = A.add("mo", -1);
        var  E = pm.getDaysInMonth()-D;

        var  F = this.cells.elements;
        var  G = this.textNodes;
        B += D;

        
        var  H = 86400000;
        var  d = (new  Date(pm.getFullYear(), pm.getMonth(), E)).clearTime();
        var  I = new  Date().clearTime().getTime();
        var  J = A.clearTime().getTime();
        var  K = this.minDate ? this.minDate.clearTime() : Number.NEGATIVE_INFINITY;
        var  L = this.maxDate ? this.maxDate.clearTime() : Number.POSITIVE_INFINITY;
        var  M = this.disabledDatesRE;
        var  N = this.disabledDatesText;
        var  O = this.disabledDays ? this.disabledDays.join("") : false;
        var  P = this.disabledDaysText;
        var  Q = this.format;

        var  R = function(U, V){
            V.title = "";
            var  t = d.getTime();
            V.firstChild.dateValue = t;
            if(t == I){
                V.className += " x-date-today";
                V.title = U.todayText;
            }
            if(t == J){
                V.className += " x-date-selected";
                setTimeout(function(){
                    try{V.firstChild.focus();}catch(e){}
                }, 50);
            }
            
            if(t < K) {
                V.className = " x-date-disabled";
                V.title = U.minText;
                return;
            }
            if(t > L) {
            ...
            if(O){
                if(O.indexOf(d.getDay()) != -1){
                    V.title = P;
                    V.className = " x-date-disabled";
                }
            }
            if(M && Q){
                var  W = d.dateFormat(Q);
                if(M.test(W)){
                    V.title = N.replace("%0", W);
                    V.className = " x-date-disabled";
                }
            }
        };
        ...
        for(; i < D; i++) {
            ...
            F[i].className = "x-date-prevday";
            R(this, F[i]);
        }
        for(; i < B; i++){
            intDay = i - D + 1;
            G[i].innerHTML = (intDay);
            d.setDate(d.getDate()+1);
            F[i].className = "x-date-active";
            R(this, F[i]);
        }
        var  S = 0;
        for(; i < 42; i++) {
        ...
        if(!this.internalRender){
            var  T = this.el.dom.firstChild;
            var  w = T.offsetWidth;
            this.el.setWidth(w + this.el.getBorderWidth("lr"));
            Roo.fly(T).setWidth(w);
...
Roo.TabPanel = function(A, B){
    ...
    if (this.toolbar) {
        var  C = this.toolbar;
        C.container = this.stripEl.child('td.x-tab-strip-toolbar');  
        this.toolbar = new  Roo.Toolbar(C);
        if (Roo.isSafari) {
            var  D = C.container.child('table', true);
...
Roo.extend(Roo.TabPanel, Roo.util.Observable, {
    ...
    addTab : function(id, A, B, C){
        var  D = new  Roo.TabPanelItem(this, id, A, C);
        this.addTabItem(D);
        if(B){
            D.setContent(B);
        }
    ...
    removeTab : function(id){
        ...
        if(!B) { return; }
        var  C = A.indexOf(B);
        if(this.active == B && A.length > 1){
            var  D = this.getNextAvailable(C);
            if(D) {
                D.activate();
            }
        }

        this.stripEl.dom.removeChild(B.pnode.dom);
        if(B.bodyEl.dom.parentNode == this.bodyEl.dom){ 
            this.bodyEl.dom.removeChild(B.bodyEl.dom);
        }

        A.splice(C, 1);
        delete  this.items[B.id];
        B.fireEvent("close", B);
        B.purgeListeners();
        this.autoSizeTabs();
    },

    getNextAvailable : function(A){
        var  B = this.items;
        var  C = A;
    ...
    activate : function(id){
        ...
        var  e = {};
        this.fireEvent("beforetabchange", this, e, A);
        if(e.cancel !== true && !A.disabled){
            if(this.active){
                this.active.hide();
            }
    ...
    syncHeight : function(A){
        var  B = (A || this.el.getHeight())-this.el.getBorderWidth("tb")-this.el.getPadding("tb");
        var  bm = this.bodyEl.getMargins();
        var  C = B-(this.stripWrap.getHeight()||0)-(bm.top+bm.bottom);
    ...
    autoSizeTabs : function(){
        ...
        var  B = A - this.hiddenCount;
        if(!this.resizeTabs || A < 1 || B < 1 || this.updating) return;
        var  w = Math.max(this.el.getWidth() - this.cpad, 10);
        var  C = Math.floor(w / B);
        var  b = this.stripBody;
        if(b.getWidth() > w){
            var  D = this.items;
            this.setTabWidth(Math.max(C, this.minTabWidth)-2);
            if(C < this.minTabWidth){
                

            }
        }else {
            if(this.currentTabWidth < this.preferredTabWidth){
                this.setTabWidth(Math.min(C, this.preferredTabWidth)-2);
            }
        }
    },
    ...
    setTabWidth : function(A){
        this.currentTabWidth = A;
        for(var  i = 0, B = this.items.length; i < B; i++) {
        	if(!this.items[i].isHidden())this.items[i].setWidth(A);
        }
    },
...
Roo.TabPanelItem = function(A, id, B, C){
    ...
    this.bodyEl = Roo.get(A.createItemBody(A.bodyEl.dom, id));
    this.bodyEl.setVisibilityMode(Roo.Element.VISIBILITY);
    this.bodyEl.setStyle("display", "block");
    this.bodyEl.setStyle("zoom", "1");
    this.hideAction();

    var  D = A.createStripElements(A.stripEl.dom, B, C);
...
Roo.extend(Roo.TabPanelItem, Roo.util.Observable, {
    ...
    setWidth : function(A){
        var  B = A - this.pnode.getPadding("lr");
        this.inner.setWidth(B);
        this.textEl.setWidth(B-this.inner.getPadding("lr"));
        this.pnode.setWidth(A);
    },
    ...
    setUrl : function(A, B, C){
        ...
        this.refreshDelegate = this._handleRefresh.createDelegate(this, [A, B, C]);
    ...
    _handleRefresh : function(A, B, C){
        if(!C || !this.loaded){
            var  D = this.bodyEl.getUpdateManager();
            D.update(A, B, this._setLoaded.createDelegate(this));
        }
    },
...
Roo.TabPanel.prototype.createStrip = function(A){
...
Roo.TabPanel.prototype.createStripList = function(A){
    
    
    A.innerHTML = '<div class="x-tabs-strip-wrap">'+
...
Roo.TabPanel.prototype.createBody = function(A){
    var  B = document.createElement("div");
    Roo.id(B, "tab-body");
    Roo.fly(B).addClass("x-tabs-body");
    A.appendChild(B);
    return  B;
};


Roo.TabPanel.prototype.createItemBody = function(A, id){
    var  B = Roo.getDom(id);
    if(!B){
    ...
    Roo.fly(B).addClass("x-tabs-item-body");
    A.insertBefore(B, A.firstChild);
    return  B;
};


Roo.TabPanel.prototype.createStripElements = function(A, B, C){
    var  td = document.createElement("td");
    A.insertBefore(td, A.childNodes[A.childNodes.length-1]);
    ...
    if(C){
        ...
        var  el = this.closeTpl.overwrite(td, {"text": B});
        var  D = el.getElementsByTagName("div")[0];
        var  E = el.getElementsByTagName("em")[0];
        return  {"el": el, "close": D, "inner": E};
    } else  {
        if(!this.tabTpl){
        ...
        var  el = this.tabTpl.overwrite(td, {"text": B});
        var  E = el.getElementsByTagName("em")[0];
        return  {"el": el, "inner": E};
    }
};
...
Roo.extend(Roo.Button, Roo.util.Observable, {
    ...
    render : function(A){
        ...
        if(!this.dhconfig){
            ...
            B = this.template.append(A, [this.text || '&#160;', this.type], true);
            ...
            B = Roo.DomHelper.append(Roo.get(A).dom, this.dhconfig, true);
        }
        ...
        if(this.repeat){
            var  D = new  Roo.util.ClickRepeater(B,
...
Roo.ButtonToggleMgr = function(){
   ...
   function  toggleGroup(B, C){
       if(C){
           var  g = A[B.toggleGroup];
...
Roo.extend(Roo.SplitButton, Roo.Button, {
    render : function(A){
        ...
        var  C = B.append(A, [this.text, this.type], true);
        var  D = C.child("button");
        if(this.cls){
            C.addClass(this.cls);
        }
        if(this.icon){
            D.setStyle('background-image', 'url(' +this.icon +')');
        }
        if(this.iconCls){
            D.addClass(this.iconCls);
            if(!this.cls){
                C.addClass(this.text ? 'x-btn-text-icon' : 'x-btn-icon');
            }
        }

        this.el = C;
        if(this.handleMouseEvents){
            C.on("mouseover", this.onMouseOver, this);
            C.on("mouseout", this.onMouseOut, this);
            C.on("mousedown", this.onMouseDown, this);
            C.on("mouseup", this.onMouseUp, this);
        }
        ...
        if(this.tooltip){
            if(typeof  this.tooltip == 'object'){
                ...
                D.dom[this.tooltipType] = this.tooltip;
            }
        }
        if(this.arrowTooltip){
            C.child("button:nth(2)").dom[this.tooltipType] = this.arrowTooltip;
        }
        if(this.hidden){
            this.hide();
        }
    ...
    autoWidth : function(){
        if(this.el){
            var  A = this.el.child("table:first");
            ...
            if(this.minWidth){
                ...
                if((A.getWidth()+B.getWidth()) < this.minWidth){
                    A.setWidth(this.minWidth-B.getWidth());
                }
                if(this.hidden){
                    this.el.endMeasure();
                }
            ...
            this.el.setWidth(A.getWidth()+B.getWidth());
        } 
    },
...
Roo.Toolbar.prototype = {
    ...
    addButton : function(A){
        if(A  instanceof  Array){
            var  B = [];
            for(var  i = 0, C = A.length; i < C; i++) {
                B.push(this.addButton(A[i]));
            }
            return  B;
        }
        var  b = A;
        if(!(A  instanceof  Roo.Toolbar.Button)){
            b = A.split ?
                new  Roo.Toolbar.SplitButton(A) :
                new  Roo.Toolbar.Button(A);
        }
        var  td = this.nextBlock();
        b.render(td);
        this.items.add(b);
    ...
    addText : function(A){
        return  this.addItem(new  Roo.Toolbar.TextItem(A));
    },
    
    

    insertButton : function(A, B){
        if(B  instanceof  Array){
            var  C = [];
            for(var  i = 0, D = B.length; i < D; i++) {
               C.push(this.insertButton(A + i, B[i]));
            }
            return  C;
        }
        if (!(B  instanceof  Roo.Toolbar.Button)){
           B = new  Roo.Toolbar.Button(B);
        }
        var  td = document.createElement("td");
        this.tr.insertBefore(td, this.tr.childNodes[A]);
        B.render(td);
        this.items.insert(A, B);
        return  B;
    },
...
Roo.Toolbar.TextItem = function(A){
    if (typeof(A) == 'object') {
        A = A.text;
    }
    var  s = document.createElement("span");
...
Roo.Toolbar.Button = function(A){
    Roo.Toolbar.Button.superclass.constructor.call(this, null, A);
};
...
Roo.Toolbar.SplitButton = function(A){
    Roo.Toolbar.SplitButton.superclass.constructor.call(this, null, A);
};
...
Roo.extend(Roo.PagingToolbar, Roo.Toolbar, {
    ...
    updateInfo : function(){
        if(this.displayEl){
            ...
            var  B = A == 0 ?
                ...
                String.format(
                    this.displayMsg,
                    this.cursor+1, this.cursor+A, this.ds.getTotalCount()    
                );
    ...
    onPagingKeydown : function(e){
        ...
        if(k == e.RETURN ){
            var  v = this.field.dom.value, A;
            if(!v || isNaN(A = parseInt(v, 10))){
                this.field.dom.value = d.activePage;
                return;
            }

            A = Math.min(Math.max(1, A), d.pages) - 1;
            this.ds.load({params:{start: A * this.pageSize, limit: this.pageSize}});
            e.stopEvent();
        }
        ...
        {
          var  A = (k == e.HOME || (k == e.DOWN && e.ctrlKey) || (k == e.LEFT && e.ctrlKey) || (k == e.PAGEDOWN && e.ctrlKey)) ? 1 : d.pages;
          this.field.dom.value = A;
          this.ds.load({params:{start: (A - 1) * this.pageSize, limit: this.pageSize}});
          e.stopEvent();
        }
        else  if(k == e.UP || k == e.RIGHT || k == e.PAGEUP || k == e.DOWN || k == e.LEFT || k == e.PAGEDOWN)
        {
          var  v = this.field.dom.value, A; 
          var  B = (e.shiftKey) ? 10 : 1;
          ...
          else  if(parseInt(v, 10) + B >= 1 & parseInt(v, 10) + B <= d.pages)
          {
            this.field.dom.value = parseInt(v, 10) + B;
            A = Math.min(Math.max(1, A + B), d.pages) - 1;
            this.ds.load({params:{start: A * this.pageSize, limit: this.pageSize}});
          }
    ...
    onClick : function(A){
        ...
        switch(A){
            ...
            case  "last":
                ...
                var  C = B % this.pageSize;
                var  D = C ? (B - C) : B-this.pageSize;
                ds.load({params:{start: D, limit: this.pageSize}});
            break;
            case  "refresh":
...
Roo.Resizable = function(el, A)
{
    ...
    var  C = this.el.getStyle("position");
    ...
    for(var  i = 0, D = hs.length; i < D; i++){
        if(hs[i] && ps[hs[i]]){
            var  E = ps[hs[i]];
            this[E] = new  Roo.Resizable.Handle(this, E, this.disableTrackOver, this.transparent);
        }
    }
...
Roo.extend(Roo.Resizable, Roo.util.Observable, {
    ...
    startSizing : function(e, A){
        ...
        if(this.enabled){ 
            ...
            this.overlay.setStyle("cursor", A.el.getStyle("cursor"));
    ...
    onMouseUp : function(e){
        ...
        this.overlay.hide();
        this.proxy.hide();
        this.fireEvent("resize", this, A.width, A.height, e);
    },
    ...
    updateChildSize : function(){
        if(this.resizeChild){
            ...
            if(el.dom.offsetWidth){
                ...
                A.setSize(b.width+B[0], b.height+B[1]);
            }
    ...
    onMouseMove : function(e){
        if(this.enabled){
            ...
            var  B = this.maxWidth, C = this.maxHeight;
            var  wi = this.widthIncrement;
            var  hi = this.heightIncrement;

            var  D = e.getXY();
            var  E = -(this.startPoint[0] - Math.max(this.minX, D[0]));
            var  F = -(this.startPoint[1] - Math.max(this.minY, D[1]));
            ...
            switch(G){
                case  "east":
                    w += E;
                    w = Math.min(Math.max(mw, w), B);
                    break;
             
                case  "south":
                    h += F;
                    h = Math.min(Math.max(mh, h), C);
                    break;
                case  "southeast":
                    w += E;
                    h += F;
...
Roo.Resizable.Handle = function(rz, A, B, C){
    if(!this.tpl){
        
        var  D = Roo.DomHelper.createTemplate(
            {tag: "div", cls: "x-resizable-handle x-resizable-handle-{0}"}
        );
        D.compile();
        Roo.Resizable.Handle.prototype.tpl = D;
    }

    this.position = A;
    this.rz = rz;
    
    var  E = A == 'hdrag' ? 'north' : A;
    
    this.el = this.tpl.append(rz.el.dom, [E], true);
    if (A == 'hdrag') {
...
Roo.extend(Roo.Editor, Roo.Component, {
    ...
    onSpecialKey : function(A, e)
    {
        ...
        this.fireEvent('specialkey', A, e);
    
    },

    

    startEdit : function(el, A){
        if(this.editing){
            this.completeEdit();
        }

        this.boundEl = Roo.get(el);
        var  v = A !== undefined ? A : this.boundEl.dom.innerHTML;
        if(!this.rendered){
...
Roo.extend(Roo.BasicDialog, Roo.util.Observable, {
    ...
    addKeyListener : function(A, fn, B){
        var  C, D, E, F;
        ...
        var  G = function(H, e){
            if((!D || e.shiftKey) && (!E || e.ctrlKey) &&  (!F || e.altKey)){
                var  k = e.getKey();
                if(C  instanceof  Array){
                    for(var  i = 0, I = C.length; i < I; i++){
                        if(C[i] == k){
                          fn.call(B || window, H, k, e);
                          return;
                        }
                    }
                }else {
    ...
    addButton : function(A, B, C){
        ...
        var  fc = false;
        if ((typeof(D.position) != 'undefined') && D.position < this.btnContainer.childNodes.length-1) {
            D.position = Math.max(0, D.position);
            fc = this.btnContainer.childNodes[D.position];
        }
         
        var  E = new  Roo.Button(
            fc ? 
    ...
    getHeaderFooterHeight : function(A){
        var  B = 0;
        if(this.header){
           B += this.header.getHeight();
        }
        if(this.footer){
           var  fm = this.footer.getMargins();
            B += (this.footer.getHeight()+fm.top+fm.bottom);
        }
    ...
    syncBodyHeight : function(){
        ...
        bd.setHeight(A-bd.getMargins("tb"));
    ...
    restoreState : function(){
        var  A = Roo.state.Manager.get(this.stateId || (this.el.id + "-state"));
        if(A && A.width){
            this.xy = [A.x, A.y];
    ...
    destroy : function(A){
        ...
        if(this.buttons){
           for(var  i = 0, B = this.buttons.length; i < B; i++){
    ...
    alignTo : function(A, B, C){
        this.xy = this.el.getAlignToXY(A, B, C);
        if(this.isVisible()){
            this.el.setXY(this.xy);
            this.adjustAssets();
        }
    ...
    anchorTo : function(el, A, B, C){
        var  D = function(){
            this.alignTo(el, A, B);
        };
        Roo.EventManager.onWindowResize(D, this);
        var  tm = typeof  C;
        if(tm != 'undefined'){
            Roo.EventManager.on(window, 'scroll', D, this,
                {buffer: tm == 'number' ? C : 50});
        }
    ...
    animHide : function(A){
        ...
        this.proxy.setBounds(b.x, b.y, b.width, b.height, true, .35,
                    this.hideEl.createDelegate(this, [A]));
    },
...
Roo.DialogManager = function(){
    ...
    return  {
        ...
        get : function(id){
            return  typeof  id == "object" ? id : A[id];
        },
        ...
        hideAll : function(){
            for(var  id  in  A){
                if(A[id] && typeof  A[id] != "function" && A[id].isVisible()){
                    A[id].hide();
                }
            }
        }
...
Roo.LayoutDialog = function(el, A){
    ...
    B.autoTabs = false;
    Roo.LayoutDialog.superclass.constructor.call(this, el, B);
    this.body.setStyle({overflow:"hidden", position:"relative"});
    this.layout = new  Roo.BorderLayout(this.body.dom, B);
...
Roo.MessageBox = function(){
    ...
    return  {
        ...
        updateProgress : function(Q, R){
            if(R){
                this.updateText(R);
            }
            if (pp) { 
                pp.setWidth(Math.floor(Q*I.dom.firstChild.offsetWidth));
            }
            return  this;
        },        
...
Roo.QuickTips = function(){
    var  el, A, B, C, tm, D, E, F = {}, G, H = null, I, J;
    var  ce, bd, xy, dd;
    var  K = false, L = true, M = false;
    var  N = 1, O = 1, P = 1, Q = [];
    ...
    var  R = function(e){
        ...
        if(t && F[t.id]){
            F[t.id].el = t;
            N = W.defer(tm.showDelay, tm, [F[t.id]]);
            return;
        }
        var  a, et = Roo.fly(t);
        var  ns = D.namespace;
        if(tm.interceptTitles && t.title){
            ...
            a = t.qtip || et.getAttributeNS(ns, D.attribute);
        }
        if(a){
            N = W.defer(tm.showDelay, tm, [{
                el: t, 
                text: a, 
    ...
    var  X = function(){
        if(ce){
            ...
            if(tm.autoDismiss && ce.autoHide !== false){
                P = setTimeout(Y, tm.autoDismissDelay);
            }
        }
    };
    ...
    var  Y = function(a){
        ...
        if(el.isVisible()){
            G.disable();
            if(a !== true && tm.animate){
                el.fadeOut({callback: Z});
            }else {
                Z();
            } 
        }
    ...
    return  {
       ...
       register : function(a){
           var  cs = a  instanceof  Array ? a : arguments;
           for(var  i = 0, b = cs.length; i < b; i++) {
               var  c = cs[i];
               var  d = c.target;
               if(d){
                   if(d  instanceof  Array){
                       for(var  j = 0, e = d.length; j < e; j++){
                           F[d[j]] = c;
                       }
                   }else {
                       F[typeof  d == 'string' ? d : Roo.id(d)] = c;
                   }
               }
           }
       },
...
Roo.extend(Roo.tree.TreePanel, Roo.data.Tree, {
    ...
    setRootNode : function(A){
        Roo.tree.TreePanel.superclass.setRootNode.call(this, A);
        if(!this.rootVisible){
            A.ui = new  Roo.tree.RootTreeNodeUI(A);
        }
        return  A;
    },
    ...
    expandPath : function(A, B, C){
        B = B || "id";
        var  D = A.split(this.pathSeparator);
        var  E = this.root;
        if(E.attributes[B] != D[1]){ 
            if(C){
                C(false, null);
            }
            return;
        ...
        E.expand(false, false, f);
    },

    

    selectPath : function(A, B, C){
        B = B || "id";
        var  D = A.split(this.pathSeparator);
        var  v = D.pop();
        if(D.length > 0){
            ...
            this.expandPath(D.join(this.pathSeparator), B, f);
        }else {
            this.root.select();
            if(C){
                C(true, this.root);
...
Roo.extend(Roo.tree.DefaultSelectionModel, Roo.util.Observable, {
    ...
    onNodeClick : function(A, e){
        if (e.ctrlKey && this.selNode == A)  {
            this.unselect(A);
            return;
        }
    ...
    select : function(A){
        var  B = this.selNode;
        if(B != A && this.fireEvent('beforeselect', this, A, B) !== false){
            if(B){
            ...
            this.fireEvent("selectionchange", this, A, B);
        }
        return  A;
    },
...
Roo.extend(Roo.tree.MultiSelectionModel, Roo.util.Observable, {
    ...
    onNodeClick : function(A, e){
        this.select(A, e, e.ctrlKey);
    },
    
    

    select : function(A, e, B){
        if(B !== true){
            this.clearSelections(true);
        }
...
Roo.extend(Roo.tree.TreeNode, Roo.data.Node, {
    ...
    setFirstChild : function(A){
        ...
        Roo.tree.TreeNode.superclass.setFirstChild.call(this, A);
        if(this.childrenRendered && of && A != of){
    ...
    setLastChild : function(A){
        ...
        Roo.tree.TreeNode.superclass.setLastChild.call(this, A);
        if(this.childrenRendered && ol && A != ol){
    ...
    appendChild : function(){
        var  A = Roo.tree.TreeNode.superclass.appendChild.apply(this, arguments);
    ...
    insertBefore : function(A, B){
        var  C = Roo.tree.TreeNode.superclass.insertBefore.apply(this, arguments);
        if(C && B && this.childrenRendered){
            A.render();
        }
    ...
    setText : function(A){
        var  B = this.text;
        this.text = A;
        this.attributes.text = A;
        if(this.rendered){ 
            this.ui.onTextChange(this, A, B);
        }

        this.fireEvent("textchange", this, A, B);
    },
    ...
    collapse : function(A, B){
        if(this.expanded && !this.isHiddenRoot()){
            if(this.fireEvent("beforecollapse", this, A, B) === false){
                return;
            }

            this.expanded = false;
            if((this.getOwnerTree().animate && B !== false) || B){
    ...
    ensureVisible : function(A){
        ...
        B.expandPath(this.parentNode.getPath(), false, function(){
            B.getTreeEl().scrollChildIntoView(this.ui.anchor);
            Roo.callback(A);
        }.createDelegate(this));
    },
    ...
    renderIndent : function(A, B){
        if(B){
            this.ui.childIndent = null;
        }

        this.ui.renderIndent();
        if(A === true && this.childrenRendered){
            var  cs = this.childNodes;
            for(var  i = 0, C = cs.length; i < C; i++){
                cs[i].renderIndent(true, B);
            }
        }
    }
});
...
Roo.extend(Roo.tree.AsyncTreeNode, Roo.tree.TreeNode, {
    expand : function(A, B, C){
        ...
        if(!this.loaded){
            ...
            var  E = this.loader || this.attributes.loader || this.getOwnerTree().getLoader();
            if(E){
                E.load(this, this.loadComplete.createDelegate(this, [A, B, C]));
                return;
            }
        }

        Roo.tree.AsyncTreeNode.superclass.expand.call(this, A, B, C);
    },
...
Roo.tree.TreeNodeUI.prototype = {
    ...
    onMove : function(A, B, C, D, E, F){
        this.childIndent = null;
        if(this.rendered){
            var  G = D.ui.getContainer();
            if(!G){
                this.holder = document.createElement("div");
                this.holder.appendChild(this.wrap);
                return;
            }
            var  H = F ? F.ui.getEl() : null;
            if(H){
                G.insertBefore(this.wrap, H);
            }else {
                G.appendChild(this.wrap);
            }
    ...
    addClass : function(A){
        if(this.elNode){
            Roo.fly(this.elNode).addClass(A);
        }
    },

    removeClass : function(A){
        if(this.elNode){
            Roo.fly(this.elNode).removeClass(A);
        }
    },
    ...
    initEvents : function(){
        ...
        var  A = Roo.fly(this.iconNode);
        A.on("click", this.onClick, this);
        A.on("dblclick", this.onDblClick, this);
        A.on("contextmenu", this.onContextMenu, this);
    ...
    focus : function(){
        if(!this.node.preventHScroll){
            ...
            try{
                var  A = this.node.getOwnerTree().getTreeEl().dom;
                var  l = A.scrollLeft;
                this.anchor.focus();
                A.scrollLeft = l;
            }catch(e){}
    ...
    toggleCheck : function(A){
        ...
        if(cb){
            cb.checked = (A === undefined ? !cb.checked : A);
        }
    },
    ...
    renderElements : function(n, a, A, B){
        ...
        var  F = ['<li class="x-tree-node"><div class="x-tree-node-el ', a.cls,'">',
            '<span class="x-tree-node-indent">',this.indentMarkup,"</span>",
            '<img src="', this.emptyIcon, '" class="x-tree-ec-icon" />',
            '<img src="', a.icon || this.emptyIcon, '" class="x-tree-node-icon',(a.icon ? " x-tree-node-inline-icon" : ""),(a.iconCls ? " "+a.iconCls : ""),'" unselectable="on" />',
            cb ? ('<input class="x-tree-node-cb" type="checkbox" ' + (a.checked ? 'checked="checked" />' : ' />')) : '',
            '<a hidefocus="on" href="',E,'" tabIndex="1" ',
        ...
        if(B !== true && n.nextSibling && n.nextSibling.ui.getEl()){
            this.wrap = Roo.DomHelper.insertHtml("beforeBegin",
                                n.nextSibling.ui.getEl(), F.join(""));
        }else {
            this.wrap = Roo.DomHelper.insertHtml("beforeEnd", A, F.join(""));
        }
    ...
    updateExpandIcon : function(){
        if(this.rendered){
            ...
            var  A = n.isLast() ? "x-tree-elbow-end" : "x-tree-elbow";
            var  B = n.hasChildNodes();
            if(B){
                if(n.expanded){
                    A += "-minus";
            ...
            var  C = "x-tree-ec-icon "+A;
            if(this.ecc != C){
                this.ecNode.className = C;
                this.ecc = C;
            }
        }
    },
...
Roo.extend(Roo.tree.TreeLoader, Roo.util.Observable, {
    ...
    load : function(A, B){
        ...
        if(A.attributes.children){ 
            var  cs = A.attributes.children;
            for(var  i = 0, C = cs.length; i < C; i++){
                A.appendChild(this.createNode(cs[i]));
            }
    ...
    getParams: function(A){
        var  B = [], bp = this.baseParams;
        for(var  C  in  bp){
            if(typeof  bp[C] != "function"){
                B.push(encodeURIComponent(C), "=", encodeURIComponent(bp[C]), "&");
            }
        }
        var  n = this.queryParam === false ? 'node' : this.queryParam;
        B.push(n + "=", encodeURIComponent(A.id));
        return  B.join("");
    },
 ...
 eval("("+D+")");
    ...
    handleResponse : function(A){
        this.transId = false;
        var  a = A.argument;
        this.processResponse(A, a.node, a.callback);
        this.fireEvent("load", this, a.node, A);
    },

    handleFailure : function(A){
        this.transId = false;
        var  a = A.argument;
        this.fireEvent("loadexception", this, a.node, A);
...
Roo.tree.TreeFilter.prototype = {
    ...
    filter : function(A, B, C){
        ...
        if(typeof  A == "string"){
            ...
            A = A.toLowerCase();
            f = function(n){
                return  n.attributes[B].substr(0, D).toLowerCase() == A;
            };
        }else  if(A.exec){ 
            f = function(n){
                return  A.test(n.attributes[B]);
            };
        }else {
            throw  'Illegal filter type, must be string or regex';
        }

        this.filterBy(f, null, C);
	},

    

    filterBy : function(fn, A, B){
        B = B || this.tree.root;
        if(this.autoClear){
            this.clear();
        }
...
Roo.extend(Roo.tree.TreeDropZone, Roo.dd.DropZone, {
    ...
    isValidDropPoint : function(n, pt, dd, e, A){
        if(!n || !A){ return  false; }
        var  B = n.node;
        var  C = A.node;
        ...
        if(pt == "append" && B.allowChildren === false){
            return  false;
        }
        if((pt == "above" || pt == "below") && (B.parentNode && B.parentNode.allowChildren === false)){
            return  false;
        }
        if(C && (B == C || C.contains(B))){
            return  false;
        }
        
        var  D = this.dragOverData;
        D.tree = this.tree;
        D.target = B;
        D.data = A;
        D.point = pt;
        ...
        D.dropNode = C;
        D.cancel = false;  
        var  E = this.tree.fireEvent("nodedragover", D);
        return  D.cancel === false && E !== false;
    },
    ...
    getDropPoint : function(e, n, dd){
        ...
        var  A = n.ddel;
        var  t = Roo.lib.Dom.getY(A), b = t + A.offsetHeight;
        var  y = Roo.lib.Event.getPageY(e);
        
        
        
        var  B = tn.allowChildren === false;
        if(this.appendOnly || tn.parentNode.allowChildren === false){
            return  B ? false : "append";
        }
        var  C = false;
        if(!this.allowParentInsert){
            C = tn.hasChildNodes() && tn.isExpanded();
        }
        var  q = (b - t) / (B ? 2 : 3);
        if(y >= t && y < (t + q)){
            return  "above";
        }else  if(!C && (B || y >= b-q && y <= b)){
            return  "below";
        }else {
            return  "append";
        }
    ...
    onNodeOver : function(n, dd, e, A){
        ...
        if(!this.expandProcId && pt == "append" && B.hasChildNodes() && !n.node.isExpanded()){
            this.queueExpand(B);
        }else  if(pt != "append"){
            this.cancelExpand();
        }
        ...
        var  C = this.dropNotAllowed;
        if(this.isValidDropPoint(n, pt, dd, e, A)){
           if(pt){
               var  el = n.ddel;
               var  D;
               if(pt == "above"){
                   C = n.node.isFirst() ? "x-tree-drop-ok-above" : "x-tree-drop-ok-between";
                   D = "x-tree-drag-insert-above";
               }else  if(pt == "below"){
                   C = n.node.isLast() ? "x-tree-drop-ok-below" : "x-tree-drop-ok-between";
                   D = "x-tree-drag-insert-below";
               }else {
                   C = "x-tree-drop-ok-append";
                   D = "x-tree-drag-append";
               }
               if(this.lastInsertClass != D){
                   Roo.fly(el).replaceClass(this.lastInsertClass, D);
                   this.lastInsertClass = D;
               }
           }
       }
       return  C;
    ...
    onNodeDrop : function(n, dd, e, A){
        var  B = this.getDropPoint(e, n, dd);
        var  C = n.node;
        C.ui.startDrop();
        if(!this.isValidDropPoint(n, B, dd, e, A)){
            C.ui.endDrop();
            return  false;
        }
        
        var  D = A.node || (dd.getTreeNode ? dd.getTreeNode(A, C, B, e) : null);
        var  E = {
            tree : this.tree,
            target: C,
            data: A,
        ...
        var  F = this.tree.fireEvent("beforenodedrop", E);
        if(F === false || E.cancel === true || !E.dropNode){
            C.ui.endDrop();
            return  false;
        }
        ...
        C = E.target;
        if(B == "append" && !C.isExpanded()){
            C.expand(false, null, function(){
                this.completeDrop(E);
            }.createDelegate(this));
        }else {
            this.completeDrop(E);
    ...
    afterNodeMoved : function(dd, A, e, B, C){
        if(this.tree.hlDrop){
            C.ui.focus();
            C.ui.highlight();
        }

        this.tree.fireEvent("nodedrop", this.tree, B, A, dd, e);
    },
...
Roo.extend(Roo.tree.TreeDragZone, Roo.dd.DragZone, {
    ...
    onInitDrag : function(e){
        ...
        this.tree.fireEvent("startdrag", this.tree, A.node, e);
    },
    ...
    onEndDrag : function(A, e){
        this.tree.fireEvent("enddrag", this.tree, A.node, e);
    },
...
5);
    ...
    beforeNodeClick : function(A, e){
        var  B = (this.lastClick ? this.lastClick.getElapsed() : 0);
        this.lastClick = new  Date();
        if(B > this.editDelay && this.tree.getSelectionModel().isSelected(A)){
            e.stopEvent();
            this.triggerEdit(A);
            return  false;
        }
...
Roo.tree.ColumnNodeUI = Roo.extend(Roo.tree.TreeNodeUI, {
    ...
    removeClass : function(A){
        if(this.elRow){
            Roo.fly(this.elRow).removeClass(A);
        }
    }
...
Roo.extend(Roo.tree.ColumnTree, Roo.tree.TreePanel, {
    ...
    onResize : function(w,h)
    {
        ...
        if (C) {
            C.setWidth(  ((w - B)-this.borderWidth - 20));
        }
...
Roo.extend(Roo.menu.Menu, Roo.util.Observable, {
    ...
    tryActivate : function(A, B){
        var  C = this.items;
        for(var  i = A, D = C.length; i >= 0 && i < D; i+= B){
            var  E = C.get(i);
            if(!E.disabled && E.canActivate){
                this.setActiveItem(E, false);
                return  E;
            }
        }
        return  false;
    ...
    show : function(el, A, B){
        ...
        this.showAt(this.el.getAlignToXY(el, A || this.defaultAlign), B, false);
    },

    

    showAt : function(xy, A, 
_e){
        this.parentMenu = A;
        if(!this.el){
            this.render();
    ...
    add : function(){
        ...
        for(var  i = 0; i < l; i++){
            ...
            if(el.render){ 
                ...
                A = this.addElement(el);
            }else  if(typeof  el == "object"){ 
                A = this.addMenuItem(el);
            }
        }
        return  A;
    },
    ...
    addMenuItem : function(A){
        if(!(A  instanceof  Roo.menu.Item)){
            if(typeof  A.checked == "boolean"){ 
                A = new  Roo.menu.CheckItem(A);
            }else {
                A = new  Roo.menu.Item(A);
            }
        }
    ...
    addText : function(A){
        return  this.addItem(new  Roo.menu.TextItem({ text : A }));
    },
    ...
    insert : function(A, B){
        ...
        if(this.ul){
            var  li = document.createElement("li");
            li.className = "x-menu-list-item";
            this.ul.dom.insertBefore(li, this.ul.dom.childNodes[A]);
            B.render(li, this);
            this.delayAutoWidth();
        }
        return  B;
...
Roo.menu.MenuNav = function(A){
    Roo.menu.MenuNav.superclass.constructor.call(this, A.el);
...
Roo.menu.MenuMgr = function(){
   ...
   function  onShow(m){
       ...
       }else  if(F && F.isVisible()){
          m.getEl().setZIndex(parseInt(F.getEl().getStyle("z-index"), 10) + 3);
       }
   }
   ...
   function  onMouseDown(e){
       if(E.getElapsed() > 50 && B.length > 0 && !e.getTarget(".x-menu")){
           hideAll();
       }
   }

   
   function  onBeforeCheck(mi, F){
       if(F){
           var  g = C[mi.group];
           for(var  i = 0, l = g.length; i < l; i++){
               if(g[i] != mi){
   ...
   return  {
       ...
       register : function(F){
           ...
           A[F.id] = F;
           F.on("beforehide", onBeforeHide);
           F.on("hide", onHide);
           F.on("beforeshow", onBeforeShow);
           F.on("show", onShow);
           var  g = F.group;
           if(g && F.events["checkchange"]){
               if(!C[g]){
                   C[g] = [];
               }

               C[g].push(F);
               F.on("checkchange", onCheck);
           }
       },
       ...
       get : function(F){
           if(typeof  F == "string"){ 
               ...
               return  new  Roo.menu.Menu({items:F});
           }else { 
               return  new  Roo.menu.Menu(F);
           }
       },
       ...
       unregister : function(F){
           delete  A[F.id];
           F.un("beforehide", onBeforeHide);
           F.un("hide", onHide);
           F.un("beforeshow", onBeforeShow);
           F.un("show", onShow);
           var  g = F.group;
           if(g && F.events["checkchange"]){
               C[g].remove(F);
               F.un("checkchange", onCheck);
           }
       },

       
       registerCheckable : function(F){
           var  g = F.group;
           if(g){
               if(!C[g]){
                   C[g] = [];
               ...
               C[g].push(F);
               F.on("beforecheckchange", onBeforeCheck);
           }
       },

       
       unregisterCheckable : function(F){
           var  g = F.group;
           if(g){
               C[g].remove(F);
               F.un("beforecheckchange", onBeforeCheck);
           }
       }
   };
}();
...
Roo.extend(Roo.menu.BaseItem, Roo.Component, {
    ...
    render : function(A, B){
        this.parentMenu = B;
        Roo.menu.BaseItem.superclass.render.call(this, A);
...
Roo.extend(Roo.menu.Item, Roo.menu.BaseItem, {
    ...
    onRender : function(A, B){
        ...
        var  C = this.html.length ? this.html  : String.format('{0}',this.text);
        
        el.innerHTML = String.format(
                '<img src="{0}" class="x-menu-item-icon {1}" />' + C,
                this.icon || Roo.BLANK_IMAGE_URL, this.iconCls || '');
        this.el = el;
        Roo.menu.Item.superclass.onRender.call(this, A, B);
    },
    ...
    setText : function(A, B){
        ...
        if(this.rendered){
            ...
            this.el.update(String.format(
                '<img src="{0}" class="x-menu-item-icon {2}">' + C,
...
Roo.extend(Roo.menu.CheckItem, Roo.menu.Item, {
    ...
    setChecked : function(A, B){
        if(this.checked != A && this.fireEvent("beforecheckchange", this, A) !== false){
            if(this.container){
                this.container[A ? "addClass" : "removeClass"]("x-menu-item-checked");
            }

            this.checked = A;
            if(B !== true){
                this.fireEvent("checkchange", this, A);
            }
        }
    },
...
Roo.extend(Roo.menu.DateItem, Roo.menu.Adapter, {
    ...
    onSelect : function(A, B){
        this.fireEvent("select", this, B, A);
...
Roo.extend(Roo.form.Field, Roo.BoxComponent,  {
    ...
    onRender : function(ct, A){
        Roo.form.Field.superclass.onRender.call(this, ct, A);
        if(!this.el){
    ...
    adjustWidth : function(A, w){
        A = A.toLowerCase();
        if(typeof  w == 'number' && Roo.isStrict && !Roo.isSafari){
            if(Roo.isIE && (A == 'input' || A == 'textarea')){
                if(A == 'input'){
                    return  w + 2;
                }
                if(A = 'textarea'){
...
Roo.extend(Roo.form.TextField, Roo.form.Field,  {
    ...
    selectText : function(A, B){
        var  v = this.getRawValue();
        if(v.length > 0){
            A = A === undefined ? 0 : A;
            B = B === undefined ? v.length : B;
            var  d = this.el.dom;
            if(d.setSelectionRange){
                ...
                C.moveEnd("character", v.length-B);
                C.select();
            }
        }
    },
...
Roo.extend(Roo.form.TriggerField, Roo.form.TextField,  {
    ...
    onRender : function(ct, A){
        Roo.form.TriggerField.superclass.onRender.call(this, ct, A);
...
Roo.form.TwinTriggerField = Roo.extend(Roo.form.TriggerField, {
    ...
    initTrigger : function(){
        ...
        var  A = this;
        ts.each(function(t, B, C){
            t.hide = function(){
                var  w = A.wrap.getWidth();
                this.dom.style.display = 'none';
                A.el.setWidth(w-A.trigger.getWidth());
            };
            t.show = function(){
                var  w = A.wrap.getWidth();
            ...
            t.on("click", this['on'+D+'Click'], this, {preventDefault:true});
...
Roo.extend(Roo.form.TextArea, Roo.form.TextField,  {
    ...
    onRender : function(ct, A){
        ...
        Roo.form.TextArea.superclass.onRender.call(this, ct, A);
        if(this.grow){
...
Roo.extend(Roo.form.NumberField, Roo.form.TextField,  {
    ...
    initEvents : function(){
        ...
        var  B = function(e){
            ...
            var  c = e.getCharCode();
            if(A.indexOf(String.fromCharCode(c)) === -1){
                e.stopEvent();
            }
        };
    ...
    validateValue : function(A){
        if(!Roo.form.NumberField.superclass.validateValue.call(this, A)){
            return  false;
        }
        if(A.length < 1){ 
             return  true;
        }
        var  B = this.parseValue(A);
        if(isNaN(B)){
            this.markInvalid(String.format(this.nanText, A));
            return  false;
        }
        if(B < this.minValue){
    ...
    fixPrecision : function(A){
        var  B = isNaN(A);
        if(!this.allowDecimals || this.decimalPrecision == -1 || B || !A){
            return  B ? '' : A;
        }
        return  parseFloat(A).toFixed(this.decimalPrecision);
    },
...
Roo.extend(Roo.form.DateField, Roo.form.TriggerField,  {
    ...
    onRender : function(ct, A)
    {
        Roo.form.DateField.superclass.onRender.call(this, ct, A);
        if (this.useIso) {
    ...
    {
        A = this.formatDate(A);
        if(!Roo.form.DateField.superclass.validateValue.call(this, A)){
            return  false;
        }
        if(A.length < 1){ 
             return  true;
        ...
        A = this.parseDate(A);
        if(!A){
            this.markInvalid(String.format(this.invalidText, B, this.format));
            return  false;
        }
        var  C = A.getTime();
        if(this.minValue && C < this.minValue.getTime()){
            this.markInvalid(String.format(this.minText, this.formatDate(this.minValue)));
            return  false;
        }
        if(this.maxValue && C > this.maxValue.getTime()){
        ...
        if(this.disabledDays){
            ...
            for(var  i = 0; i < this.disabledDays.length; i++) {
            	if(D === this.disabledDays[i]){
            	    this.markInvalid(this.disabledDaysText);
                    return  false;
            	}
            }
        }
        var  E = this.formatDate(A);
        if(this.ddMatch && this.ddMatch.test(E)){
            this.markInvalid(String.format(this.disabledDatesText, E));
            return  false;
        }
        return  true;
    },
    ...
    setValue : function(A){
        if (this.hiddenField) {
            this.hiddenField.value = this.formatDate(this.parseDate(A), 'Y-m-d');
        }

        Roo.form.DateField.superclass.setValue.call(this, this.formatDate(this.parseDate(A)));
    },
    ...
    parseDate : function(A){
        ...
        var  v = Date.parseDate(A, this.format);
        if(!v && this.altFormats){
            if(!this.altFormatsArray){
                this.altFormatsArray = this.altFormats.split("|");
            }
            for(var  i = 0, B = this.altFormatsArray.length; i < B && !v; i++){
                v = Date.parseDate(A, this.altFormatsArray[i]);
            }
        }
        return  v;
    },
...
Roo.form.ComboBox = function(A){
    ...
    if(this.transform){
        ...
        if(!this.store){
            ...
            for(var  i = 0, C = B.length;i < C; i++){
                var  o = B[i];
                var  D = (Roo.isIE ? o.getAttributeNode('value').specified : o.hasAttribute('value')) ? o.value : o.text;
                if(o.selected) {
                    this.value = D;
                }
...
Roo.extend(Roo.form.ComboBox, Roo.form.TriggerField, {
    ...
    onRender : function(ct, A){
        Roo.form.ComboBox.superclass.onRender.call(this, ct, A);
        if(this.hiddenName){
        ...
        this.list = new  Roo.Layer({
            shadow: this.shadow, cls: [B, this.listClass].join(' '), constrain:false
        });
    ...
    initEvents : function(){
        ...
        this.keyNav = new  Roo.KeyNav(this.el, {
            ...
            doRelay : function(A, B, C){
                if(C == 'down' || this.scope.isExpanded()){
    ...
    onTypeAhead : function(){
        if(this.store.getCount() > 0){
            ...
            var  A = r.data[this.displayField];
            var  B = A.length;
            var  C = this.getRawValue().length;
            if(C != B){
                this.setRawValue(A);
                this.selectText(C, A.length);
            }
        }
    },
    ...
    onSelect : function(A, B){
        if(this.fireEvent('beforeselect', this, A, B) !== false){
            this.setFromData(B > -1 ? A.data : false);
            this.collapse();
    ...
    setValue : function(v){
        ...
        if(this.valueField){
            ...
            if(r){
                A = r.data[this.displayField];
            }else  if(this.valueNotFoundText !== undefined){
                A = this.valueNotFoundText;
            }
        }

        this.lastSelectionText = A;
        if(this.hiddenField){
            this.hiddenField.value = v;
        }

        Roo.form.ComboBox.superclass.setValue.call(this, A);
        this.value = v;
    },
    ...
    onViewOver : function(e, t){
        ...
        var  A = this.view.findItemFromChild(t);
        if(A){
            var  B = this.view.indexOf(A);
            this.select(B, false);
        }
    },
    ...
    onViewClick : function(A){
        ...
        var  r = this.store.getAt(B);
        if(r){
            this.onSelect(r, B);
        }
        if(A !== false && !this.blockFocus){
            this.el.focus();
        }
    },
    ...
    restrictHeight : function(){
        ...
        var  h = Math.max(A.clientHeight, A.offsetHeight, A.scrollHeight);
    ...
    selectByValue : function(v, A){
        if(v !== undefined && v !== null){
            var  r = this.findRecord(this.valueField || this.displayField, v);
            if(r){
                this.select(this.store.indexOf(r), A);
                return  true;
            }
        }
        return  false;
    ...
    doQuery : function(q, A){
        ...
        q = qe.query;
        A = qe.forceAll;
        if(A === true || (q.length >= this.minChars)){
    ...
    listKeyPress : function(e)
    {
        ...
        var  A  = false;
        var  B = this.view.getSelectedNodes();
        var  C = false;
        if (B.length) {
            var  ix = this.view.indexOf(B[0]);
            C  = this.store.getAt(ix);
            if (!C.get(this.displayField) || C.get(this.displayField).substring(0,1).toUpperCase() != k) {
                C = false;
            }
...
Roo.extend(Roo.form.Checkbox, Roo.form.Field,  {
    ...
    onRender : function(ct, A){
        Roo.form.Checkbox.superclass.onRender.call(this, ct, A);
        

        
        this.wrap = this.el.wrap({cls: 'x-menu-check-item '});
        var  B = this.wrap.createChild({ 
    ...
    setValue : function(v,A){
        ...
        this.setChecked(String(v) === String(this.inputValue), A);
    ...
    setChecked : function(A,B)
    {
        ...
        if(this.wrap){
            this.wrap[A ? 'addClass' : 'removeClass']('x-menu-item-checked');
        }

        this.checked = A;
        if(B !== true){
            this.fireEvent('check', this, A);
        }

        this.inSetChecked = true;
        this.el.dom.value = A ? this.inputValue : this.valueOff;
...
Roo.form.HtmlEditor = Roo.extend(Roo.form.Field, {
    ...
    createToolbar : function(A){
        ...
        for (var  i =0 ; i < A.toolbars.length;i++) {
            A.toolbars[i] = Roo.factory(A.toolbars[i], Roo.form.HtmlEditor);
            A.toolbars[i].init(A);
        }
    ...
    {
        var  _t = this;
        Roo.form.HtmlEditor.superclass.onRender.call(this, ct, A);
        ...
        if(this.el && this.iframe){
            ...
            if(typeof  h == 'number'){
                ...
                for (var  i =0; i < this.toolbars.length;i++) {
                    ...
                    A += this.toolbars[i].tb.el.getHeight();
                }
                
                
                
                
                var  ah = h - this.wrap.getFrameWidth('tb') - A;
                ah -= 10; 
    ...
    syncValue : function(){
        if(this.initialized){
            ...
            if(Roo.isSafari){
                ...
                if(m && m[1]){
                    A = '<div style="'+m[0]+'">' + A + '</div>';
                }
            }

            A = this.cleanHtml(A);
            if(this.fireEvent('beforesync', this, A) !== false){
                this.el.dom.value = A;
    ...
    assignDocWin: function()
    {
        var  A = this.iframe;
         ...
         if(Roo.isIE){
            this.doc = A.contentWindow.document;
            ...
            this.doc = (A.contentDocument || Roo.get(this.frameId).dom.document);
    ...
    initEditor : function(){
        ...
        var  A = (this.doc.body || this.doc.documentElement);
        ...
        Roo.DomHelper.applyStyles(A, ss);
    ...
    adjustFont: function(A){
        var  B = A.cmd == 'increasefontsize' ? 1 : -1;
    ...
    insertText : function(A)
    {
        ...
        range.insertNode(this.doc.createTextNode(A));
    } ,
    ...
    execCmd : function(A, B){
        this.doc.execCommand(A, false, B === undefined ? null : B);
        this.syncValue();
    },
    ...
    getSelectedNode: function() 
    {
        ...
        var  A = this.createRange(this.getSelection());
        ...
        if (Roo.isIE) {
            ...
            while (true) {
                ...
                if ((B.nodeType != 1) || (B.tagName.toLowerCase() == 'body')) {
                    break;
                }
        ...
        var  ar = A.endContainer.childNodes;
        if (!ar.length) {
            ar = A.commonAncestorContainer.childNodes;
        ...
        for (var  i=0;i<ar.length;i++) {
            ...
            if (this.rangeIntersectsNode(A,ar[i]) && this.rangeCompareNode(A,ar[i]) == 3) {
                D.push(ar[i]);
                continue;
            }
            
            
            if ((ar[i].nodeType == 1) && this.rangeIntersectsNode(A,ar[i]) && (this.rangeCompareNode(A,ar[i]) > 0)) {
                E.push(ar[i]);
                continue;
            }
            if (!this.rangeIntersectsNode(A,ar[i])|| (this.rangeCompareNode(A,ar[i]) == 0))  {
                continue;
            }
    ...
    createRange: function(A)
    {
        ...
        if (typeof  A != "undefined") {
            try {
                return  A.getRangeAt ? A.getRangeAt(0) : A.createRange();
            } catch(e) {
    ...
    getParentElement: function()
    {
        ...
        this.assignDocWin();
        var  A = Roo.isIE ? this.doc.selection : this.win.getSelection();
        
        var  B = this.createRange(A);
         
        try {
            var  p = B.commonAncestorContainer;
            while (p.nodeType == 3) { 
                p = p.parentNode;
            }
            return  p;
    ...
    rangeIntersectsNode : function(A, B)
    {
        ...
        return  A.compareBoundaryPoints(Range.END_TO_START, C) == -1 &&
                 A.compareBoundaryPoints(Range.START_TO_END, C) == 1;
    },
    rangeCompareNode : function(A, B) {
        var  C = B.ownerDocument.createRange();
        try {
            C.selectNode(B);
        } catch (e) {
        ...
        var  D = A.compareBoundaryPoints(Range.START_TO_START, C) == 1;
        var  E = A.compareBoundaryPoints(Range.END_TO_END, C) == -1;
    ...
    cleanUpChild : function (A)
    {
        ...
        if (Roo.form.HtmlEditor.black.indexOf(A.tagName.toLowerCase()) > -1) {
            
            A.parentNode.removeChild(A);
            return;
            
        }
        if (Roo.form.HtmlEditor.remove.indexOf(A.tagName.toLowerCase()) > -1) {
            this.cleanUpChildren(A);
            
            while (A.childNodes.length) {
                var  cn = A.childNodes[0];
                A.removeChild(cn);
                A.parentNode.insertBefore(cn, A);
            }
        ...
        function  cleanStyle(n,v)
        {
            ...
            Roo.each(B, function(p) {
                ...
                if (Roo.form.HtmlEditor.cwhite.indexOf(l) < 0) {
                    Roo.log('(REMOVE)' + A.tagName +'.' + n + ':'+l + '=' + v);
                    A.removeAttribute(n);
                    return  false;
                }
                return  true;
...
Roo.apply(Roo.form.HtmlEditor.ToolbarStandard.prototype,  {
    ...
    init : function(A)
    {
        ...
        function  btn(id, E, F){
            ...
            return  {
                ...
                scope: A, 
                handler:F||A.relayBtnCmd,
                clickEvent:'mousedown',
                tooltip: C.buttonTips[id] || undefined, 
                tabIndex:-1
            };
        }
        ...
        var  tb = new  Roo.Toolbar(A.wrap.dom.firstChild);
        this.tb = tb;
        ...
        if(!this.disable.fontSize){
            tb.add(
                ...
                btn('increasefontsize', false, A.adjustFont),
                btn('decreasefontsize', false, A.adjustFont)
            );
        };
            ...
            tb.add(
                '-', {
                    ...
                    menu : new  Roo.menu.ColorMenu({
                        ...
                        value:'000000',
                        plain:true,
                        selectHandler: function(cp, E){
                            A.execCmd('forecolor', Roo.isSafari || Roo.isIE ? '#'+E : E);
                            A.deferFocus();
                        },
                        scope: A,
                        clickEvent:'mousedown'
                    ...
                    menu : new  Roo.menu.ColorMenu({
                        ...
                        selectHandler: function(cp, E){
                            if(Roo.isGecko){
                                A.execCmd('useCSS', false);
                                A.execCmd('hilitecolor', E);
                                A.execCmd('useCSS', true);
                                A.deferFocus();
                            }else {
                                A.execCmd(Roo.isOpera ? 'hilitecolor' : 'backcolor', 
                                    Roo.isSafari || Roo.isIE ? '#'+E : E);
                                A.deferFocus();
                            }
                        },
                        scope:A,
        ...
        if(!this.disable.alignments){
            ...
            if(!this.disable.links){
                tb.add(
                    '-',
                    btn('createlink', false, A.createLink)    
                );
            };
            ...
            if(!this.disable.sourceEdit){
                tb.add(
                    '-',
                    btn('sourceedit', true, function(E){
            ...
            for (var  i =0; i < this.specialChars.length; i++) {
                D.menu.items.push({
                    ...
                    handler: function(a,b) {
                        A.insertAtCursor(String.fromCharCode(a.html.replace('&#','').replace(';', '')));
        ...
        A.on('editorevent', this.updateToolbar, this);
    ...
    updateToolbar: function(){
        ...
        var  A = this.tb.items.map, 
            B = this.editor.doc,
            C = this.editor.frameId;
        ...
        if(!this.disable.format){
            A[C + '-bold'].toggle(B.queryCommandState('bold'));
            A[C + '-italic'].toggle(B.queryCommandState('italic'));
            A[C + '-underline'].toggle(B.queryCommandState('underline'));
        }
        if(!this.disable.alignments){
            A[C + '-justifyleft'].toggle(B.queryCommandState('justifyleft'));
            A[C + '-justifycenter'].toggle(B.queryCommandState('justifycenter'));
            A[C + '-justifyright'].toggle(B.queryCommandState('justifyright'));
        }
        if(!Roo.isSafari && !this.disable.lists){
            A[C + '-insertorderedlist'].toggle(B.queryCommandState('insertorderedlist'));
            A[C + '-insertunorderedlist'].toggle(B.queryCommandState('insertunorderedlist'));
        }
        
        var  D = this.editor.getAllAncestors();
        if (this.formatCombo) {
            
            
            var  E = this.formatCombo.store;
            this.formatCombo.setValue("");
            for (var  i =0; i < D.length;i++) {
                if (D[i] && E.query('tag',D[i].tagName.toLowerCase(), false).length) {
                    
                    this.formatCombo.setValue(D[i].tagName.toLowerCase());
                    break;
                }
            }
        }
    ...
    toggleSourceEdit : function(A){
        ...
        this.sourceEditMode = A === true;
        var  B = this.tb.items.get(this.editor.frameId +'-sourceedit');
        
        if(B.pressed !== this.editor.sourceEditMode){
            B.toggle(this.editor.sourceEditMode);
            return;
        }
...
Roo.apply(Roo.form.HtmlEditor.ToolbarContext.prototype,  {
    ...
    init : function(A)
    {
        ...
        var  D = A.wrap.createChild({
                tag: 'div'
            }, A.wrap.dom.firstChild.nextSibling, true);
    ...
    updateToolbar: function(){
        ...
        var  A = this.editor.getAllAncestors();
        
        
        var  ty= Roo.form.HtmlEditor.ToolbarContext.types;
        var  B = A.length ? (A[0] ?  A[0]  : A[1]) : this.editor.doc.body;
        B = B ? B : this.editor.doc.body;
        B = B.tagName.length ? B : this.editor.doc.body;
        var  tn = B.tagName.toUpperCase();
        B = typeof(ty[tn]) != 'undefined' ? B : this.editor.doc.body;
        tn = B.tagName.toUpperCase();
        if (this.tb.name  == tn) {
            return; 
        }
        ...
        this.tb.fields.each(function(e) {
            e.setValue(B.getAttribute(e.name));
        });
        this.tb.selectedNode = B;
    ...
    buildToolbar: function(A, nm)
    {
        var  B = this.editor;
         
        var  C = B.wrap.createChild({
                tag: 'div'
            }, B.wrap.dom.firstChild.nextSibling, true);
        
       
        var  tb = new  Roo.Toolbar(C);
        tb.add(nm+ ":&nbsp;");
        for (var  i  in  A) {
            var  D = A[i];
            tb.add(D.title + ":&nbsp;");
...
Roo.extend(Roo.form.BasicForm, Roo.util.Observable, {
    ...
    doAction : function(A, B){
        if(typeof  A == 'string'){
            A = new  Roo.form.Action.ACTION_TYPES[A](this, B);
        }
        if(this.fireEvent('beforeaction', this, A) !== false){
            this.beforeAction(A);
            A.run.defer(100, A);
        }
        return  this;
    ...
    updateRecord : function(A){
        ...
        var  fs = A.fields;
        fs.each(function(f){
            var  B = this.findField(f.name);
            if(B){
                A.set(f.name, B.getValue());
            }
        }, this);
        A.endEdit();
        return  this;
    ...
    afterAction : function(A, B){
        ...
        if(B){
            ...
            Roo.callback(o.success, o.scope, [this, A]);
            this.fireEvent('actioncomplete', this, A);
            
        }else {
            Roo.callback(o.failure, o.scope, [this, A]);
    ...
    addForm : function(A)
    {
        ...
        if (this.childForms.indexOf(A) > -1) {
            
            return;
        }

        this.childForms.push(A);
        var  n = '';
        Roo.each(A.allItems, function (fe) {
            
            n = typeof(fe.getName) == 'undefined' ? fe.name : fe.getName();
            if (this.findField(n)) { 
                return;
            }
            var  B = new  Roo.form.Hidden({
                name : n
            });
            B.render(this.el);
    ...
    markInvalid : function(A){
        if(A  instanceof  Array){
            ...
            var  D, id;
            for(id  in  A){
                if(typeof  A[id] != 'function' && (D = this.findField(id))){
                    D.markInvalid(A[id]);
                }
            }
        }
    ...
    setValues : function(A){
        if(A  instanceof  Array){ 
            ...
            var  C, id;
            for(id  in  A){
                if(typeof  A[id] != 'function' && (C = this.findField(id))){
                    ...
                    if (C.setFromData && 
                        ...
                        (C.store && !C.store.isLocal)
                        ) {
                        
                        var  sd = { };
                        sd[C.valueField] = typeof(A[C.hiddenName]) == 'undefined' ? '' : A[C.hiddenName];
                        sd[C.displayField] = typeof(A[C.name]) == 'undefined' ? '' : A[C.name];
                        C.setFromData(sd);
...
Roo.extend(Roo.form.Form, Roo.form.BasicForm, {
    ...
    add : function(){
        ...
        var  r = [];
        for(var  i = 0, a = arguments, A = a.length; i < A; i++) {
            if(a[i].isFormField){
                r.push(a[i]);
            }
        }
    ...
    render : function(ct)
    {
        ...
        if(this.buttons.length > 0){
            ...
            for(var  i = 0, A = this.buttons.length; i < A; i++) {
    ...
    bindHandler : function(){
        ...
        for(var  i = 0, B = this.buttons.length; i < B; i++){
            ...
            if(C.formBind === true && C.disabled === A){
                C.setDisabled(!A);
            }
        }

        this.fireEvent('clientvalidation', this, A);
    }
...
Roo.form.Action.prototype = {
    ...
    getUrl : function(A){
        var  B = this.options.url || this.form.url || this.form.el.dom.action;
        if(A){
...
Roo.form.Action.Submit = function(A, B){
    Roo.form.Action.Submit.superclass.constructor.call(this, A, B);
};
...
Roo.extend(Roo.form.Action.Submit, Roo.form.Action, {
    ...
    uploadProgress : function()
    {
        ...
        var  A = this.form.findField('UPLOAD_IDENTIFIER').getValue();
        ...
        c.request({
            ...
            success : function(B){
                ...
                if (E){
                    Roo.MessageBox.updateProgress(E.bytes_uploaded/E.bytes_total,
                       Math.floor((E.bytes_total - E.bytes_uploaded)/1000) + 'k remaining'
                    );
                }
    ...
    success : function(A)
    {
        ...
        var  B = this.processResponse(A);
        if(B === true || B.success){
            this.form.afterAction(this, true);
            return;
        }
        if(B.errors){
            this.form.markInvalid(B.errors);
    ...
    handleResponse : function(A){
        if(this.form.errorReader){
            var  rs = this.form.errorReader.read(A);
            var  B = [];
            if(rs.records){
                for(var  i = 0, C = rs.records.length; i < C; i++) {
                    var  r = rs.records[i];
                    B[i] = r.data;
                }
            }
...
Roo.form.Action.Load = function(A, B){
    Roo.form.Action.Load.superclass.constructor.call(this, A, B);
...
Roo.extend(Roo.form.Action.Load, Roo.form.Action, {
    ...
    handleResponse : function(A){
        if(this.form.reader){
            var  rs = this.form.reader.read(A);
            var  B = rs.records && rs.records[0] ? rs.records[0].data : null;
            return  {
                success : rs.success,
                data : B
            };
        }
        return  Roo.decode(A.responseText);
    }
});
...
Roo.extend(Roo.form.Layout, Roo.Component, {
    ...
    onRender : function(ct, A){
        ...
        if(D > 0){
            ...
            for(var  i = 0; i < D; i++) {
                if(C[i].isFormField){
                    this.renderField(C[i]);
                }else {
                    this.renderComponent(C[i]);
                }
            }
        }
        if(this.clear){
...
Roo.form.Column = function(A){
    Roo.form.Column.superclass.constructor.call(this, A);
};
...
Roo.extend(Roo.form.Column, Roo.form.Layout, {
    ...
    onRender : function(ct, A){
        Roo.form.Column.superclass.onRender.call(this, ct, A);
        if(this.width){
...
Roo.form.Row = function(A){
    Roo.form.Row.superclass.constructor.call(this, A);
};
...
Roo.form.FieldSet = function(A){
    Roo.form.FieldSet.superclass.constructor.call(this, A);
};
...
Roo.extend(Roo.form.FieldSet, Roo.form.Layout, {
    ...
    onRender : function(ct, A){
        Roo.form.FieldSet.superclass.onRender.call(this, ct, A);
        if(this.legend){
...
Roo.extend(Roo.form.FCKeditor, Roo.form.TextArea,
{
    ...
    onRender : function(ct, A)
    {
        ...
        Roo.form.FCKeditor.superclass.onRender.call(this, ct, A);
    ...
    setValue : function(A)
    {
        ...
        Roo.form.FCKeditor.superclass.setValue.apply(this,[A]);
    ...
    getRawValue : function()
    {
        ...
        var  A=this.getEditor().GetData();
        Roo.form.FCKeditor.superclass.setRawValue.apply(this,[A]);
        return  Roo.form.FCKeditor.superclass.getRawValue.call(this);
    ...
    toggleSourceEdit : function(A) {
        ...
        this.el.dom.style.display = A ? '' : 'none';
        this.frame.dom.style.display = A ?  'none' : '';
    ...
    focus: function(A)
    {
        ...
        var  B = this.getEditor().EditorDocument.getElementsByTagName(A);
    ...
    replaceTextarea : function()
    {
        ...
        var  A = document.getElementById( this.getId() );
        
        var  B = document.getElementsByName( this.getId() ) ;
    ...
    _getConfigHtml : function()
    {
        ...
        for ( var  o  in  this.fckconfig ) {
            A += A.length > 0  ? '&amp;' : '';
            A += encodeURIComponent( o ) + '=' + encodeURIComponent( this.fckconfig[o] ) ;
        }

        return  '<input type="hidden" id="' + this.getId() + '___Config" value="' + A + '" style="display:none" />' ;
    },
    
    
    _getIFrameHtml : function()
    {
        var  A = 'fckeditor.html' ;
        


        var  B = this.basePath + 'editor/' + A + '?InstanceName=' + encodeURIComponent( this.getId() ) ;
        B += this.toolbarSet ? ( '&amp;Toolbar=' + this.toolbarSet)  : '';
        
        
        var  C = '<iframe id="' + this.getId() +
            '___Frame" src="' + B +
            '" width="' + this.width +
            '" height="' + this.height + '"' +
    ...
    _insertHtmlBefore : function( A, B )
    {
...
function  FCKeditor_OnComplete(A){
    var  f = Roo.form.FCKeditor.editors[A.Name];
    f.fckEditor = A;
    
    f.fireEvent('editorinit', f, A);
} 
...
Roo.extend(Roo.form.GridField, Roo.form.Field,  {
    ...
    onRender : function(ct, A){
        ...
        Roo.form.GridField.superclass.onRender.call(this, ct, A);
        this.wrap = this.el.wrap({cls: ''}); 
        this.viewEl = this.wrap.createChild({ tag: 'div' });
        if (B) {
            this.viewEl.applyStyles(B);
        }
        if (this.width) {
    ...
    setValue : function(v){
        ...
        if (this.grid && this.grid.getDataSource() && typeof(v) != 'undefined') {
            ...
            A[ds.reader.meta.root ] =  typeof(v) == 'string' ? Roo.decode(v) : v;
            ds.loadData( A);
        }
...
Roo.form.DisplayField = function(A){
    Roo.form.DisplayField.superclass.constructor.call(this, A);
...
Roo.extend(Roo.form.DisplayField, Roo.form.TextField,  {
    ...
    onRender : function(ct, A){
        ...
        Roo.form.DisplayField.superclass.onRender.call(this, ct, A);
    ...
    setValue : function(v){
        this.value = v;
        var  A = this.valueRenderer ?  this.valueRenderer(v) : String.format('{0}', v);
        
        if (!this.viewEl) {
            return;
        }

        this.viewEl.dom.innerHTML = A;
...
Roo.form.DayPicker= function(A){
    Roo.form.DayPicker.superclass.constructor.call(this, A);
...
Roo.extend(Roo.form.DayPicker, Roo.form.Field,  {
    ...
    onRender : function(ct, A){
        ...
        var  B = this.wrap.createChild( r1 + '</tr>' + r2 + '</tr></table>');
        B.select('img').on('click', this.onClick, this);
        this.viewEl = B;   
    ...
    refreshValue : function()
    {
        ...
        this.viewEl.select('img',true).each(function(e,i,n)  {
            A += e.is(".x-menu-item-checked") ? String(n) : '';
        });
    ...
    setValue : function(v,A){
        ...
        this.fireEvent('change', this, v, B);
...
Roo.extend(Roo.DDView, Roo.View, {
	...
	setValue: function(v) {
		...
		var  A = {};
		A[this.store.reader.meta.root] = v ? [].concat(v) : [];
		this.store.proxy = new  Roo.data.MemoryProxy(A);
		this.store.load();
	},
	...
	getIds: function() {
		var  i = 0, A = new  Array(this.store.getCount());
		this.store.each(function(B) {
			A[i++] = B.id;
		});
		return  A;
    ...
    getDragData : function(e) {
		...
		if(A) {
			...
			var  D = this.getSelectedIndexes();
			for (var  i = 0; i < D.length; i++) {
				C.records.push(this.store.getAt(D[i]));
			}
			if (B.length == 1) {
				C.ddel = A.cloneNode(true);	
			} else  {
				var  E = document.createElement('div'); 
				E.className = 'multi-proxy';
				for (var  i = 0, F = B.length; i < F; i++) {
					E.appendChild(B[i].cloneNode(true));
				}
    ...
    setDraggable: function(A) {
    	if (A  instanceof  Array) {
    		Roo.each(A, this.setDraggable, this);
    		return;
    	}
    	if (this.dragZone) {
    ...
    setDroppable: function(A) {
    	if (A  instanceof  Array) {
    		Roo.each(A, this.setDroppable, this);
    		return;
    	}
    	if (this.dropZone) {
    ...
    onNodeOver : function(n, dd, e, A){
		...
		if (pt) {
			var  C;
			if (pt == "above"){
				B = n.previousSibling ? "x-tree-drop-ok-between" : "x-tree-drop-ok-above";
				C = "x-view-drag-insert-above";
			} else  {
				B = n.nextSibling ? "x-tree-drop-ok-between" : "x-tree-drop-ok-below";
				C = "x-view-drag-insert-below";
			}
			if (this.lastInsertClass != C){
				Roo.fly(n).replaceClass(this.lastInsertClass, C);
    ...
    onNodeDrop : function(n, dd, e, A){
    	if (this.fireEvent("drop", this, n, dd, e, A) === false) {
    		return  false;
    	}
    	var  pt = this.getDropPoint(e, n, dd);
		var  B = (n == this.el.dom) ? this.nodes.length : n.nodeIndex;
		if (pt == "below") { B++; }
		for (var  i = 0; i < A.records.length; i++) {
			var  r = A.records[i];
			var  C = this.store.getById(r.id);
			if (C && (dd != this.dragZone)) {
				Roo.fly(this.getNode(this.store.indexOf(C))).frame("red", 1);
			} else  {
				if (A.copy) {
					this.store.insert(B++, r.copy());
				} else  {
					A.source.isDirtyFlag = true;
					r.store.remove(r);
					this.store.insert(B++, r);
				}
	...
	setDeletable: function(A) {
		...
		this.contextMenu.on("itemclick", function(B) {
			switch (B.id) {
				case  "delete":
	...
	showContextMenu: function(e, A) {
        A = this.findItemFromChild(e.getTarget());
		if (A) {
			e.stopEvent();
			this.select(this.getNode(A), this.multiSelect && e.ctrlKey, true);
    ...
    onDblClick : function(e){
        var  A = this.findItemFromChild(e.getTarget());
        if(A){
            if (this.fireEvent("dblclick", this, this.indexOf(A), A, e) === false) {
            	return  false;
            }
            if (this.dragGroup) {
	            var  B = Roo.dd.DragDropMgr.getRelated(this.dragZone, true);
	            while (B.indexOf(this.dropZone) > -1) {
		            B.remove(this.dropZone);
				}
	            if (B.length == 1) {
					this.dragZone.cachedTarget = null;
	            	var  el = Roo.get(B[0].getEl());
	            	var  C = el.getBox(true);
	            	B[0].onNodeDrop(el.dom, {
	            		target: el.dom,
    ...
    handleSelection: function(e) {
		...
		if (A && (this.multiSelect || this.singleSelect)){
			if(this.multiSelect && e.shiftKey && (!e.ctrlKey) && this.lastSelection){
				this.select(this.getNodes(this.indexOf(this.lastSelection), A.nodeIndex), false);
			}else  if (this.isSelected(this.getNode(A)) && e.ctrlKey){
				this.unselect(A);
			} else  {
				this.select(A, this.multiSelect && e.ctrlKey);
				this.lastSelection = A;
    ...
    unselect : function(A, B){
		...
		if(C && this.isSelected(C)){
			if(this.fireEvent("beforeselect", this, C, this.selections) !== false){
				Roo.fly(C).removeClass(this.selectedClass);
...
Roo.extend(Roo.LayoutManager, Roo.util.Observable, {
    ...
    onRegionResized : function(A, B){
        this.fireEvent("regionresized", A, B);
        this.layout();
    },
    ...
    getViewSize : function(){
        ...
        if(this.el.dom != document.body){
            ...
            A = {width: Roo.lib.Dom.getViewWidth(), height: Roo.lib.Dom.getViewHeight()};
        }
...
Roo.BorderLayout = function(A, B){
    ...
    for(var  i = 0, C = this.factory.validRegions.length; i < C; i++) {
    	var  D = this.factory.validRegions[i];
    	if(B[D]){
    	    this.addRegion(D, B[D]);
    	}
    }
};
...
Roo.extend(Roo.BorderLayout, Roo.LayoutManager, {
    ...
    addRegion : function(A, B){
        if(!this.regions[A]){
            var  r = this.factory.create(A, this, B);
    	    this.bindRegion(A, r);
        }
    ...
    layout : function(){
        ...
        if(F && F.isVisible()){
            ...
            b.x = m.left;
            b.y = m.top;
            D = b.height + b.y + m.bottom;
            C -= D;
            F.updateBox(this.safeBox(b));
        }
        if(G && G.isVisible()){
            var  b = G.getBox();
            var  m = G.getMargins();
            b.width = w - (m.left+m.right);
            b.x = m.left;
            var  K = (b.height + m.top + m.bottom);
            b.y = h - K + m.top;
            C -= K;
            G.updateBox(this.safeBox(b));
        }
        if(H && H.isVisible()){
            var  b = H.getBox();
            var  m = H.getMargins();
            b.height = C - (m.top+m.bottom);
            b.x = m.left;
            b.y = D + m.top;
            var  L = (b.width + m.left + m.right);
            E += L;
            B -= L;
            H.updateBox(this.safeBox(b));
        }
        if(I && I.isVisible()){
            var  b = I.getBox();
            var  m = I.getMargins();
        ...
        if(J){
            ...
            var  M = {
                x: E + m.left,
                y: D + m.top,
                width: B - (m.left+m.right),
                height: C - (m.top+m.bottom)
            };
            
                
            
            J.updateBox(this.safeBox(M));
        }
    ...
    remove : function(A, B){
        A = A.toLowerCase();
        return  this.regions[A].remove(B);
    },

    

    findPanel : function(A){
        var  rs = this.regions;
        for(var  B  in  rs){
            if(typeof  rs[B] != "function"){
                var  p = rs[B].getPanel(A);
                if(p){
                    return  p;
                }
            }
    ...
    batchAdd : function(A){
        this.beginUpdate();
        for(var  B  in  A){
            var  lr = this.regions[B];
            if(lr){
                this.addTypedPanels(lr, A[B]);
            }
        }
    ...
    addxtype : function(A)
    {
        ...
        switch(A.xtype) 
        {
            ...
            case  'NestedLayoutPanel': 
                ...
                if (C == 'center' && this.active && this.getRegion('center').panels.length < 1) {
                    A.background = false;
                }
                var  F = new  Roo.BorderLayout(el, E);
                
                B = new  Roo[A.xtype](F, A); 
            ...
            case  'GridPanel': 
                ...
                var  G = new  Roo.grid[A.grid.xtype](el, A.grid);
                delete  A.grid;
                if (C == 'center' && this.active ) {
                    A.background = false;
                }

                B = new  Roo[A.xtype](G, A); 
                
                this.add(C, B);
                if (A.background) {
                    B.on('activate', function(gp) {
                        if (!gp.grid.rendered) {
                            gp.grid.render();
                        }
                    });
            ...
            default: 
                alert("Can not add '" + A.xtype + "' to BorderLayout");
                return  null;
...
Roo.BorderLayout.create = function(A, B){
    var  C = new  Roo.BorderLayout(B || document.body, A);
    C.beginUpdate();
    var  D = Roo.BorderLayout.RegionFactory.validRegions;
    for(var  j = 0, E = D.length; j < E; j++){
        var  lr = D[j];
        if(C.regions[lr] && A[lr].panels){
            var  r = C.regions[lr];
            var  ps = A[lr].panels;
            C.addTypedPanels(r, ps);
        }
    }
...
Roo.BorderLayout.RegionFactory = {
    ...
    create : function(A, B, C){
        A = A.toLowerCase();
        if(C.lightweight || C.basic){
            return  new  Roo.BasicLayoutRegion(B, C, A);
        }
        switch(A){
            case  "north":
                return  new  Roo.NorthLayoutRegion(B, C);
            case  "south":
                return  new  Roo.SouthLayoutRegion(B, C);
            case  "east":
                return  new  Roo.EastLayoutRegion(B, C);
            case  "west":
                return  new  Roo.WestLayoutRegion(B, C);
            case  "center":
                return  new  Roo.CenterLayoutRegion(B, C);
        }
...
Roo.extend(Roo.BasicLayoutRegion, Roo.util.Observable, {
    ...
    updateBox : function(A){
        ...
        el.dom.style.left = A.x + "px";
        el.dom.style.top = A.y + "px";
    ...
    remove : function(A, B){
        ...
        var  e = {};
        this.fireEvent("beforeremove", this, A, e);
        if(e.cancel === true){
            return  null;
        }
        var  C = A.getId();
...
Roo.extend(Roo.LayoutRegion, Roo.BasicLayoutRegion, {
    ...
    initPanelAsTab : function(A){
        var  ti = this.tabs.addTab(A.getEl().id, A.getTitle(), null,
                    this.config.closeOnTab && A.isClosable());
        if(A.tabTip !== undefined){
            ti.setTooltip(A.tabTip);
        }
    ...
    updatePanelTitle : function(A, B){
        if(this.activePanel == A){
            this.updateTitle(B);
        }
        if(this.tabs){
            var  ti = this.tabs.getTab(A.getEl().id);
            ti.setText(B);
            if(A.tabTip !== undefined){
                ti.setTooltip(A.tabTip);
            }
    ...
    add : function(A){
        ...
        if(this.panels.getCount() == 1 && !this.config.alwaysShowTabs){
            this.bodyEl.dom.appendChild(A.getEl().dom);
        ...
        if(!this.tabs){
            ...
            this.initPanelAsTab(A);
        }
        if(A.background !== true){
            this.tabs.activate(A.getEl().id);
        }
    ...
    remove : function(A, B){
        ...
        B = (typeof  B != "undefined" ? B : (this.config.preservePanels === true || A.preserve === true));
        var  C = A.getId();
        this.panels.removeKey(C);
        if(B){
            document.body.appendChild(A.getEl().dom);
        }
        if(this.tabs){
            this.tabs.removeTab(A.getEl().id);
        }else  if (!B){
            this.bodyEl.dom.removeChild(A.getEl().dom);
        }
        if(this.panels.getCount() == 1 && this.tabs && !this.config.alwaysShowTabs){
            var  p = this.panels.first();
            var  D = document.createElement("div"); 
            D.appendChild(p.getEl().dom);
            this.bodyEl.update("");
        ...
        if(this.config.autoDestroy !== false && B !== true){
            try{A.destroy();}catch(e){}
        }

        this.fireEvent("panelremoved", this, A);
        return  A;
    },
    ...
    createTool : function(A, B){
        var  C = Roo.DomHelper.append(A, {tag: "div", cls: "x-layout-tools-button",
            children: [{tag: "div", cls: "x-layout-tools-button-inner " + B, html: "&#160;"}]}, true);
        C.addClassOnOver("x-layout-tools-button-over");
        return  C;
    }
});
...
Roo.extend(Roo.SplitLayoutRegion, Roo.LayoutRegion, {
    ...
    applyConfig : function(A){
        Roo.SplitLayoutRegion.superclass.applyConfig.call(this, A);
        if(A.split){
            if(!this.split){
                var  B = Roo.DomHelper.append(this.mgr.el.dom, 
                        {tag: "div", id: this.el.id + "-split", cls: "x-layout-split x-layout-split-"+this.position, html: "&#160;"});
                

                this.split = new  Roo.SplitBar(B, this.el, this.orientation);
    ...
    getHMaxSize : function(){
         var  A = this.config.maxSize || 10000;
         var  B = this.mgr.getRegion("center");
         return  Math.min(A, (this.el.getWidth()+B.getEl().getWidth())-B.getMinWidth());
    },
    ...
    onSplitMove : function(A, B){
        this.fireEvent("resized", this, B);
    },
    ...
    animateCollapse : function(){
        ...
        this.el.slideOut(A, {
            callback : function(){
                this.el.setStyle("z-index", "");
                this.collapsedEl.slideIn(A, {duration:.3});
                this.afterSlide();
...
Roo.SouthLayoutRegion = function(A, B){
    Roo.SplitLayoutRegion.call(this, A, B, "south", "s-resize");
    if(this.split){
...
Roo.EastLayoutRegion = function(A, B){
    Roo.SplitLayoutRegion.call(this, A, B, "east", "e-resize");
    if(this.split){
...
Roo.extend(Roo.EastLayoutRegion, Roo.SplitLayoutRegion, {
    ...
    updateBox : function(A){
        if(this.split && !this.collapsed){
            ...
            A.width -= sw;
            this.split.el.setLeft(A.x);
            this.split.el.setTop(A.y);
            this.split.el.setHeight(A.height);
            A.x += sw;
        }
        if(this.collapsed){
            this.updateBody(null, A.height);
        }
...
Roo.WestLayoutRegion = function(A, B){
    Roo.SplitLayoutRegion.call(this, A, B, "west", "w-resize");
    if(this.split){
...
Roo.extend(Roo.WestLayoutRegion, Roo.SplitLayoutRegion, {
    ...
    updateBox : function(A){
        if(this.split && !this.collapsed){
            var  sw = this.split.el.getWidth();
            A.width -= sw;
            this.split.el.setLeft(A.x+A.width);
            this.split.el.setTop(A.y);
            this.split.el.setHeight(A.height);
        }
        if(this.collapsed){
...
Roo.LayoutStateManager.prototype = {
    init : function(A, B){
        ...
        this.layout = A;
        A.on("regionresized", this.onRegionResized, this);
        A.on("regioncollapsed", this.onRegionCollapsed, this);
        A.on("regionexpanded", this.onRegionExpanded, this);
    },
    ...
    onRegionResized : function(A, B){
        this.state[A.getPosition()].size = B;
        this.storeState();
    },
...
Roo.extend(Roo.ContentPanel, Roo.util.Observable, {
    ...
    _handleRefresh : function(A, B, C){
        if(!C || !this.loaded){
            var  D = this.el.getUpdateManager();
            D.update(A, B, this._setLoaded.createDelegate(this));
        }
    },
    ...
    adjustForComponents : function(A, B){
        if(this.resizeEl != this.el){
            A -= this.el.getFrameWidth('lr');
            B -= this.el.getFrameWidth('tb');
        }
        if(this.toolbar){
        ...
        if(this.adjustments){
            A += this.adjustments[0];
            B += this.adjustments[1];
        }
        return  {"width": A, "height": B};
    },
    
    setSize : function(A, B){
        if(this.fitToFrame && !this.ignoreResize(A, B)){
            if(this.fitContainer && this.resizeEl != this.el){
                this.el.setSize(A, B);
            }
            var  C = this.adjustForComponents(A, B);
            this.resizeEl.setSize(this.autoWidth ? "auto" : C.width, this.autoHeight ? "auto" : C.height);
    ...
    addxtype : function(A) {
        ...
        if (A.xtype.match(/^Form$/)) {
            var  el = this.el.createChild();

            this.form = new   Roo.form.Form(A);
        ...
        if (['View', 'JsonView', 'DatePicker'].indexOf(A.xtype) > -1) {
            
            A.el = this.el.appendChild(document.createElement("div"));
            
            
            var  B = new  Roo.factory(A);
            B.render && B.render(false, ''); 
            this.view = B;
            return  B;
        }
        return  false;
...
Roo.GridPanel = function(A, B){
    
  
    this.wrapper = Roo.DomHelper.append(document.body, 
        {tag: "div", cls: "x-layout-grid-wrapper x-layout-inactive-content"}, true);
        
    this.wrapper.dom.appendChild(A.getGridEl().dom);
    
    Roo.GridPanel.superclass.constructor.call(this, this.wrapper, B);
...
Roo.extend(Roo.GridPanel, Roo.ContentPanel, {
    ...
    setSize : function(A, B){
        if(!this.ignoreResize(A, B)){
            var  C = this.grid;
            var  D = this.adjustForComponents(A, B);
            C.getGridEl().setSize(D.width, D.height);
            C.autoSize();
        }
    },
...
Roo.NestedLayoutPanel = function(A, B)
{
    ...
    Roo.NestedLayoutPanel.superclass.constructor.call(this, A.getEl(), B);
...
Roo.extend(Roo.NestedLayoutPanel, Roo.ContentPanel, {
    ...
    setSize : function(A, B){
        if(!this.ignoreResize(A, B)){
            ...
            el.setSize(C.width, C.height);
            var  D = el.dom.offsetWidth;
            this.layout.layout();
    ...
    setActiveState : function(A){
        ...
        for (var  r  in  this.layout.regions) {
            B = this.layout.getRegion(r);
            if (B.getActivePanel()) {
                
                B.setActivePanel(B.getActivePanel());
                continue;
            }
            if (!B.panels.length) {
                continue;
    ...
    addxtype : function(A) {
        return  this.layout.addxtype(A);
    
    }
});

Roo.ScrollPanel = function(el, A, B){
    A = A || {};
    A.fitToFrame = true;
    Roo.ScrollPanel.superclass.constructor.call(this, el, A, B);
    ...
    var  up = C.createChild({cls: "x-scroller-up", html: "&#160;"}, this.el.dom);
    var  D = C.createChild({cls: "x-scroller-down", html: "&#160;"});
    up.unselectable(); D.unselectable();
    ...
    this.adjustments = [0, -(up.getHeight() + D.getHeight())];

    this.resizeEl = this.el;
    this.el = C; this.up = up; this.down = D;
};
...
Roo.TreePanel = function(A){
    ...
    var  C = el.createChild();
    A.resizeEl = C;
    
    
    
    Roo.TreePanel.superclass.constructor.call(this, el, A);
 
 
    this.tree = new  Roo.tree.TreePanel(C , B);
...
Roo.extend(Roo.grid.Grid, Roo.util.Observable, {
    ...
    reconfigure : function(A, B){
        if(this.loadMask){
            ...
            this.loadMask = new  Roo.LoadMask(this.container,
                    Roo.apply({store:A}, this.loadMask));
        }
    ...
    processEvent : function(A, e){
        this.fireEvent(A, e);
        var  t = e.getTarget();
        var  v = this.view;
        var  B = v.findHeaderIndex(t);
        if(B !== false){
            this.fireEvent("header" + A, this, B, e);
        }else {
            var  C = v.findRowIndex(t);
            var  D = v.findCellIndex(t);
            if(C !== false){
                this.fireEvent("row" + A, this, C, e);
                if(D !== false){
                    this.fireEvent("cell" + A, this, C, D, e);
                }
            }
        }
    },
    ...
    walkCells : function(A, B, C, fn, D){
        var  cm = this.colModel, E = cm.getColumnCount();
        var  ds = this.dataSource, F = ds.getCount(), G = true;
        if(C < 0){
            if(B < 0){
                A--;
                G = false;
...
Roo.extend(Roo.grid.AbstractGridView, Roo.util.Observable, {
	...
	getColumnRenderers : function(){
        ...
        var  B = cm.getColumnCount();
        for(var  i = 0; i < B; i++){
            A[i] = cm.getRenderer(i);
        }
        return  A;
    },
    ...
    setCSSStyle : function(A, B, C){
        var  D = "#" + this.grid.id + " .x-grid-col-" + A;
        Roo.util.CSS.updateRule(D, B, C);
    },
    
    generateRules : function(cm){
        var  A = [], B = this.grid.id + '-cssrules';
        Roo.util.CSS.removeStyleSheet(B);
        for(var  i = 0, C = cm.getColumnCount(); i < C; i++){
            var  D = cm.getColumnId(i);
...
Roo.extend(Roo.grid.HeaderDropZone, Roo.dd.DropZone, {
    ...
    positionIndicator : function(h, n, e){
        ...
        var  A = this.view.getCellIndex(h);
        var  B = this.view.getCellIndex(n);

        if(this.grid.colModel.isFixed(B)){
            return  false;
        }

        var  C = this.grid.colModel.isLocked(B);
        ...
        if(A == B && (C == this.grid.colModel.isLocked(A))){
            return  false;
        }
    ...
    onNodeEnter : function(n, dd, e, A){
        if(A.header != n){
            this.positionIndicator(A.header, n, e);
        }
    },
    ...
    onNodeOver : function(n, dd, e, A){
        var  B = false;
        if(A.header != n){
            B = this.positionIndicator(A.header, n, e);
        }
        if(!B){
            this.proxyTop.hide();
    ...
    onNodeDrop : function(n, dd, e, A){
        ...
        if(h != n){
            ...
            var  B = this.view.getCellIndex(h);
            var  C = this.view.getCellIndex(n);
            var  D = cm.isLocked(C);
            if(pt == "after"){
                C++;
            }
            ...
            if(B == C && (D == cm.isLocked(B))){
                return  false;
            }

            cm.setLocked(B, D, true);
            cm.moveColumn(B, C);
...
Roo.extend(Roo.grid.GridView, Roo.grid.AbstractGridView, {
    ...
    init: function(A){
        Roo.grid.GridView.superclass.init.call(this, A);
        ...
        if(!B.hsplit){
            B.hsplit = new  Roo.Template('<div class="x-grid-split {splitId} x-grid-split-{id}" style="{style}" unselectable="on">&#160;</div>');
        ...
        if(!B.row){
            B.row = new  Roo.Template('<tr class="x-grid-row {alt}">{cells}</tr>');
    ...
    refreshRow : function(A){
        ...
        this.insertRows(ds, B, B, true);
        this.onRemove(ds, A, B+1, true);
    ...
    onAdd : function(ds, A, B){
        this.insertRows(ds, B, B + (A.length-1));
    },

    onRemove : function(ds, A, B, C){
        if(C !== true){
            this.fireEvent("beforerowremoved", this, B, A);
        }
        var  bt = this.getBodyTable(), lt = this.getLockedTable();
        if(bt.rows[B]){
            bt.firstChild.removeChild(bt.rows[B]);
        }
        if(lt.rows[B]){
            lt.firstChild.removeChild(lt.rows[B]);
        }
        if(C !== true){
            this.stripeRows(B);
            this.syncRowHeights(B, B);
    ...
    getHeaderCell : function(A){
        return  Roo.DomQuery.select(this.headerSelector)[A];
    },
    ...
    getRowComposite : function(A){
        if(!this.rowEl){
            this.rowEl = new  Roo.CompositeElementLite();
        }
        var  B = [], C, D;
        if(C = this.getLockedRow(A)){
            B.push(C);
        }
        if(D = this.getRow(A)){
            B.push(D);
    ...
    getCell : function(A, B){
        var  C = this.cm.getLockedCount();
        var  D;
        if(B < C){
            D = this.lockedBody.dom.firstChild;
        }else {
            D = this.mainBody.dom.firstChild;
            B -= C;
        }
        return  D.rows[A].childNodes[B];
    },
    ...
    getCellBox : function(A){
        var  b = this.fly(A).getBox();
        if(Roo.isOpera){ 
            b.y = A.offsetTop + this.mainBody.getY();
        }
        return  b;
    },

    getCellIndex : function(A){
        var  id = String(A.className).match(this.cellRE);
        if(id){
    ...
    findCellIndex : function(A){
        var  B = this.el.dom;
        while(A && A != B){
            if(this.findRE.test(A.className)){
    ...
    onRowOver : function(e, t){
        var  A;
        if((A = this.findRowIndex(t)) !== false){
            this.getRowComposite(A).addClass("x-grid-row-over");
        }
    },

    onRowOut : function(e, t){
        var  A;
        if((A = this.findRowIndex(t)) !== false && A !== this.findRowIndex(e.getRelatedTarget())){
            this.getRowComposite(A).removeClass("x-grid-row-over");
        }
    },
    ...
    renderHeaders : function(){
        ...
        var  cb = [], lb = [], sb = [], A = [], p = {};
        for(var  i = 0, B = cm.getColumnCount(); i < B; i++){
            ...
            if(!cm.isLocked(i)){
                ...
                A[A.length] = st.apply(p);
            }
        }
        return  [ht.apply({cells: lb.join(""), splits:A.join("")}),
    ...
    focusCell : function(A, B, C)
    {
        
        var  el = this.ensureVisible(A, B, C);
    ...
    ensureVisible : function(A, B, C)
    {
        ...
        var  el = this.getCell(A, B);
        if(!el){
            return  null;
        }
        var  c = this.scroller.dom;

        var  D = parseInt(el.offsetTop, 10);
        var  E = parseInt(el.offsetLeft, 10);
        var  F = D + el.offsetHeight;
        var  G = E + el.offsetWidth;
        
        var  ch = c.clientHeight - this.mainHd.dom.offsetHeight;
        var  H = parseInt(c.scrollTop, 10);
        var  I = parseInt(c.scrollLeft, 10);
        var  J = H + ch;
        var  K = I + c.clientWidth;
    ...
    updateColumns : function(){
        ...
        var  cm = this.grid.colModel, A = this.getColumnIds();
        
        var  B = 0;
        for(var  i = 0, C = cm.getColumnCount(); i < C; i++){
            
            var  w = cm.getColumnWidth(i);
            this.css.updateRule(this.colSelector+this.idToCssName(A[i]), "width", (w - this.borderWidth) + "px");
            this.css.updateRule(this.hdSelector+this.idToCssName(A[i]), "width", (w - this.borderWidth) + "px");
        }
    ...
    generateRules : function(cm){
        var  A = [], B = this.idToCssName(this.grid.id)+ '-cssrules';
        Roo.util.CSS.removeStyleSheet(B);
        for(var  i = 0, C = cm.getColumnCount(); i < C; i++){
            var  D = cm.getColumnId(i);
            var  E = '';
            if(cm.config[i].align){
                E = 'text-align:'+cm.config[i].align+';';
            }
            var  F = '';
            if(cm.isHidden(i)){
                F = 'display:none;';
            }
            var  G = "width:" + (cm.getColumnWidth(i) - this.borderWidth) + "px;";
            A.push(
                    this.colSelector, D, " {\n", cm.config[i].css, E, G, "\n}\n",
                    this.hdSelector, D, " {\n", E, G, "}\n",
                    this.tdSelector, D, " {\n",F,"\n}\n",
    ...
    updateSplitters : function(){
        ...
        if(s){ 
            ...
            for(var  i = 0, C = cm.getColumnCount(); i < C; i++){
                ...
                A += w;
                s[i].style.left = (A-this.splitOffset) + "px";
            }
        }
    },

    handleHiddenChange : function(A, B, C){
        if(C){
            this.hideColumn(B);
        }else {
            this.unhideColumn(B);
    ...
    hideColumn : function(A){
        ...
        this.css.updateRule(this.tdSelector+this.idToCssName(B), "display", "none");
        this.css.updateRule(this.splitSelector+this.idToCssName(B), "display", "none");
        if(Roo.isSafari){
            this.updateHeaders();
        }
    ...
    unhideColumn : function(A){
        var  B = this.getColumnId(A);
        this.css.updateRule(this.tdSelector+this.idToCssName(B), "display", "");
        this.css.updateRule(this.splitSelector+this.idToCssName(B), "display", "");
    ...
    insertRows : function(dm, A, B, C){
        if(A == 0 && B == dm.getCount()-1){
            this.refresh();
        }else {
            if(!C){
                this.fireEvent("beforerowsinserted", this, A, B);
            }
            var  s = this.getScrollState();
            var  D = this.renderRows(A, B);
            this.bufferRows(D[0], this.getLockedTable(), A);
            this.bufferRows(D[1], this.getBodyTable(), A);
            this.restoreScroll(s);
            if(!C){
                this.fireEvent("rowsinserted", this, A, B);
                this.syncRowHeights(A, B);
                this.stripeRows(A);
                this.layout();
            }
        }
    ...
    bufferRows : function(A, B, C){
        var  D = null, E = B.rows, F = B.tBodies[0];
        if(C < E.length){
            D = E[C];
        }
        var  b = document.createElement("div");
        b.innerHTML = "<table><tbody>"+A+"</tbody></table>";
        var  G = b.firstChild.rows;
        for(var  i = 0, H = G.length; i < H; i++){
            if(D){
                F.insertBefore(G[0], D);
            }else {
                F.appendChild(G[0]);
            }
        }
    ...
    deleteRows : function(dm, A, B){
        if(dm.getRowCount()<1){
            ...
            this.fireEvent("beforerowsdeleted", this, A, B);
            ...
            this.stripeRows(A);
            this.fireEvent("rowsdeleted", this, A, B);
        }
    },

    updateRows : function(A, B, C){
        var  s = this.getScrollState();
        this.refresh();
        this.restoreScroll(s);
    },

    handleSort : function(A, B, C, D){
        if(!D){
           this.refresh();
        }
    ...
    stripeRows : function(A){
        if(!this.grid.stripeRows || this.ds.getCount() < 1){
            return;
        }

        A = A || 0;
        var  B = this.getBodyTable().rows;
        var  C = this.getLockedTable().rows;
        var  D = ' x-grid-row-alt ';
        for(var  i = A, E = B.length; i < E; i++){
            var  F = B[i], G = C[i];
            var  H = ((i+1) % 2 == 0);
            var  I = (' '+F.className + ' ').indexOf(D) != -1;
            if(H == I){
                continue;
            }
            if(H){
                F.className += " x-grid-row-alt";
            }else {
                F.className = F.className.replace("x-grid-row-alt", "");
            }
            if(G){
    ...
    renderRows : function(A, B){
        
        var  g = this.grid, cm = g.colModel, ds = g.dataSource, C = g.stripeRows;
        ...
        var  cs = [];
        for(var  i = 0; i < D; i++){
            var  E = cm.getDataIndex(i);
            cs[i] = {
                name : typeof  E == 'undefined' ? ds.fields.get(i).name : E,
        ...
        A = A || 0;
        B = typeof  B == "undefined"? ds.getCount()-1 : B;

        
        var  rs = ds.getRange(A, B);

        return  this.doRender(cs, rs, ds, A, D, C);
    },
    ...
    doRender : Roo.isGecko ?
            function(cs, rs, ds, A, B, C){
                var  ts = this.templates, ct = ts.cell, rt = ts.row;
                
                var  D = "", E = "", cb, F, c, p = {}, rp = {}, r, G;
                
                var  H = this.grid.hasListener('rowclass');
                var  I = {};
                for(var  j = 0, J = rs.length; j < J; j++){
                    r = rs[j]; cb = ""; F = ""; G = (j+A);
                    for(var  i = 0; i < B; i++){
                        c = cs[i];
                        p.cellId = "x-grid-cell-" + G + "-" + i;
                        p.id = c.id;
                        p.css = p.attr = "";
                        p.value = c.renderer(r.data[c.name], p, r, G, i, ds);
                    ...
                    var  L = [];
                    if(C && ((G+1) % 2 == 0)){
                        L.push("x-grid-row-alt")
                    }
                    if(r.dirty){
                        L.push(  " x-grid-dirty-row");
                    }

                    rp.cells = F;
                    if(this.getRowClass){
                        L.push(this.getRowClass(r, G));
                    }
                    if (H) {
                        I = {
                        ...
                        this.grid.fireEvent('rowclass', this, I);
                        L.push(I.rowClass);
                    }
            ...
            function(cs, rs, ds, A, B, C){
                ...
                for(var  j = 0, J = rs.length; j < J; j++){
                    ...
                    rp.alt = L.join(" ");
                    rp.cells = F.join("");
                    E[E.length] = rt.apply(rp);
                    rp.cells = cb.join("");
                    D[D.length] =  rt.apply(rp);
                }
    ...
    renderBody : function(){
        ...
        return  [bt.apply({rows: A[0]}), bt.apply({rows: A[1]})];
    },
    ...
    refresh : function(A){
        ...
        this.lockedBody.update(B[0]);
        this.mainBody.update(B[1]);
        if(A === true){
            this.updateHeaders();
            this.updateColumns();
            this.updateSplitters();
    ...
    handleColumnMove : function(cm, A, B){
        this.indexMap = null;
        var  s = this.getScrollState();
        this.refresh(true);
        this.restoreScroll(s);
    ...
    updateCell : function(dm, A, B){
        var  C = this.getColumnIndexByDataIndex(B);
        if(typeof  C == "undefined"){ 
            return;
        }
        var  cm = this.grid.colModel;
        var  D = this.getCell(A, C);
        var  E = this.getCellText(A, C);

        var  p = {
            cellId : "x-grid-cell-" + A + "-" + C,
            id : cm.getColumnId(C),
            css: C == cm.getColumnCount()-1 ? "x-grid-col-last" : ""
        };
        var  F = cm.getRenderer(C);
        var  G = F(dm.getValueAt(A, B), p, A, C, dm);
        if(typeof  G == "undefined" || G === "") G = "&#160;";
        E.innerHTML = G;
        D.className = this.cellClass + " " + this.idToCssName(p.cellId) + " " + p.css;
        this.syncRowHeights(A, A);
    },

    calcColumnWidth : function(A, B){
        var  C = 0;
        if(this.grid.autoSizeHeaders){
            var  h = this.getHeaderCellMeasure(A);
            C = Math.max(C, h.scrollWidth);
        }
        var  tb, D;
        if(this.cm.isLocked(A)){
            tb = this.getLockedTable();
            D = A;
        }else {
            tb = this.getBodyTable();
            D = A - this.cm.getLockedCount();
        }
        if(tb && tb.rows){
            var  E = tb.rows;
            var  F = Math.min(B || E.length, E.length);
            for(var  i = 0; i < F; i++){
                var  G = E[i].childNodes[D].firstChild;
                C = Math.max(C, G.scrollWidth);
            }
        }
        return  C + 
 5;
     ...
     autoSizeColumn : function(A, B, C){
         if(this.cm.isHidden(A)){
             return; 
         }
        if(B){
            var  D = this.cm.getColumnId(A);
            this.css.updateRule(this.colSelector +this.idToCssName( D), "width", this.grid.minColumnWidth + "px");
           if(this.grid.autoSizeHeaders){
               this.css.updateRule(this.hdSelector + this.idToCssName(D), "width", this.grid.minColumnWidth + "px");
           }
        }
        var  E = this.calcColumnWidth(A);
        this.cm.setColumnWidth(A,
            Math.max(this.grid.minColumnWidth, E), C);
        if(!C){
            this.grid.fireEvent("columnresize", A, E);
        }
    },
    ...
    fitColumns : function(A){
        ...
        var  F = (E - cm.getTotalWidth())/D;
        while (C.length){
            w = C.pop();
            i = C.pop();
            cm.setColumnWidth(i, Math.floor(w + w*F), true);
        }
    ...
    onRowSelect : function(A){
        var  B = this.getRowComposite(A);
        B.addClass("x-grid-row-selected");
    },
    ...
    onCellSelect : function(A, B){
        var  C = this.getCell(A, B);
        if(C){
            Roo.fly(C).addClass("x-grid-cell-selected");
        }
    },
    ...
    onCellDeselect : function(A, B){
        var  C = this.getCell(A, B);
        if(C){
            Roo.fly(C).removeClass("x-grid-cell-selected");
        }
    },
    ...
    updateHeaderSortState : function(){
        ...
        if (!this.ds.multiSort) { 
            ...
            A[B.field] = B.direction;
            
            this.sortState = B;
            
        } else  {
            A = this.ds.sortToggle;
        }
        
        
        var  sc = this.sortClasses;
        var  C = this.el.select(this.headerSelector).removeClass(sc);
        
        for(var  f  in  A) {
        
            var  D = this.cm.findColumnIndex(f);
            
            if(D != -1){
                var  E = A[f];        
                C.item(D).addClass(sc[E == "DESC" ? 1 : 0]);
            }
        }
    ...
    handleHeaderClick : function(g, A){
        ...
        if (dm.multiSort) {
            ...
            for(var  i = 0; i < cm.config.length; i++ ) {
                ...
                if ((typeof(dm.sortToggle[cm.config[i].dataIndex]) == 'undefined') && (A != i)) {
                    continue; 
                }
    ...
    destroy : function(){
        ...
        if(this.grid.enableColumnMove){
            var  A = Roo.dd.DDM.ids['gridHeader' + this.grid.getGridEl().id];
            if(A){
                for(var  dd  in  A){
                    if(!A[dd].config.isTarget && A[dd].dragElId){
                        var  B = A[dd].dragElId;
                        A[dd].unreg();
                        Roo.get(B).remove();
                    } else  if(A[dd].config.isTarget){
                        A[dd].proxyTop.remove();
                        A[dd].proxyBottom.remove();
                        A[dd].unreg();
                    }
    ...
    handleHdMenuClick : function(A){
        ...
        switch(A.id){
            case  "asc":
                ds.sort(cm.getDataIndex(B), "ASC");
                break;
            case  "desc":
                ds.sort(cm.getDataIndex(B), "DESC");
                break;
            case  "lock":
                ...
                if(lc != B){
                    cm.setLocked(B, true, true);
                    cm.moveColumn(B, lc);
                    this.grid.fireEvent("columnmove", B, lc);
                }else {
                    cm.setLocked(B, true);
                }
            break;
            case  "unlock":
                var  lc = cm.getLockedCount();
                if((lc-1) != B){
                    cm.setLocked(B, false, true);
                    cm.moveColumn(B, lc-1);
                    this.grid.fireEvent("columnmove", B, lc-1);
                }else {
                    cm.setLocked(B, false);
                }
            break;
            default:
                B = cm.getIndexById(A.id.substr(4));
                if(B != -1){
                    if(A.checked && cm.getColumnCount(true) <= 1){
                        this.onDenyColumnHide();
                        return  false;
                    }

                    cm.setHidden(B, A.checked);
                }
        }
        return  true;
    },
    ...
    beforeColMenuShow : function(){
        var  cm = this.cm,  A = cm.getColumnCount();
        this.colMenu.removeAll();
        for(var  i = 0; i < A; i++){
    ...
    handleHdCtx : function(g, A, e){
        e.stopEvent();
        var  hd = this.getHeaderCell(A);
        this.hdCtxIndex = A;
        var  ms = this.hmenu.items, cm = this.cm;
        ms.get("asc").setDisabled(!cm.isSortable(A));
        ms.get("desc").setDisabled(!cm.isSortable(A));
        if(this.grid.enableColLock !== false){
            ms.get("lock").setDisabled(cm.isLocked(A));
            ms.get("unlock").setDisabled(!cm.isLocked(A));
        }
    ...
    render : function(){
        ...
        var  C = this.templates.body.apply({rows:""});
        var  D = this.templates.master.apply({
            lockedBody: C,
            body: C,
            lockedHeader: B[0],
            header: B[1]
        ...
        this.grid.getGridEl().dom.innerHTML = D;
    ...
    onColumnSplitterMoved : function(i, w){
        ...
        var  A = cm.getColumnId(i);
        this.css.updateRule(this.colSelector + this.idToCssName(A), "width", (w-this.borderWidth) + "px");
        this.css.updateRule(this.hdSelector + this.idToCssName(A), "width", (w-this.borderWidth) + "px");
        this.updateSplitters();
        this.layout();
    ...
    layout : function(A, B){
        ...
        var  G = this.headerPanel.getHeight();
        var  H = this.footerPanel.getHeight();

        if(C){
            var  ch = this.getBodyTable().offsetHeight + G + H + this.mainHd.getHeight();
            var  I = ch + c.getBorderWidth("tb");
            if(g.maxHeight){
                I = Math.min(g.maxHeight, I);
            }
        ...
        var  J = c.getSize(true);

        this.el.setSize(J.width, J.height);

        this.headerPanel.setWidth(J.width);
        this.footerPanel.setWidth(J.width);
        ...
        var  M = bt.offsetHeight;
        var  N = L + bt.offsetWidth;
        var  O = false, P = false;

        this.scrollSizer.setSize(N, M+K);
        ...
        setTimeout(function(){
            ...
            if(B !== true && !gv.userResized && E){
                
                
                var  ci = cm.getIndexById(E);
                if (ci < 0) {
                    ci = cm.findColumnIndex(E);
                }

                ci = Math.max(0, ci); 
                var  Q = cm.getColumnId(ci);
                var   tw = cm.getTotalWidth(false);
                var  R = cm.getColumnWidth(ci);
                var  cw = Math.min(Math.max(((w-tw)+R-2)-
(w <= s.dom.offsetWidth ? 0 : 18), g.autoExpandMin), g.autoExpandMax);
                if(R != cw){
                    cm.setColumnWidth(ci, cw, true);
                    gv.css.updateRule(gv.colSelector+gv.idToCssName(Q), "width", (cw - gv.borderWidth) + "px");
                    gv.css.updateRule(gv.hdSelector+gv.idToCssName(Q), "width", (cw - gv.borderWidth) + "px");
                    gv.updateSplitters();
...
Roo.grid.GridView.ColumnDragZone = function(A, hd){
    Roo.grid.GridView.ColumnDragZone.superclass.constructor.call(this, A, hd, null);
...
Roo.extend(Roo.grid.SplitDragZone, Roo.dd.DDProxy, {
    ...
    b4StartDrag : function(x, y){
        ...
        var  A = Math.max(w-this.grid.minColumnWidth, 0);
        this.resetConstraints();
        this.setXConstraint(A, 1000);
    ...
    endDrag : function(e){
        ...
        var  A = Math.max(this.minX, Roo.lib.Event.getPageX(e));
...
Roo.extend(Roo.grid.GridDragZone, Roo.dd.DragZone, {
    ...
    getDragData : function(e){
        var  t = Roo.lib.Event.getTarget(e);
        var  A = this.view.findRowIndex(t);
        if(A !== false){
            ...
            return  {grid: this.grid, ddel: this.ddel, rowIndex: A, selections:sm.getSelections()};
        }
        return  false;
    },
...
Roo.extend(Roo.grid.ColumnModel, Roo.util.Observable, {
    ...
    getColumnByDataIndex: function(A){
        var  B = this.findColumnIndex(A);
        return  B > -1 ? this.config[B] : false;
    },
    
    

    getIndexById : function(id){
        for(var  i = 0, A = this.config.length; i < A; i++){
    ...
    findColumnIndex : function(A){
        for(var  i = 0, B = this.config.length; i < B; i++){
            if(this.config[i].dataIndex == A){
                return  i;
            }
        }
        return  -1;
    ...
    setLocked : function(A, B, C){
        if(this.isLocked(A) == B){
            return;
        }

        this.config[A].locked = B;
        if(!C){
            this.fireEvent("columnlockchange", this, A, B);
        }
    },
    ...
    getColumnsBy : function(fn, A){
        var  r = [];
        for(var  i = 0, B = this.config.length; i < B; i++){
            var  c = this.config[i];
            if(fn.call(A||this, c, i) === true){
                r[r.length] = c;
            }
        }
        return  r;
    ...
    setRenderer : function(A, fn){
        this.config[A].renderer = fn;
    },

    

    getColumnWidth : function(A){
        return  this.config[A].width * 1 || this.defaultWidth;
    },

    

    setColumnWidth : function(A, B, C){
        this.config[A].width = B;
        this.totalWidth = null;
        if(!C){
             this.fireEvent("widthchange", this, A, B);
        }
    },
...
Roo.grid.ColumnModel.defaultRenderer = function(A){
...
Roo.extend(Roo.grid.RowSelectionModel, Roo.grid.AbstractSelectionModel,  {
    ...
    selectRecords : function(A, B){
        if(!B){
            this.clearSelections();
        }
        var  ds = this.grid.dataSource;
        for(var  i = 0, C = A.length; i < C; i++){
            this.selectRow(ds.indexOf(A[i]), true);
        }
    },
    ...
    selectAll : function(){
        if(this.locked) return;
        this.selections.clear();
        for(var  i = 0, A = this.grid.dataSource.getCount(); i < A; i++){
    ...
    isSelected : function(A){
        var  r = typeof  A == "number" ? this.grid.dataSource.getAt(A) : A;
    ...
    handleMouseDown : function(e, t){
        var  A = this.grid.getView(), B;
        if(this.isLocked() || (B = A.findRowIndex(t)) === false){
            return;
        };
        if(e.shiftKey && this.last !== false){
            ...
            if(e.button !== 0 && D){
                ...
                this.selectRow(B, e.button === 0 && (e.ctrlKey || e.shiftKey));
                A.focusRow(B);
            }
        }
    ...
    handleDragableRowClick :  function(A, B, e) 
    {
    ...
    selectRow : function(A, B, C){
        if(this.locked || (A < 0 || A >= this.grid.dataSource.getCount())) return;
        if(this.fireEvent("beforerowselect", this, A, B) !== false){
            if(!B || this.singleSelect){
                this.clearSelections();
            }
            ...
            this.fireEvent("rowselect", this, A, r);
    ...
    acceptsNav : function(A, B, cm){
        return  !cm.isHidden(B) && cm.isCellEditable(B, A);
    },

    
    onEditorKey : function(A, e){
        var  k = e.getKey(), B, g = this.grid, ed = g.activeEditor;
        if(k == e.TAB){
            e.stopEvent();
            ed.completeEdit();
            if(e.shiftKey){
                B = g.walkCells(ed.row, ed.col-1, -1, this.acceptsNav, this);
            }else {
                B = g.walkCells(ed.row, ed.col+1, 1, this.acceptsNav, this);
            }
            ...
            ed.completeEdit();
            if(e.shiftKey){
                B = g.walkCells(ed.row-1, ed.col, -1, this.acceptsNav, this);
            }else {
                B = g.walkCells(ed.row+1, ed.col, 1, this.acceptsNav, this);
            }
        }else  if(k == e.ESC){
...
Roo.extend(Roo.grid.CellSelectionModel, Roo.grid.AbstractSelectionModel,  {
    ...
    onRowUpdated : function(v, A, r){
        if(this.selection && this.selection.record == r){
            v.onCellSelect(A, this.selection.cell[1]);
        }
    },
    ...
    handleMouseDown : function(e, t){
        ...
        var  A = v.findRowIndex(t);
        var  B = v.findCellIndex(t);
        if(A !== false && B !== false){
            this.select(A, B);
        }
    },
 ...
 r){
        if(this.fireEvent("beforecellselect", this, A, B) !== false){
            this.clearSelections();
            r = r || this.grid.dataSource.getAt(A);
            this.selection = {
                record : r,
                cell : [A, B]
            };
            ...
            this.fireEvent("cellselect", this, A, B);
            this.fireEvent("selectionchange", this, this.selection);
        }
    },

	
    isSelectable : function(A, B, cm){
        return  !cm.isHidden(B);
    },
    ...
    handleKeyDown : function(e){
        ...
        if(!s){
            e.stopEvent();
            var  A = g.walkCells(0, 0, 1, this.isSelectable,  this);
            if(A){
                this.select(A[0], A[1]);
            }
            return;
        ...
        var  B = function(D, E, F){
            return  g.walkCells(D, E, F, sm.isSelectable,  sm);
        };
    ...
    onEditorKey : function(A, e){
        ...
        if(B){
            ...
            g.startEditing.defer(100, g, [B[0], B[1]]);
        }
    }
});
...
Roo.extend(Roo.grid.EditorGrid, Roo.grid.Grid, {
    ...
    onCellDblClick : function(g, A, B){
        this.startEditing(A, B);
    },

    onEditComplete : function(ed, A, B){
        this.editing = false;
        this.activeEditor = null;
        ed.un("specialkey", this.selModel.onEditorKey, this.selModel);
        var  r = ed.record;
        var  C = this.colModel.getDataIndex(ed.col);
        var  e = {
            grid: this,
            record: r,
            field: C,
    ...
    startEditing : function(A, B){
        this.stopEditing();
        if(this.colModel.isCellEditable(B, A)){
            this.view.ensureVisible(A, B, true);
            var  r = this.dataSource.getAt(A);
            var  C = this.colModel.getDataIndex(B);
            var  e = {
                grid: this,
                record: r,
                field: C,
            ...
            if(this.fireEvent("beforeedit", e) !== false && !e.cancel){
                this.editing = true;
                var  ed = this.colModel.getCellEditor(B, A);
                ...
                (function(){ 
                    ...
                    var  v = r.data[C];
                    ed.startEdit(this.view.getCell(A, B), v);
...
Roo.extend(Roo.grid.PropertyStore, Roo.util.Observable, {
    setSource : function(o){
        ...
        for(var  k  in  o){
            if(this.isEditableValue(o[k])){
                A.push(new  Roo.grid.PropertyRecord({name: k, value: o[k]}, k));
            }
        }

        this.store.loadRecords({records: A}, {}, true);
    },

    onUpdate : function(ds, A, B){
        if(B == Roo.data.Record.EDIT){
            var  v = A.data['value'];
            var  C = A.modified['value'];
            if(this.grid.fireEvent('beforepropertychange', this.source, A.id, v, C) !== false){
                this.source[A.id] = v;
                A.commit();
                this.grid.fireEvent('propertychange', this.source, A.id, v, C);
            }else {
                A.reject();
            }
        }
    ...
    setValue : function(A, B){
        this.source[A] = B;
        this.store.getById(A).set('value', B);
    },
...
Roo.grid.PropertyColumnModel = function(A, B){
    this.grid = A;
    var  g = Roo.grid;
...
Roo.extend(Roo.grid.PropertyColumnModel, Roo.grid.ColumnModel, {
    ...
    renderCell : function(A){
        ...
        if(A  instanceof  Date){
            rv = this.renderDate(A);
        }else  if(typeof  A == 'boolean'){
            rv = this.renderBool(A);
        }
    ...
    getPropertyName : function(A){
        ...
        return  pn && pn[A] ? pn[A] : A;
    },

    getCellEditor : function(A, B){
        var  p = this.store.getProperty(B);
        var  n = p.data['name'], C = p.data['value'];
...
Roo.grid.PropertyGrid = function(A, B){
    B = B || {};
    var  C = new  Roo.grid.PropertyStore(this);
    this.store = C;
    var  cm = new  Roo.grid.PropertyColumnModel(this, C);
    C.store.sort('name', 'ASC');
    Roo.grid.PropertyGrid.superclass.constructor.call(this, A, Roo.apply({
        ds: C.store,
        cm: cm,
        enableColLock:false,
...
Roo.XTemplate = function(){
    ...
    while(m = s.match(re)){
       ...
       var  m3 = m[0].match(B);
       var  m4 = m[0].match(C);
       var  E = null, fn = null, F = null;
       var  G = m2 && m2[1] ? m2[1] : '';
       if(m3){
           E = m3 && m3[1] ? m3[1] : null;
           if(E){
               fn = new  Function('values', 'parent', 'with(values){ return '+(Roo.util.Format.htmlDecode(E))+'; }');
           }
       }
       if(m4){
           E = m4 && m4[1] ? m4[1] : null;
           if(E){
               F = new  Function('values', 'parent', 'with(values){ '+(Roo.util.Format.htmlDecode(E))+'; }');
           }
       }
       if(G){
           switch(G){
               case  '.': G = new  Function('values', 'parent', 'with(values){ return values; }'); break;
               case  '..': G = new  Function('values', 'parent', 'with(values){ return parent; }'); break;
               default: G = new  Function('values', 'parent', 'with(values){ return '+G+'; }');
           }
       }
...
Roo.extend(Roo.XTemplate, Roo.Template, {
    ...
    applySubTemplate : function(id, A, B){
        var  t = this.tpls[id];
        if(t.test && !t.test.call(this, A, B)){
            return  '';
        }
        if(t.exec && t.exec.call(this, A, B)){
            return  '';
        }
        var  vs = t.target ? t.target.call(this, A, B) : A;
        B = t.target ? A : B;
        if(t.target && vs  instanceof  Array){
            var  C = [];
            for(var  i = 0, D = vs.length; i < D; i++){
                C[C.length] = t.compiled.call(this, vs[i], B);
            }
            return  C.join('');
        }
        return  t.compiled.call(this, vs, B);
    },

    compileTpl : function(A){
        var  fm = Roo.util.Format;
        var  B = this.disableFormats !== true;
        var  C = Roo.isGecko ? "+" : ",";
        var  fn = function(m, E, F, G){
            if(E.substr(0, 4) == 'xtpl'){
                return  "'"+ C +'this.applySubTemplate('+E.substr(4)+', values, parent)'+C+"'";
            }
            var  v;
            if(E.indexOf('.') != -1){
                v = E;
        ...
        if(Roo.isGecko){
            D = "tpl.compiled = function(values, parent){ return '" +
                   A.body.replace(/(\r\n|\n)/g, '\\n').replace(/'/g, "\\'").replace(this.re, fn) +
                    "';};";
        }else {
            D = ["tpl.compiled = function(values, parent){ return ['"];
            D.push(A.body.replace(/(\r\n|\n)/g, '\\n').replace(/'/g, "\\'").replace(this.re, fn));
            D.push("'].join('');};");
            D = D.join('');
        }
...
Roo.extend(Roo.XComponent, Roo.util.Observable, {
    ...
    render : function(el)
    {
        ...
        if (!el && typeof(this.parent) == 'string' && this.parent.substring(0,1) == '#') {
            ...
            var  A = this.parent.substr(1)

            this.parent = false;
            el = Roo.get(A);
            if (!el) {
                Roo.log("Warning - element can not be found :#" + A );
                return;
            }
        }
        ...
        var  B = this.tree();
        B.region = B.region || this.region;
        this.el = this.parent.el.addxtype(B);
...
Roo.apply(Roo.XComponent, {
    ...
    {
        var  _t = this;
        Roo.each(this.modules , function (A)
        {
            var  B = A.parent;
            try { 
                A.parent = this.toObject(B);
            } catch(e) {
                Roo.log(e.toString());
                return;
            }
            ...
            if (typeof(A.parent) == 'string') {
                this.elmodules.push(A);
                return;
            }
            if (A.parent.constructor != Roo.XComponent) {
                Roo.log("Object Parent is not instance of XComponent:" + A.name)
            }
            if (!A.parent.modules) {
                A.parent.modules = new  Roo.util.MixedCollection(false, 
                    function(o) { return  o.order + '' }
                );
            }

            
            A.parent.modules.add(A);
        }, this);
    },
    ...
    buildOrder : function()
    {
        ...
        var  C = this.topModule ? [ this.topModule ] : [];
        Roo.each(this.elmodules,function(e) { C.push(e) });
        ...
        if (this.topModule) { 
            this.topModule.modules.keySort('ASC',  B );
            this.topModule.modules.each(D);
        }
        return  C;
    },
    ...
    build : function() 
    {
        ...
        var  D = function() {
            if (!A.length) {
                ...
                Roo.MessageBox.hide();
                if (C.topModule) { 
                    C.topModule.fireEvent('buildcomplete', C.topModule);
                }
            ...
            Roo.MessageBox.updateProgress(
                (B  - A.length)/B,  "Building Interface " + (B  - A.length) + 
                    " of " + B + 
                    (m.name ? (' - ' + m.name) : '')
                    );
            ...
            var  E = (typeof(m.disabled) == 'function') ?
...
Roo.extend(Roo.Login, Roo.LayoutDialog, {
    ...
    onLoad : function() 
    {
        ...
        this.check({
            success:  function(A, B)  {  
                ...
                var  C = this.processResponse(A);
                this.checkFails =0;
                if (!C.success) { 
                    this.checkFails = 5;
                    
                    return  this.failure(A,B);
                }
    ...
    check: function(A) 
    {
        ...
        Roo.Ajax.request({  
            ...
            method: this.method,
            success:  A.success || this.success,
            failure : A.failure || this.failure,
            scope : this,
            callCfg : A
    ...
    processResponse : function (A)
    {
        var  B = '';
        try {
            B = Roo.decode(A.responseText);
            
            if (typeof(B) != 'object') {
                B = { success : false, errorMsg : B, errors : true };
            }
    ...
    failure : function (A, B) 
    {
        ...
        if ( this.checkFails > 2) {
            ...
            Roo.MessageBox.alert("Error", C.errorMsg ? C.errorMsg : 
                "Error getting authentication status. - try reloading"); 
            return;
        }

        B.callCfg.again = true;
        this.check.defer(1000, this, [ B.callCfg ]);
        return;  
    },
    ...
    switchLang : function (A) 
    {
        ...
        if (!_T && A != 'en') {
            Roo.MessageBox.alert("Sorry", "Language not available yet (" + A +')');
            return;
        }
    ...
    listeners : {
        ...
        show  : function(A)
        {
            ...
            this.buttons[0].dialog = A;
            this.buttons[1].form = this.form;
            this.buttons[1].dialog = A;
    ...
    
                {
                    ...
                    listeners : {
                        actionfailed : function(f, A) {
                            ...
                            Roo.MessageBox.alert("Error", A.result.errorMsg ? A.result.errorMsg : 
                                        "Login failed - communication error - try again.");
                                      
                        },
                        actioncomplete: function(re, A) {
                            ...
                            this.dialog.fillAuth(A.result.data);
                    ...
                    items : [
                        ...
                        },
                        {
                            ...
                            selectOnFocus:true,
                            listeners : {
                                select :  function(cb, A, ix) {
                                    this.form.switchLang(A.data.lang);
                                }
                            }
                ...
                click : function() {
                    ...
                    Roo.Ajax.request({
                        ...
                        success:  function(A, B)  {  
                            ...
                            var  C = this.dialog.processResponse(A);
                            if (!C.success) { 
                               Roo.MessageBox.alert("Error" ,
                                    C.errorMsg ? C.errorMsg  : "Problem Requesting Password Reset");
                               return;
                            }
