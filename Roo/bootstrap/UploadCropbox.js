...
Roo.extend(Roo.bootstrap.UploadCropbox, Roo.bootstrap.Component,  {
    ...
    crop : function()
    {
        ...
        var cropWidth = thumbWidth;
        var cropHeight = thumbHeight;
        
        if(this.scale < 0){
            x = x * Math.pow(1.1, this.scale) * -1;
            y = y * Math.pow(1.1, this.scale) * -1;
            
            cropWidth = cropWidth * Math.pow(1.1, this.scale) * -1
        }
        ...
        context.drawImage(this.image.dom, x, y, this.image.OriginWidth, this.image.OriginHeight, 0, 0, thumbWidth, thumbHeight);
